...
*   Asset Importers: Importing some assets store packages fails while editor is trying to import .meta file ([1186510](https://issuetracker.unity3d.com/issues/importing-some-assets-store-packages-fails-while-editor-is-trying-to-import-meta-file))
...
### New 2019.3.0b5 Entries since 2019.3.0b4
...
*   Version Control: Add Revert button to inspector VCS status bar. ([1182214](https://issuetracker.unity3d.com/issues/vcs-have-revert-button-in-inspector-vcs-bar), 1184215)  
    _This is a new issue, not seen in any released version._
...
*   Animation: Prevent users from attempting to add properties in the animation window when the game object's hierarchy does not match the Animator's human avatar. ([1162335](https://issuetracker.unity3d.com/issues/animation-failing-to-add-animation-property-to-the-parent-of-a-rigged-character), 1183925)
...
*   Editor: A new script is not being opened as a different tab when using VSCode ([1173165](https://issuetracker.unity3d.com/issues/mac-a-new-script-is-not-being-opened-as-a-different-tab-when-using-vscode), 1175199)  
    _This has already been backported to older releases._
    
*   Editor: Double clicking on the Error entry in the Unity Editor console does not open the line causing the error ([1171319](https://issuetracker.unity3d.com/issues/mac-vscode-editor-vscode-does-not-show-the-error-line-when-double-clicking-on-the-error-entry-in-unity-editor-console), 1175921)  
    _This has already been backported to older releases._
...
*   Editor: Fix splashscreen background not being built correctly ([1177632](https://issuetracker.unity3d.com/issues/background-image-missing-during-splash-screen), 1183040)  
    _This has already been backported to older releases._
    
*   Editor: Fixed an issue where framing the selected object to the Scene View Camera could result in the Move Tool rendering incorrectly. ([1144461](https://issuetracker.unity3d.com/issues/move-gizmo-glitches-when-moving-the-camers-close-to-a-smaller-object), 1184912)
...
*   Editor: Fixed incorrect label width when drawing Splash Screen logo list items. (1175164, 1182161)
    
*   Editor: Fixed some edge cases that could result in duplicate EditorTool instances being instantiated. (1178894, 1184916)
    
*   Editor: Hide "Select Icon" for annotation/gizmo that belongs to immutable package. ([1117690](https://issuetracker.unity3d.com/issues/textmeshpro-gizmos-icon-appear-again-after-being-disabled-when-gizmos-pop-up-is-dismissed), 1180076)
    
*   Graphics: Fixed Streamed AudioClip causes stack overflow, glitching and a crash when closing the player in Linux ([1167107](https://issuetracker.unity3d.com/issues/linux-streamed-audioclip-causes-stack-overflow-glitching-and-a-crash-when-closing-the-player), 1182312)  
    _This has already been backported to older releases._
    
*   Graphics: New option in Player Settings to select number of swapchains when using Vulkan.  
    _This is a new issue, not seen in any released version._
    
*   IL2CPP: Avoid an error from the Unity Linker when a type from a different assembly is used as a constructor argument for an attribute and passed to an object array. ([1180541](https://issuetracker.unity3d.com/issues/fatal-error-in-unity-cil-linker-for-type-within-attribute-when-building-with-medium-or-high-stripping-level), 1183697)
    
*   IL2CPP: Correct the behavior of Marshal.OffsetOf for byval arrays of types with 4 byte alignment when they are not the first field in a struct. ([1182074](https://issuetracker.unity3d.com/issues/64bit-il2cpp-marshal-dot-offsetof-returns-incorrect-offset), 1183541)
...
*   iOS: Better handle UnityWebRequest timeout. ([1175344](https://issuetracker.unity3d.com/issues/ios-unitywebrequest-dot-sendwebrequest-timeout-property-is-being-ignored-until-the-server-response), 1183524)
    
*   iOS: Use single NSURLSession for all UnityWebRequests reducing memory footprint and fixing memory leaks. ([1177888](https://issuetracker.unity3d.com/issues/memory-leak-when-sending-unitywebrequests), 1183979)
    
*   Linux: Fixed so that Input.GetAxis returns correct values when running a built project (1176709, 1184939)  
    _This has already been backported to older releases._
    
*   Package Manager: Fix Package Manager showing incorrect supported unity versions for asset store asset (1179747, 1184518)  
    _This is a 2019.3 issue, not seen in any released version._
    
*   Package Manager: Fix Package Manager UI showing asset publish date in system locale (1179741, 1184561)
    
*   Package Manager: Fixed Package Tests been only added when making a package testable and adding a dummy package. ([1135941](https://issuetracker.unity3d.com/issues/package-tests-are-only-added-when-making-a-package-testable-and-adding-a-dummy-package), 1179575)
...
*   Physics: Fix a typo in the name of a solver, should have been Gauss-Seidel (1174607, 1184829)  
    _This is a new issue, not seen in any released version._
...
*   Scripting: Trigger script compilation when define symbols or allow unsafe code are modified from a external tool. ([1171967](https://issuetracker.unity3d.com/issues/recompilation-is-not-triggered-when-projectsettings-dot-asset-file-changes-while-unity-is-open), 1179585)  
    _This has already been backported to older releases._
...
*   UI: Fix UGUI scaling issues when interacting with various Editor windows ([1179289](https://issuetracker.unity3d.com/issues/scene-slash-game-view-ui-is-jittery-slash-scales-randomly-when-cycling-through-gameobjects-and-canvas-scaler-mode-is-scale-with-screen), 1184439)  
    _This is a 2019.3 issue, not seen in any released version._
...
*   UI Elements: Fixed white rectangles in Linux/OpenGL when using overflow=hidden in UIElements. ([1164245](https://issuetracker.unity3d.com/issues/graphview-nodes-are-displayed-white-on-linux), 1183079)  
    _This is a new issue, not seen in any released version._
...
*   UI Elements: Fixed white rectangles in Windows/Vulkan when using overflow=hidden in UIElements. ([1175622](https://issuetracker.unity3d.com/issues/vfx-editor-draws-white-nodes-on-windows-slash-vulkan), 1183080)  
    _This is a new issue, not seen in any released version._
    
*   UI Elements: Some items in ListView become invisible when DisplayStyle is changed to None and then back to Flex ([1181499](https://issuetracker.unity3d.com/issues/uielements-some-items-in-listview-become-invisible-when-displaystyle-is-changed-to-none-and-then-back-to-flex), 1185438)
    
*   Version Control: Fixed API Updater exceptions when it tries to update scripts in a non-local Package. ([1181750](https://issuetracker.unity3d.com/issues/vcs-adding-probuilder-package-while-under-version-control-throws-api-updater-exceptions), 1181977)  
    _This is a new issue, not seen in any released version._
    
*   WebGL: Fixed modifier keys for simulated mouse events in WebGL. ([1143387](https://issuetracker.unity3d.com/issues/webgl-webgl-builds-has-different-gui-event-handling-compared-to-the-editor), 1176146)  
    _This has already been backported to older releases._
    

### Preview of Final 2019.3.0b5 Release Notes
...
*   Profiler: Fixed an issue where there was reduced memory usage due to fragmentation ([1168484](https://issuetracker.unity3d.com/issues/unity-leaks-memory-when-doing-assertion-with-interpolated-string-when-burst-compilation-is-off-and-profiler-is-recording), 1178375)
...
*   XR: Fixed Using the TPD and Depth Support on Oculus results in visual artifacts ([1142620](https://issuetracker.unity3d.com/issues/black-screen-on-oculus-rift-when-shared-dpeth-buffer-on-oculus-sdk-enabled-and-tracked-pose-drive-applied-on-main-camera-gameobj), 1183779)
