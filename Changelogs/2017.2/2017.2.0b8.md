...
*   Android: Shader `#pragma` target 4.0 4.6 and 5.0 not working on Android phone [(933203)](https://issuetracker.unity3d.com/issues/shaders-regression-number-pragma-target-4-dot-0-4-dot-6-5-dot-0-not-working-on-some-androids)
...
*   Build Pipeline: \[ARKit\] Unity crashes when building a scene [(932080)](https://issuetracker.unity3d.com/issues/arkit-unity-crashes-when-building-a-scene)
*   Editor: BeginsWithCaseInsensitive Assertion Error when creating a folder from the create menu (931888)
*   Editor: Camera.targetTexture is not updated from OnPreRender to OnPostRender [(937031)](https://issuetracker.unity3d.com/issues/camera-dot-targettexture-is-not-updated-from-onprerender-to-onpostrender)
...
*   Graphics: Camera using temporary RenderTexture breaks on the last frames(Frame Debugger) [(932065)](https://issuetracker.unity3d.com/issues/camera-using-temporary-rendertexture-breaks-on-the-last-frames-frame-debugger)
*   Graphics: GameObjects mesh is stuck at Combined Mesh (root:scene) and cannot be moved after aborting a build process [(891971)](https://issuetracker.unity3d.com/issues/gameobjects-mesh-is-stuck-at-combined-mesh-root-scene-and-cannot-be-moved-after-aborting-a-build-process)
...
*   Graphics: TextMeshPro is not displayed, and errors are logged. [(929909)](https://issuetracker.unity3d.com/issues/the-textmeshpro-text-is-not-displayed-on-the-screen-the-errors-appear)
...
*   Scripting: `MonoBehaviour::GetObjectGUIState` crashes when selecting color via Color Picker. [(934613)](https://issuetracker.unity3d.com/issues/windows-monobehaviour-getobjectguistate-crashes-when-selecting-color-via-color-picker)
...
*   WebGL: Memory leak when building for WebGL. [(935474)](https://issuetracker.unity3d.com/issues/memory-leak-when-building-for-webgl)
*   Windows: \[WSA\] Error messages appear while/after building a project with "Unity C# projects", if the Editor folder contains a script. [(918989)](https://issuetracker.unity3d.com/issues/wsa-error-messages-appear-while-slash-after-building-a-project-with-unity-c-number-projects-and-if-the-editor-folder-contains-a-script)
*   Windows: \[XR\]\[WSA\] Including "Unity C# Projects" while building to Universal Windows Platform, causes builds to fail [(927925)](https://issuetracker.unity3d.com/issues/xr-wsa-including-unity-c-number-projects-while-building-to-universal-windows-platform-causes-builds-to-fail)
...
*   Editor: Ensure we always provide an asset preview. [(858928)](https://issuetracker.unity3d.com/issues/assetpreview-dissapears-after-clearing-cache-when-using-renderstaticpreview)
*   Editor: Fix in-launcher names of projects loaded via relative path. [(873823)](https://issuetracker.unity3d.com/issues/when-the-path-of-project-is-loaded-with-in-the-end-project-name-gets-overridden-with-in-the-project-list)
*   Editor: Fix screen capture. [(932630)](https://issuetracker.unity3d.com/issues/editor-becomes-unresponsive-when-calling-screencapture-dot-capturescreenshot-if-scale-value-of-game-view-is-less-than-1)
*   Editor: Fixed bug where entering play mode made part of PlayerSettings inspector disappear until it repainted at least once. [(930646)](https://issuetracker.unity3d.com/issues/half-of-other-settings-options-disappears-after-entering-playmode)
*   Editor: Fixed Security Vulnerability UNITY-SEC-844 - [https://unity3d.com/security](/security)
...
*   Scripting: Fix internal compiler error due to `yield` in a lambda statement. [(634364)](https://issuetracker.unity3d.com/issues/a-yield-statement-inside-a-lambda-expression-which-is-a-callback-of-another-coroutine-causes-an-internal-compiler-error)
*   Scripting Upgrade: Avoid allocating GC memory when calling `List.Sort` with `Comparer\`1\` [(935845)](https://issuetracker.unity3d.com/issues/list-dot-sort-causes-memory-allocations-with-net-4-dot-6-runtime-but-not-net-3-dot-5)
*   Scripting Upgrade: Report GC pause time correctly in profiler. [(931981)](https://issuetracker.unity3d.com/issues/profiler-shows-garbage-collection-execution-time-as-script-execution-time-when-using-experimental-net-4-dot-6)
*   Shaders: Set the correct value for the `SHADER_TARGET` preprocessor macro during Surface Shader analysis to match the value used during Surface Shader compilation. [(893229)](https://issuetracker.unity3d.com/issues/surface-shader-does-not-display-correct-color-if-using-number-ifdef-on-parallaxmap-preprocessor-variable-to-check-if-it-is-defined)
*   Timeline: Fix for hidden time area when reopening Timeline. (935500)
*   Timeline: Fix for timeline clip modifications not always applying. [(926861)](https://issuetracker.unity3d.com/issues/timeline-clip-name-is-not-saved-after-renaming-it)
*   Timeline: Fix Playable Director not dirtying properly when changing wrap mode. [(930195)](https://issuetracker.unity3d.com/issues/changing-the-value-for-wrap-mode-on-a-playable-director-doesnt-mark-it-as-dirty)
*   Timeline: Fix preview mode on root transforms when recording. (931859)
*   Timeline: Fix runtime exception on AOT compiled builds. [(937737)](https://issuetracker.unity3d.com/issues/timeline-ios-build-causes-executionengineexception-attempting-to-jit-compile-method-in-xcode)
