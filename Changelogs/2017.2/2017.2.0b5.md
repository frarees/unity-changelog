...
*   Graphics: Fixed memory leak in `Texture2DArray` and `CubeMapArrayTexture`. [(919162, 930677)](https://issuetracker.unity3d.com/issues/texture2darray-dot-apply-makenolongerreadable-true-doesnt-free-heap-memory)
...
*   Animation: Fixed a bug where Generic animation layers were affecting base humanoid root motion. [(928562)](https://issuetracker.unity3d.com/issues/anim-layer-with-no-rm-overrides-rm-on-base-layer)
*   Animation: Fixed a bug where humanoid body position was not working in additve. [(916487)](https://issuetracker.unity3d.com/issues/additive-layer-in-animator-window-on-humanoid-rig-type-cause-avatar-hips-to-be-placed-at-around-00-0)
*   Animation: Fixed a crash that was triggered during the assignment of an incomplete `AnimatorController` stored in an `AssetBundle`. [(925276, 927441)](https://issuetracker.unity3d.com/issues/crash-in-animationclip-buildmecanimdatamainthread-when-entering-a-play-mode)
*   Animation: Fixed a glitch that was causing a stuttering in the Animation curve when rotating a GameObject in the Animation window. [(924605, 930334)](https://issuetracker.unity3d.com/issues/animation-curve-is-stuttering-while-rotating-the-game-object)
*   Animation: Fixed case of `AnimatorState`'s WriteDefaults not working properly on very specific setups. [(893779)](https://issuetracker.unity3d.com/issues/some-parts-of-the-model-becomes-invisable-when-animations-blend)
*   Animation: Fixed crash when changing OverrideController during interrupted transition. [(928601)](https://issuetracker.unity3d.com/issues/override-controller-created-with-script-and-without-animator-rebind-call-crashes-unity)
*   Build Pipeline: Fix for visual error in scene build numbering. (925628, 932909)
*   Build Pipeline: Fixed case of potential domain reload being triggered in play mode. [(926516)](https://issuetracker.unity3d.com/issues/appdomain-is-unloaded-when-a-an-asset-referencing-another-bundles-assets-child-is-loaded-from-an-assetbundle)
*   Build Pipeline: Fixed `PostProcessScene` to handle multiple scenes when entering play mode. [(927707)](https://issuetracker.unity3d.com/issues/postprocessscene-is-only-called-for-the-first-loaded-scene)
*   Editor: Fixed crash that would be seen when adding component via "Add Component". [(923355, 932029)](https://issuetracker.unity3d.com/issues/crashes-on-usabilityanalytics-custom-sendevent-when-adding-light-component-under-any-game-object)
*   Editor: \[macOS\] Fixed issue where the close window shortcut (CMD+W) had stopped working on some editor windows. [(923357)](https://issuetracker.unity3d.com/issues/macos-close-window-shortcut-cmd-plus-w-stopped-working-on-some-editor-windows)
*   Multiplayer: Fixed regression where the networking assembly would not be ready when script update pointer was initialized. [(927427, 935273)](https://issuetracker.unity3d.com/issues/unable-to-launch-new-host-if-you-previously-exited-play-mode-while-host-was-active)
*   Particles: Fixed case of mesh particles becoming inverted. [(927395)](https://issuetracker.unity3d.com/issues/particles-mesh-particles-are-inverted-when-using-render-alignment-view)
...
*   Timeline: Fixed inconsistency between player and editor root motion positions. [(926701)](https://issuetracker.unity3d.com/issues/timeline-root-motion-in-timeline-looks-different-on-builds)
*   Universal Windows Platform: Enabled `BuildOptions.CompressWithLz4` support for the UWP platform. (920404)
*   Windows: Fixed problems when switching from windowed to full screen mode when using DX11. [(904471)](https://issuetracker.unity3d.com/issues/button-game-object-hitbox-moves-above-when-switching-to-fullscreen-in-exclusive-mode)
*   XR: \_MainTex\_ST values are now correct in non-VR mode. (905062)

2017.2.0b5 Release Notes (Full)
-------------------------------
