...
*   Android: When MuteOtherAudioSources option is disabled and there's no READ\_PHONE\_STATE permission, the Unity application running on Android 5.1 or older devices may not mute itself during an incoming phone call. This might cause rejections when submitting to Samsung app store.
...
*   Android: Audio from background applications now plays at the original volume when not muted.
*   Windows: Disabled Alt+Enter hotkey for entering fullscreen when Standalone player is embedded into other window.

### Improvements

*   Android: Improved WebRequest performance.
*   IL2CPP: Implemented support for calling System.Collections.Generic.IEnumerable methods on native objects that implement the Windows.Foundation.Collections.IIterable interface.

### Fixes

*   Android: Fixed proguard template to actually keep Unity classes. [(857978)](https://issuetracker.unity3d.com/issues/android-java-classes-are-missing-from-apk-when-building-using-gradle-with-custom-gradle-template)
*   Android: Opening on-screen keyboard no longer makes webcam update slow. [(827454)](https://issuetracker.unity3d.com/issues/android-inputtext-makes-webcam-update-texture-slow)
*   Android: \[Gradle\] Fixed build error on relative keystore paths on Windows. [(854834)](https://issuetracker.unity3d.com/issues/gradle-gradle-build-fails-because-of-wrong-keystore-path-in-the-generated-build-dot-gradle)
*   Android: \[Input\] Fixed joystick polling and startup. [(863399)](https://issuetracker.unity3d.com/issues/application-detects-gamepad-only-after-its-input-was-made)
*   Animation: Fixed selection sorting when shift-selecting in the curve editor. [(857435)](https://issuetracker.unity3d.com/issues/curveeditor-dragging-keyframes-after-shift-selection-gives-erroneous-results)
*   Editor: Fixed case of time not advancing in edit mode. [(864246)](https://issuetracker.unity3d.com/issues/editor-entering-play-mode-while-paused-and-then-exiting-play-mode-causes-the-time-manager-to-not-advance-further)
*   Editor: Fixed case of vertex snapping not working with some camera angles. [(861345)](https://issuetracker.unity3d.com/issues/cant-use-vertex-snapping-when-in-isometric-view)
*   Editor: Fixed crash when alt-tabbing with popup window visible. (859260)
*   IL2CPP: Corrected a build error in UnusedBytecodeStripper2 when a Unity API method or property has changed but has not been updated. [(863806)](https://issuetracker.unity3d.com/issues/ios-mono-dot-cecil-dot-resolutionexception-failed-to-resolve-unityengine-dot-rendertexture-when-building-for-ios)
*   iOS: 2k shadowmap textures are now allowed on devices with less than 2 GB of RAM, when best quality shadows are selected. [(871606)](https://issuetracker.unity3d.com/issues/5-dot-5-mobile-regression-mobile-shadow-resolution-capped-to-1024-in-unity-5-dot-5-vs-5-dot-4-4)
*   Linux: Input with characters that require shift key are now permitted.
*   Particles: Fixed corruption when using mesh particles that don't have both normals and tangents. [(863967)](https://issuetracker.unity3d.com/issues/shuriken-particle-system-glitch-across-all-screen-with-particular-mesh-type)
*   Particles: Fixed crash when using Texture Animation vertex streams, when Module was disabled. [(859380)](https://issuetracker.unity3d.com/issues/vertex-streams-crash-at-particlesystemgeometryjob-dot-cpp-337-buildcustomdata-when-using-uv-vertex-streams-with-1-xy-tile)
*   Particles: Fixed issue where nested sub-emitters could cause "Assertion failed on expression: 'particles.array\_size() == count' ". [(863298)](https://issuetracker.unity3d.com/issues/standardassets-particles-scene-fireworks-gives-lot-of-assertion-failed-on-expression-particles-dot-array-size-equals-equals-count)
*   Particles: Fixed Trail Renderer corruption where start/end vertices would occasionally connect. [(863946, 869219)](https://issuetracker.unity3d.com/issues/regression-from-5-dot-6-0b2-trail-renderer-is-broken-in-5-dot-6-0b3)
*   Particles: Fixed undo when using Gizmo handles in the Scene View for the Shape Module. (859828)
*   Physics: Fix to ensure that a PhysicsMaterial2D can be reassigned to a Collider2D if it's already assigned.
*   Physics: When turning on Rigidbody2D.simulated, the body position/rotation is now synchronized to the current Transform. [(868700)](https://issuetracker.unity3d.com/issues/stange-behaviour-with-parented-rigidbody2d-in-5-dot-5-0f3)
*   Playables: Changing PlayableOutput now triggers Topology change.
*   Playables: Fixed issue whre a leaking playable message would show until Unity is closed.
*   UI: Fixed case of mesh being miscoloured when created with CanvasRenderer.SetMesh, passing an array of Color objects to Mesh.colors. [(857087)](https://issuetracker.unity3d.com/issues/ui-canvasrenderer-canvasrenderer-dot-setmesh-results-in-miscolored-blue-mesh)
*   WebGL: Fixed an exception that would occur when rebuilding a WebGL project. [(867613)](https://issuetracker.unity3d.com/issues/il2cpp-dot-exe-failure-when-building-for-webgl-with-strip-engine-code-disabled)
*   Windows: Fixed player crash when embedding window with delayed option. (856729)

* * *

5.6.0b6 Release Notes (Full)
----------------------------
...
*   Editor: Mac: Experimental support for using the Metal rendering backend on the editor added. Disabled by default; enable via the Metal Editor Support checkbox in the Standalone player settings (Edit > Project Settings > Player > Other Settings).
*   Editor: Windows: All Visual Studio installs are now shown in the External Script Editor list.
*   Editor: Windows: Visual Studio 2017 installs with the Unity workload are now detected automatically and added to the External Script Editor list.
...
*   Graphics: Refactored camera render ordering code. When a Scene is rendered, it now figures out which cameras can share the same render target. The rules for this are:
    ...
    *   The cameras share the same viewport.
    *   All cameras have a depth buffer, or none of them have a depth buffer. These cameras are then split into a stack, which is rendered into a shared Texture based on depth ordering. This Texture has the most common settings found in the cameras in the stack. The following rules also apply:
...
*   Compute: ComputeShader inspector now lists kernels, and compiled code shows disassembly on DirectX 11. The UNITY\_VERSION macro is passed while compiling compute shaders, just like for regular shaders.
...
*   Shaders: Preprocessor macros SHADER\_STAGE\_VERTEX, SHADER\_STAGE\_FRAGMENT, SHADER\_STAGE\_DOMAIN, SHADER\_STAGE\_HULL, SHADER\_STAGE\_GEOMETRY, SHADER\_STAGE\_COMPUTE are now defined when compiling each shader stage.
