...
- Package Manager: Switching Project when importing complete project does not import all assets and project opens incomplete
...
- ShaderGraph: Shader Graph nodes has inconsistent play speed
    ([UUM-99396](https://issuetracker.unity3d.com/issues/shader-graph-nodes-has-inconsistent-play-speed))
...
- UI Toolkit: Added support for world-space mouse and touch input in UI Toolkit runtime when the world-space rendering feature is used.
...
- Bug Reporter: Added folder size to attachments table.

- Bug Reporter: Sign Bug Reporter executable on Windows.
...
- Editor: Improved Water samples with custom current and deformation maps using spline package.
...
- GI: Improved error message when the user tries to load Adaptive Probe Volumes from Addressables or AssetBundles without "Probe Volume Disable Streaming Assets" ticked.
...
- Graphics: Metal: dump some information/caps about metal device on startup.

- Kepler: VSync and MSAA Quality Setting applied.
...
- Shadergraph: Made the Shader Graph asset icon sharper on Retina/HiDPI displays.
    ([UUM-93099](https://issuetracker.unity3d.com/issues/shader-graph-asset-icon-is-blurry-in-the-4k-monitors))

- Shadergraph: Time node output should be smoother across graph actions.
...
- SRP Core: Improved Render Graph subpass merging workflow to reduce its CPU performance cost.

- SRP Core: Optimized the Render Graph workflow in ExecuteBeginRenderPass\(\).

- SRP Core: Rendering Debugger - Volumes - Warn users that the values of the table might be out of date if the selected camera is not rendered.
    ([UUM-91548](https://issuetracker.unity3d.com/issues/rendering-debugger-results-are-not-updated-unless-the-camera-has-rendered))<br>
    *First seen in 6000.2.0a1.*

- Universal RP: Removed useless Render Graph AllowPassCulling\(\) calls when AllowGlobalStateModification\(true\).

- URP: Cleaned up URP RenderGraph initialization step.

- URP: Removed unused internal ScriptableRenderPass variable breakGBufferAndDeferredRenderPass.
...
- URP: Deprecated: Deprecate SetupRenderPasses in ScriptableRenderFeature in URP.



#### Changes

- Editor: Editor: Added a better error message to prevent player building inside PlayerLoop.
    (UUM-90979)
...
- XR: Updated xr.sdk.openxr package version to 1.14.1.
...
- Editor: Fixed Rendering Debugger - Volumes - Proper refresh of the affected volumes and interpolation factor from the Volume Stack.
    (UUM-91645)<br>
    *First seen in 6000.2.0a1.*
...
- HDRP: Reduced the aliasing of punctual lights in volumetric fog.
    (UUM-92412)<br>
    *First seen in 6000.2.0a1.*
...
- PS4: Fixed an issue by Adjusting application shutdown to better handle test scenarios.
    (UUM-97416)<br>
    *First seen in 6000.2.0a1.*
...
- Tests: Fixed some graphics perf tests being disabled on simulator.
    (UUM-97735)<br>
...
- UI Toolkit: Fixed setting editor extension mode doesn't update preview.
    (UUM-97540)<br>
    *First seen in 6000.2.0a1.*
...
- VFX Graph: Updated internal image references.
    (UUM-98138)<br>
...
- com.unity.inputsystem: [1.11.2](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.11//changelog/CHANGELOG.html) to [1.13.1](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.13//changelog/CHANGELOG.html)
