...
*   Scripting: Crash on "CollectAndProduceGameObject' when opening Project or entering Play mode ([1147211](https://issuetracker.unity3d.com/issues/crash-on-collectandproducegameobject-when-opening-project-or-entering-play-mode))
...
*   Android: Support for supplementary Unicode characters for UnitySendMessage. (1059652, 1147132)  
    _This has already been backported to older releases._
    
*   Asset Import: Fixed an issue where the importer could crash if a Mesh Asset has no vertex data. ([1144169](https://issuetracker.unity3d.com/issues/unity-freeze-and-mono-arch-find-jit-info-ext-crash-when-importing-corrupted-mesh-asset-files), 1150807)
    
*   Asset Import: Fixed an issue with the Texture Import setting where the "Alpha is Transparency" option was enabling itself when switching to the Default Texture type. ([1117308](https://issuetracker.unity3d.com/issues/import-settings-alpha-transparency-enables-itself), 1150833)  
    _This has already been backported to older releases._
    
*   Editor: Fixed crash on quit in UNITY\_FT\_Done\_Face when using large prefab with TextMesh. (1075972, 1150370)
    
*   Editor: Fixed several Inspector window layouting issues. (1148706, 1155405)  
    _This has already been backported to older releases._
    
*   Editor: Sorting Groups were not properly displayed in the Inspector Window (1134112, 1155407)
    
*   GI: Fixed selection freeze in scene view when light probes are currently selected. ([1152479](https://issuetracker.unity3d.com/issues/impossible-to-select-anything-else-in-scene-view-if-currently-selected-object-has-light-probe-group-component), 1153088)
...
*   OSX: Fixed the editor crash while calling GetVSyncsPerSecond(). ([1148335](https://issuetracker.unity3d.com/issues/metal-calling-frametimingmanager-dot-getvsyncspersecond-crashes-the-editor), 1152184)  
    _This has already been backported to older releases._
    
*   SceneManager: Fixed Scene view skybox toggle not working in Prefab Mode if no Prefab editing environment scene is specified. (1153083, 1154308)  
    _This has already been backported to older releases._
    
*   Scripting Upgrade: Fix AssemblyUpdater.exe failing silently if Unity is installed to a path with spaces ([1154131](https://issuetracker.unity3d.com/issues/assemblyupdater-silently-fails-if-its-path-contains-spaces), 1154970)  
    _This has already been backported to older releases._
    
*   UI Elements: Fixed PropertyField layouting issue with vector data types ([1145908](https://issuetracker.unity3d.com/issues/when-dragging-or-mouseing-over-int2-property-fields-in-a-uielements-property-constantly-flickers), 1155410)
    

### Preview of Final 2019.2.0b5 Release Notes
...
*   Android: Fixed a crash when using Android JNI methods from custom thread.
...
*   Editor: fixes issue where scene visibility status was not properly ignored for objects with hide flags set to "hide in hierarchy." ([1111720](https://issuetracker.unity3d.com/issues/a-button-which-hides-all-gameobjects-in-the-scene-does-not-un-hide-them-when-clicked-again), [1156127](https://issuetracker.unity3d.com/issues/gameobjects-with-hideflags-dot-hideinhierarchy-prevent-scene-visibility-from-being-toggled-back-on), 1158024, 1158755)
