...
- DX12: Fixed a crash when capturing a frame in RenderDoc on a device with new "tight alignment" feature enabled. We will disable the feature for now when running with renderdoc.dll.
    (UUM-125528)<br>
    *First seen in 6000.3.0b7.*    <br>*Fixed in 6000.3.0b11.*

- Editor: Fixed an issue where a Raycast did not hit when using the default `BoxcastCommand()` constructor.
    ([UUM-123124](https://issuetracker.unity3d.com/issues/no-valid-hits-are-returned-when-using-raycastcommand))    <br>*Fixed in 6000.3.0f1.*
...
- Graphics: Fix editor freeze if Lens SRP asset references form a cyclic dependency
    ([UUM-122934](https://issuetracker.unity3d.com/issues/the-editor-freezes-when-creating-a-new-lens-flare-srp-and-adding-itself-to-the-lens-flare-data-srp-type-element))    <br>*Fixed in 6000.3.0b11.*
...
- Test Framework: Fixed a stack overflow exception in the Test Runner UI that might happen with some assembly setups with precompiled assemblies.
    ([UUM-122047](https://issuetracker.unity3d.com/issues/stackoverflowexception-occurs-and-tests-fail-to-load-when-the-test-runner-window-is-opened-while-system-dot-windows-dot-forms-dot-dll-and-system-dot-deployment-dot-dll-are-present-in-the-project))    <br>*Fixed in 6000.3.0b11.*

- UI Toolkit: Fixed UI Builder exceptions when name field is being edited during a save.
    ([UUM-125901](https://issuetracker.unity3d.com/issues/ui-builder-throws-errors-and-breaks-the-ui-document-when-renaming-elements-and-saving-without-validating))    <br>*Fixed in 6000.3.0b11.*
...
- Adaptive Performance: Fixed a *No default loader found* error that occurred when Adaptive Performance settings were added to unsupported platforms in Build Profiles.
    ([UUM-121991](https://issuetracker.unity3d.com/issues/no-default-loader-found-error-is-thrown-when-adaptive-performance-settings-are-added-to-the-facebook-instant-games-build-profile))<br>
...
- Editor: Fixed an issue in the VFX Graph where sticky notes with a size of 0 were incorrectly resized. The sticky notes now resize correctly.
    ([UUM-115165](https://issuetracker.unity3d.com/issues/vfx-graph-broken-fit-to-text-option-for-sticky-notes))<br>
...
- Editor: Fixed an issue where dragging main toolbar element to far left would not insert it in the correct place.
    ([UUM-114440](https://issuetracker.unity3d.com/issues/item-is-not-placed-at-the-edge-when-editing-the-main-toolbar-and-dragging-the-item-past-the-toolbars-left-side-limit))<br>
    *First seen in 6000.3.0a5.*
...
- Editor: Hide adaptive settings in build profile when the platform module is not installed.
    ([UUM-124745](https://issuetracker.unity3d.com/issues/adaptive-performance-settings-do-not-get-added-to-a-build-profile-and-no-errors-or-warnings-are-thrown-when-the-selected-platforms-module-is-not-installed))<br>
    *First seen in 6000.3.0b8.*
...
- Editor: Play Mode Scenarios icons are off-centered and not aligned with the Scenario's name/title.
    ([UUM-125142](https://issuetracker.unity3d.com/issues/play-mode-scenarios-icons-are-off-centered-and-not-aligned-with-the-scenarios-name-slash-title))<br>
    *First seen in 6000.3.0b9.*
...
- Graphics: Fixed an issue where the Color Picker's `Styles` static class could be initialized outside of the `OnGUI` context, causing style lookups to fail and breaking the Color Picker layout. The `Styles` class is now enforced to initialize only during `OnGUI`, and preset initialization is now handled within `OnGUI`.
    ([UUM-121992](https://issuetracker.unity3d.com/issues/hdr-color-and-color-windows-are-broken-and-unusable-across-editor-after-it-was-opened-via-vfx-graph))<br>
    *First seen in 6000.3.0b6.*
...
- Physics 2D: Added a missing 64-bit integer property to "PhysicsUserData" to allow set/get for custom data.
    ([UUM-125383](https://issuetracker.unity3d.com/issues/add-missing-64-bit-storage-to-physicsuserdata))<br>
    *First seen in 6000.3.0a5.*
...
- Physics 2D: Added the `PhysicsWorld.GetBodyUpdateUserData(allocator)` method to retrieve all `PhysicsUserData` associated with the current `BodyUpdateEvents`.
    ([UUM-125653](https://issuetracker.unity3d.com/issues/no-way-to-return-all-physicsuserdata-for-all-bodyupdateevents))<br>
    *First seen in 6000.3.0a5.*
...
- Physics 2D: Fixed an issue where the `PhysicsUserData.int64Value` property was not displayed in the Editor Inspector.
    ([UUM-125627](https://issuetracker.unity3d.com/issues/physicsuserdata-proeprty-drawer-does-not-display-the-int64value-property))<br>
...
- Shaders: Fixed compute shader compiler settings which caused confusing *Use of potentially uninitialized variable* compilation warnings on some platforms.
    ([UUM-112459](https://issuetracker.unity3d.com/issues/switching-platform-to-tvos-throws-shader-warnings-in-tracetransparentrays))<br>
    *First seen in 6000.3.0a2.*
...
- UI Toolkit: Fixed color field labels showing in UI Builder when using filter function definitions.
    ([UUM-125294](https://issuetracker.unity3d.com/issues/color-properties-names-in-filterfunctiondefinition-shows-up-as-value-n-in-ui-builder))<br>
    *First seen in 6000.3.0a2.*
...
- UI Toolkit: Removed the *Count* option from filter types.
    ([UUM-124742](https://issuetracker.unity3d.com/issues/count-option-exists-as-a-function-for-the-ui-toolkit-filter))<br>
    *First seen in 6000.3.0b8.*

- URP: Fixed an issue where the render target was not assigned when using the `OnRenderObject` callback.
    ([UUM-125129](https://issuetracker.unity3d.com/issues/constant-commandbuffer-temporary-render-texture-not-found-while-executing-setrendertarget-color-buffer-warnings-when-using-custom-editor-tools-with-scene-view-interaction))<br>
    *First seen in 6000.3.0b8.*
...
- Version Control: Fixed the icon that was blurry and pixelated when displayed in our Unity Version Control window's tab.
    ([UUM-122495](https://issuetracker.unity3d.com/issues/unity-version-control-icon-is-blurry-and-pixalated))<br>
    *First seen in 6000.3.0b7.*
...
- Version Control: Fixed the Project Settings window to automatically refresh when the plugin is enabled.
    ([UUM-122519](https://issuetracker.unity3d.com/issues/unity-version-control-window-lets-vcs-workspace-get-created-even-when-unity-vcs-is-disabled-in-project-settings-and-switches-to-vcs-being-enabled-after-reopening-the-project-settings-window))<br>
    *First seen in 6000.3.0b7.*
...
- Version Control: Fixed the Toolbar button to correctly hide when disabling the Unity Version Control package.
    ([UUM-122521](https://issuetracker.unity3d.com/issues/version-control-customizable-toolbar-is-fully-functioning-and-has-no-visual-changes-or-indication-when-vcs-is-disabled-in-project-settings-after-creating-a-workspace))<br>
    *First seen in 6000.3.0b7.*
...
- VFX Graph: Fixed an issue in URP with Render Graph where Sprite Custom Lit lighting did not work correctly.
    ([UUM-121678](https://issuetracker.unity3d.com/issues/particles-are-not-lit-by-2d-lights-when-custom-sprite-lit-shader-is-used))<br>
...
- VFX Graph: Fixed an issue where a sticky note could not be moved into another group after its original group was deleted.
    ([UUM-121917](https://issuetracker.unity3d.com/issues/sticky-note-cannot-be-assigned-to-a-group-after-its-assigned-group-has-been-deleted))<br>
...
- VFX Graph: Fixed an issue where empty groups were removed when grouping a node selection. Empty groups are now preserved.
    ([UUM-113869](https://issuetracker.unity3d.com/issues/empty-group-node-in-the-vfx-graph-is-deleted-when-group-selection-is-pressed-on-a-sticky-note))<br>
...
- VFX Graph: Fixed an issue where inserting a sticky note by right-clicking over a group did not place the sticky note inside the group, unlike with operators.
    ([UUM-114194](https://issuetracker.unity3d.com/issues/vfx-sticky-notes-are-not-created-inside-the-group-node-unless-the-group-node-is-selected-during-creation))<br>
    *First seen in 6000.3.0a4.*
...
- VFX Graph: Fixed renaming a property of a template asset in the blackboard was not working as expected after the template was inserted in another VFX Graph asset.
    ([UUM-114919](https://issuetracker.unity3d.com/issues/properties-are-not-renamed-in-the-blackboard-if-the-graph-is-being-used-as-a-template))<br>
...
- VFX Graph: Prevented a warning about using `pow` with a potential negative value in the Position Sphere block.
    ([UUM-116017](https://issuetracker.unity3d.com/issues/vfx-graph-warning-set-position-shape-block-using-the-thickness-mode))<br>
    *First seen in 6000.3.0b1.*
...
- com.unity.meta-instant-games-sdk: [2.0.0](https://docs.unity3d.com/Packages/com.unity.meta-instant-games-sdk@2.0//changelog/CHANGELOG.html) to [1.3.0](https://docs.unity3d.com/Packages/com.unity.meta-instant-games-sdk@1.3//changelog/CHANGELOG.html)
...
### Preview of Final 6000.3.0b10 Release Notes
...
#### Preview of Final 6000.3.0b10 Package changes