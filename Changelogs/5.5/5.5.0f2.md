...
*   Editor: Mem Record: Native memory performance profiling for CPU Profiler Timeline. Shows callstacks of native memory allocations. ![description](/sites/default/files/5_5_feature_native_mem_profiling.png)
*   Editor: New High Detail view in the CPU Profiler Timeline. Shows all profiler samples without filtering. ![description](/sites/default/files/5_5_feature_profiler_timeline_high_detail.png)
...
*   Graphics: Unity splash screen tools. ![description](/sites/default/files/5_5_feature_splashscreentools1.gif) ![description](/sites/default/files/5_5_feature_splashscreentools2.gif)
*   Particles: New Modules:
    ...
    *   Noise Module, for adding Curl Noise to Particle Movement ![description](/sites/default/files/5_5_feature_particlenoise.gif)
    *   Trails Module, for rendering ribbonized trails behind particles ![description](/sites/default/files/5_5_feature_particletrails.gif)
...
*   VR: Holographic Emulation ![description](/sites/default/files/5_5_feature_holographicemulation_0.gif)
...
*   Graphics: Added support for BC4/BC5 texture formats on PC and consoles. The new formats are available in Texture Import format override options (they are not used by default), or when creating Textures from scripts. ![description](/sites/default/files/5_5_improvement_bc5textureformat.png)
...
*   Particles: Added "Show Bounds" to Particle System UI. This shows the world and local AABB for the system. ![description](/sites/default/files/5_5_improvement_particlebounds.png)
*   Particles: Added a new Render mode to make particle billboards face the eye position. This is good for VR rendering. ![description](/sites/default/files/5_5_improvement_particlefacingeye.png)
*   Particles: Added a new Shape Module option to rotate particles to face their initial direction of travel. ![description](/sites/default/files/5_5_improvement_particlealigntodirection.png)
*   Particles: Added support for sending custom data to Particle System vertex Shaders. ![description](/sites/default/files/5_5_improvement_particlevertexstreams.gif)
*   Particles: Gradients now support a fixed color mode, where no blending is performed. This is useful when users want to define a simple list of possible colors to be chosen from. ![description](/sites/default/files/5_5_improvement_particlefixedgradient.png)
...
*   UI: Fixed threading problem when computing bounds which could lead to an infinite loop in the UI system. [(846096)](https://issuetracker.unity3d.com/issues/unity-goes-into-infinite-memory-consuming-loop-with-too-heavy-ui)
...
*   Windows Store: Microsoft .NET compiler will always output errors in english ignoring OS region settings, the same way as Mono compiler. [(755238)](https://issuetracker.unity3d.com/issues/editors-console-unable-to-display-unicode-characters)

5.5.0f2 Release Notes (Delta since f1)
--------------------------------------
...
*   iOS: Fixed Application.Unload() crash when touching or rotating screen

### Fixes

*   Asset Pipeline: Fixed regression where cache lz4 compression was not being performed for uncompressed AssetBundles (845859)
*   Audio: Fix issue which prevented streaming audio loading on 32-bit Apple devices which were upgraded to iOS 10 [(832578)](https://issuetracker.unity3d.com/issues/ios-audio-with-streaming-load-type-does-not-play-on-some-devices-running-ios-10)
*   iOS: Fixed an issue where the splash screen was able to register touch events [(828824)](https://issuetracker.unity3d.com/issues/unity-is-registering-inputs-while-loading-and-applies-them-on-a-first-frame-of-the-application)
*   Linux: Fix shutdown crash (847976)
*   Shaders: Fixed a specific compute shader warning being treated as error. Found when using TerrainComposer2 asset store package.
*   Windows Store: Fixed player build with C# projects enabled when dependency exists between projects [(847985)](https://issuetracker.unity3d.com/issues/visual-studio-fails-to-resolve-assembly-with-photon-voice-when-editor-builds-wsa-universal-10-solution-with-c-number-project-setting)
*   Windows Store: Fixed stack corruption in UnityEngine.UI (847534)