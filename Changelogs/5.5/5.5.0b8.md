...
*   HoloLens: Renamed "Holographic Simulation and Remoting" to "Holographic Emulation". This change is reflected in the Windows menu.

### Improvements

*   VR: Updated the Oculus plugin to version 1.9.

### Fixes

*   2D: Fixed an issue with UV rounding errors when using the OverrideGeometry method on sprites. [(823935)](https://issuetracker.unity3d.com/issues/some-uv-values-get-shifted-by-1-pixel-and-results-in-incorrect-uvs)
*   Animation: Fixed a crash in the Editor when loading a scene that contains an invalid Avatar . [(829868)](https://issuetracker.unity3d.com/issues/editor-crash-when-loading-scene-with-an-animator-with-an-invalid-avatar-reference)
*   Animation: Fixed an issue where changing Animation events in the Animation Importer would not activate the Apply button. [(824180)](https://issuetracker.unity3d.com/issues/anim-importer-animation-events-do-not-activate-apply-on-new-fbx-files-first-time-only)
*   Animation: Fixed an issue where negative speeds in states or blend trees caused root motion character to warp to a new position. [(830951)](https://issuetracker.unity3d.com/issues/blendtree-negative-time-value-causes-root-motion-character-to-warp-into-a-new-position)
*   Animation: Fixed case of missing visual control in the animation window vertical splitter. [(830501)](https://issuetracker.unity3d.com/issues/aw-cant-see-rotation-interpolation-mode-in-read-only-clip)
*   Animation: Fixed crash that would be triggered when a null property modification was attempted during animation recording. [(832837)](https://issuetracker.unity3d.com/issues/ui-editor-crashes-when-changing-rotation-property-on-a-recttransform-in-animation-record-mode)
*   Animation: Fixed crash when playing the same Playable multiple times. (821431)
*   Animation: Fixed vertical slider glitch in the dopesheet editor. [(829408)](https://issuetracker.unity3d.com/issues/small-ui-glitch-in-animation-window-dope-sheet-mode)
*   Animation: The property contextual menu is now displayed for read-only clips, in order to allow interpolation mode to be displayed. (830501)
*   Editor: Canceling a build while NuGet packages are being restored no longer causes a crash. (826083)
*   Editor: Fixed case of collider editing handles not appearing in projects when services are enabled. [(829182)](https://issuetracker.unity3d.com/issues/edit-handle-of-polygon-collider-2d-does-not-appear)
*   Editor: Fixed case of icon tab in player settings closing immediately when opened. [(831307)](https://issuetracker.unity3d.com/issues/icon-tab-in-playersettings-doesnt-stay-open)
*   Editor: Fixed menu validation crash, triggered when pressing the Assets menu item while Scene in Hierarchy is selected. [(833463)](https://issuetracker.unity3d.com/issues/crash-on-assetsmenu-candeleteselectedassets-when-pressing-assets-menu-item-while-scene-in-hierarchy-is-selected)
*   Graphics: Fix to make dynamic batching on DX11 work consistently with other platforms, by only outputting color stream to shaders when it is used. [(811875)](https://issuetracker.unity3d.com/issues/dx11-dynamic-batching-on-dx11-isnt-consistent-to-other-graphics-apis)
*   Graphics: Fixed a memory leak that was occurring when using any of the GI visualizers in the scene view. (835378)
*   Graphics: Fixed case of projectors rendering twice if transparent objects are in the scene. [(814282)](https://issuetracker.unity3d.com/issues/projector-gets-drawn-two-times-when-transparent-object-is-in-the-scene)
*   HoloLens: Fixed a crash in SurfaceObserver that could occur after restarting Holographic Emulation.
*   iOS: The splash screen is now not shown as expected if it is disabled.
*   Kernel: Fixed issue where the local scale of a duplicate game object did not match the local scale of the original game object if the parent had rotation set. [(815816)](https://issuetracker.unity3d.com/issues/duplicating-a-gameobject-twice-causes-a-change-in-the-duplicated-transforms-scale-value)
*   Particles: Fixed occasional crash when using lens flares. [(827746)](https://issuetracker.unity3d.com/issues/editor-crashes-with-flaremanagergeometryjob-operator-equals)
*   SamsungTV: Fixed case of splash screen not rendering.
*   Scripting: Fixed case of splash screen isFinished not reporting correctly.
*   Scripting: Fixed crash on Invoke(null, ...). [(766144)](https://issuetracker.unity3d.com/issues/ui-crash-on-scripting-class-get-method-from-name-filter-after-click-on-ui-button)
*   Window Management: Fixed issue whereby the Main editor window couldn't be resized properly after being resized to a minimal height. [(797810)](https://issuetracker.unity3d.com/issues/main-editor-window-cant-be-resized-under-an-unusual-height)
*   Windows: The initial scene is now loaded while the splash screen is displaying.

* * *

5.5.0b8 Release Notes (Full)
----------------------------
