...
*   AI: Fix for regression where a pushed passive NavMeshAgent would sometimes move to its origin after navmesh carving. [(830443)](https://issuetracker.unity3d.com/issues/active-obstacle-causes-navmesh-agent-spawn-point-to-be-set-as-destination)
*   Editor: Fixed case of services tab losing focus when a service refresh occurs. (704234)
*   Editor: Fixed issue where a KeyNotFoundException would be thrown when dragging prefabs to the scene view after deleting a pre-existing prefab. [(826474)](https://issuetracker.unity3d.com/issues/keynotfoundexception-on-adding-prefab-to-the-scene)
*   Editor: Fixed issue where yield instruction WaitForEndOfFrame did not work in ExecuteInEditMode. [(804814)](https://issuetracker.unity3d.com/issues/yield-instruction-waitforendofframe-doesnt-work-in-executeineditmode)
*   Graphics: Fixed a bug where "Generate NormalMap from grayscale" would stay enabled when changing texture type from NormalMap. [(830812)](https://issuetracker.unity3d.com/issues/textureimporterinspector-normalmap-setting-carries-over-to-default-texture-type)
*   Graphics: Fixed a bug where the "Mapping" popup for a cubemap would be empty (due to it containing obsolete values). [(830805)](https://issuetracker.unity3d.com/issues/textureimporterinspector-mapping-popup-text-is-empty)
*   Graphics: Fixed Advanced foldout so that its behavior is consistent between different importers. [(830806)](https://issuetracker.unity3d.com/issues/textureimporterinspector-advanced-foldout-collapses-always)
*   Graphics: Fixed an issue whereby the "Alpha Usage" parameter would not display "mixed values" as expected, because of an incompatibility between the default value and the texture in question not having alpha channel. [(830803)](https://issuetracker.unity3d.com/issues/textureimporterinspector-shows-alpha-usage-equals-input-texture-alpha-when-assets-that-have-differrent-alpha-usage-modes-selected)
*   VR: Fixed Oculus HMD restart functionality.
*   VR: Removed warning message when running in VR with Audio disabled. [(826132)](https://issuetracker.unity3d.com/issues/vr-if-unity-audio-is-disabled-error-vr-error-setting-active-audio-output-driver-falling-back-to-default-is-shown)
...
*   VR: Holographic Simulation and Remoting
...
*   Graphics: Metal: Shader backend switch to HLSLcc
...
*   Profiler: Fixed self-profiling of Profiler.BeginSample calls in deep-profiling mode [(776595)](https://issuetracker.unity3d.com/issues/profiler-shows-unnecessary-entries-when-deep-profiling)
