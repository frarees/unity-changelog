...
*   Scripting: Mouse callbacks are now called on mobile platforms when and as expected. [(833157)](https://issuetracker.unity3d.com/issues/wsa-slash-uwp-mousecallbacks-dont-work-in-5-dot-5)
...
*   Windows Store: Fix to ensure that the build will no longer fail if there are read-only plugin files in the project. [(832604)](https://issuetracker.unity3d.com/issues/uwp-wsa-build-fails-if-project-contains-some-read-only-files-star-dot-dll-star-dot-pdb-star-dot-winmd-dot)

* * *

5.5.0b7 Release Notes (Full)
----------------------------
...
*   Web: The Accept-Encoding header may now be set via scripting, but note that doing so is at the user's risk.
