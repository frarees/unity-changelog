...
*   HD RP: \[HDRP\] Editor crashes on RaiseException when building project using Build and Run for the second time ([1417508](https://issuetracker.unity3d.com/issues/hdrp-editor-crashes-when-building-project-using-build-and-run-for-the-second-time))
...
### New 2022.1.0b16 Entries since 2022.1.0b15
...
*   Editor: Fixed bug where the OSX splash art would show 2021 copyright when it should be 2022. ([1413716](https://issuetracker.unity3d.com/issues/splash-screen-still-shows-2005-2021-unity-technologies-while-creating-slash-opening-project-on-unity-2022-dot-1))  
...
*   Scripting: Fixed crash in AssemblyUpdater when Unity path contains space. ([1393230](https://issuetracker.unity3d.com/issues/analytics-library-package-throws-errors-when-there-are-spaces-in-the-unity-editor-path))  
...
*   UI Toolkit: Fixed asset reimportation/refresh cause exception when displayed on the game view. ([1402588](https://issuetracker.unity3d.com/issues/asset-reimportation-slash-refresh-cause-exception-when-displayed-on-the-game-view))
...
*   URP: Fixed a bug in where it was confusing for our users in using different renderer types and camera stacking. ([1376601](https://issuetracker.unity3d.com/issues/urp-attempting-to-get-camera-relative-temporary-rendertexture-is-thrown-when-stacking-camera-with-different-renderers))
    
*   URP: Fixed Blit and MotionVectors shader compilation errors with GLES2 API. (1388668)
    
*   Web: This fix sets the mouse positions in the MouseMove handler in the new input system based on the mouse event. MouseUp and MouseMove events are relative to the window object, whereas MouseDown and Wheel events are relative to the canvas object. ([1385365](https://issuetracker.unity3d.com/issues/mouse-position-is-offset-when-mouse-scrolling-in-webgl-build))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    

#### New 2022.1.0b16 Package Changes since 2022.1.0b15
...
### Preview of Final 2022.1.0b16 Release Notes
...
*   Virtual Texturing: `InvalidateRegion` calls now generate new tile requests in their "importance" order. The importance is defined as a combination of the tile's screen space size and the number of frames since the tile has been requested. This makes completing tile requests contribute more to the rendering result when `InvalidateRegion` is called continuously.
...
*   iOS: Removed: Removed the obsolete iOS Notification Services API. Please consider using the "Mobile Notifications" package (available in the Unity package manager) which implements the iOS UserNotifications framework.
...
*   Android: Fixed crash caused by out of memory on Android when using Vulkan. ([1394782](https://issuetracker.unity3d.com/issues/android-crash-with-signal-11-caused-by-null-pointer-dereference-on-android-device-when-using-vulkan-graphics-api))
...
*   Editor: On macOS, undocked windows can no longer enter fullscreen and do not open fullscreened when the main Editor window is in fullscreen mode.  
    On macOS, removes error message in console when moving the main editor window in and out of fullscreen mode.  
...
*   Graphics: Fixed crash in Vulkan backend when running out of GPU memory while building asset bundles on AMD GPUs. ([1385669](https://issuetracker.unity3d.com/issues/linux-vulkan-editor-crashes-due-to-memory-leak-when-building-assetbundles))
