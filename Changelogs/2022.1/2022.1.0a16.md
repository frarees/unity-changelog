...
*   Customer QA Onboarding: ArgumentException from hierarchyWindowItemOnGUI is thrown when leaving Prefab Mode ([1382181](https://issuetracker.unity3d.com/issues/argumentexception-from-hierarvhywindowitemongui-is-thrown-when-leaving-prefab-mode))
    
*   Customer QA Onboarding: Crash when calling hideFlags after removing missing nested prefab ([1381563](https://issuetracker.unity3d.com/issues/crash-when-calling-hideflags-after-removing-missing-nested-prefab))
    
*   Customer QA Onboarding: TransientSceneViewOverlays are causing RepaintAll and GPU/CPU usage increases to 20-30% when hover over Camera preview window ([1381613](https://issuetracker.unity3d.com/issues/transientsceneviewoverlays-are-causing-repaintall-and-gpu-slash-cpu-usage-increases-to-20-30-percent-when-hover-over-camera-preview-window))
    
*   DX12: Setting RandomWriteTarget and drawing without needing it will no longer prevent subsequent draws from having it bound if they need it. ([1375746](https://issuetracker.unity3d.com/issues/directx12-objects-are-not-lighted-in-game-view-when-using-segi-system-with-graphics-api-for-windows-set-to-direct3d12))  
    _Fixed in 2022.1.0b1._
...
*   Package Manager: Fixed Asset Store package dependencies import. ([1377896](https://issuetracker.unity3d.com/issues/importing-a-project-from-the-asset-store-doesnt-import-packages-even-if-you-chose-yes-at-the-dialogue))  
...
*   Serialization: Fix an issue with fields on SerializeReference instances within Prefab losing their override when entering play mode. ([1383439](https://issuetracker.unity3d.com/issues/serialisation-going-into-play-mode-does-not-deserialize-the-object-properly))  
    _Fixed in 2022.1.0b1._
...
### New 2022.1.0a16 Entries since 2022.1.0a15
...
*   Graphics: Added: DebugFrameTiming class that can be used by render pipelines to display CPU/GPU frame timings and bottlenecks in Rendering Debugger.
    
*   Graphics: Added: New DebugUI widget types: ProgressBarValue and ValueTuple.
    
*   Graphics: Added: Support for high performant unsafe (uint only) Radix, Merge and Insertion sort algorithms on CoreUnsafeUtils.
...
*   HDRP: Added: API to edit diffusion profiles and set IES on lights.
    
*   HDRP: Added: New configuration ShderOptions.FPTLMaxLightCount in ShaderConfig.cs for maximum light count per fine pruned tile.
    
*   HDRP: Added: New functions that sample the custom buffer in custom passes (CustomPassSampleCustomColor and CustomPassLoadCustomColor) to handle the RTHandleScale automatically.
    
*   HDRP: Added: New panels to Rendering Debugger Display Stats panel, displaying improved CPU/GPU frame timings and bottlenecks.
    
*   HDRP: Added: Option to use full ACES tonemap instead of the approximation.
    
*   HDRP: Added: Public API to edit materials from script at runtime.
    
*   HDRP: Added: Public API to reset path tracing accumulation, and check its status.
    
*   HDRP: Added: Support for HDR output devices.
    
*   HDRP: Added: Support for orthographic camera in path tracing.
    
*   HDRP: Added: Support for SensorSDK's Lidar and camera models in path tracing.
    
*   HDRP: Added: Support for "Always Draw Dynamic" option for directional light cached shadows.
    
*   Physics: Added: Inverse Dynamics force getter methods for Articulation Bodies.
    
*   Shadergraph: Added: Ability to automatically cast Bools to Vector types in ShaderGraph. (1359160)
    
*   Shadergraph: Added: Automatically generated material subasset on ShaderGraphs.
    
*   Shadergraph: Added: Normal transforms to the Transform node.
    
*   Shadergraph: Added: ShaderGraph import warning to old nodes and properties, and ability to dismiss the warning if old behavior is desired.
...
*   URP: Added: Depth Priming support for Vulkan with MSAA.
    
*   URP: Added: Depth Texture setting for Overlay Camera.
    
*   URP: Added: Display Stats panel to Rendering Debugger that displays CPU/GPU frame timings and bottlenecks.
    
*   URP: Added: Shadows and Additional Lights off variants stripping.
...
*   GI: Avoid rare infinite loop when baking reflection probes while shaders are asynchronously compiling. (1376248)
...
*   GI: Fixed performance regression in CPU Lightmapper. ([1369204](https://issuetracker.unity3d.com/issues/cpu-plm-major-performance-regression-introduced-in-2022-dot-1-0a5))  
    _First seen in 2022.1.0a5._
    
*   GI: Light probe produces valid output when punctual light overlaps it. ([1242829](https://issuetracker.unity3d.com/issues/point-lights-have-no-guard-when-light-probes-are-near-or-overlapping-them-in-progressive-lightmapper))
...
*   Graphics: Fixed issue where unused constant buffer members may cause (SRP Batcher) batch breaks. ([1261542](https://issuetracker.unity3d.com/issues/multiple-render-main-shadowmap-dot-dot-dot-draw-srp-batch-are-present-when-the-materials-only-have-different-properties))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   HDRP: Fixed decal position when created from context menu. ([1368987](https://issuetracker.unity3d.com/issues/hdrp-when-decals-are-placed-as-child-objects-they-dont-spawn-at-00-0-coordinates))  
    _First seen in 2022.1.0b8._
...
*   HDRP: Fixed pyramid blur being corrupted when hardware DRS was on (case 1372245). (1372245)  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Prefabs: Removed unpack Prefab instances option when deleting Prefabs  
    This makes case 1360671 obsolete. ([1360671](https://issuetracker.unity3d.com/issues/delete-and-unpack-prefab-asset-does-not-unpack-all-the-prefab-instances-in-the-scene))  
...
*   Profiler: Fixed crash when exiting Unity Editor related to UnsafeUtility.Malloc usage. (1377694)  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   Search: Enabled Show Package Results for the find provider (find:). ([1377480](https://issuetracker.unity3d.com/issues/searching-with-wantmoreresult-and-show-package-results-doesnt-actually-search-in-packages))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Shaders: Added missing ability to use SM 6.6. integer atomics with DXC. ([1365040](https://issuetracker.unity3d.com/issues/dxc-slash-dx12-no-way-to-enable-sm-6-dot-6-features-results-in-compiler-error-when-eg-using-64-bit-atomics-in-a-shader))  
    _First seen in 2022.1.0a7._
...
*   Shaders: Fixed LocalKeyword.type, LocalKeyword.isOverridable and LocalKeyword.isValid crashing Unity when called on an unitialized LocalKeyword. ([1376999](https://issuetracker.unity3d.com/issues/unity-crashes-when-calling-isvalid-or-isoverridable-or-type-on-an-uninitialized-localkeyword-variable))
    
*   Stadia: Splash screen is now completely displayed on Stadia till it completes. (1256069)
...
*   UI Toolkit: Fixed the canvas document styling getting broken after Save. ([1378420](https://issuetracker.unity3d.com/issues/canvas-gets-corrupted-after-save))
...
*   Undo System: Editor no longer crashes when an undo operations changes type on an object. ([1372752](https://issuetracker.unity3d.com/issues/fatal-error-appears-while-undoing-replace-transform-operation))  
...
*   VFX Graph: Extract position from a transform is wrong on GPU. ([1353533](https://issuetracker.unity3d.com/issues/vfx-extract-position-from-transform-on-gpu-returns-zero))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Video: Crash on MediaEncoder\_CUSTOM\_Internal\_AddFrame\_Injected when pushing a frame into MediaEncoder that has been disposed. ([1368301](https://issuetracker.unity3d.com/issues/crash-on-mediaencoder-custom-internal-addframe-injected-when-pushing-a-frame-into-mediaencoder-that-has-been-disposed))
...
### Preview of Final 2022.1.0a16 Release Notes
