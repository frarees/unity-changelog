...
*   Asset Import Pipeline: Assemblies not being reloaded when reimporting C# script asset ([1293888](https://issuetracker.unity3d.com/issues/assemblies-not-being-reloaded-when-reimporting-c-number-script-asset))
...
*   MacOS: Non-development Apple silicon builds don't run ([1293633](https://issuetracker.unity3d.com/issues/non-development-apple-silicon-builds-dont-run))
...
*   Package Manager: Fix the issue where opening Package Manager window throws an null exception error. (1293933)  
...
*   Shaders: Fix rendering on DX when using "Use full Sampler precision by default" option in the player settings. ([1293568](https://issuetracker.unity3d.com/issues/shader-editor-gets-blank-on-selecting-use-full-sampler-precision-by-default-option-in-shader-precision-model))  
    _Fixed in 2021.1.0a8._
    
*   Shaders: Fixed an issue with Compile and show code button when "Custom:" API is selected ([1294078](https://issuetracker.unity3d.com/issues/compile-and-show-code-button-doesnt-compile-for-selected-custom-platform))  
...
*   Vulkan: Editor crashes on vk::ImageManager::RemoveTextureIDBinding on changing Graphics APIs to OpenGLCore ([1293565](https://issuetracker.unity3d.com/issues/vulkan-editor-crashes-on-vk-imagemanager-removetextureidbinding-on-changing-graphics-apis-to-openglcore))
...
### New 2021.1.0a7 Entries since 2021.1.0a6
...
*   2D: Fixed assertion failed exception is thrown on undoing after clicking on Create Range button. ([1273705](https://issuetracker.unity3d.com/issues/2d-sprite-shape-assertion-failed-exception-is-thrown-on-undoing-after-clicking-on-create-range-button))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   2D: Fixed icon is missing on creating SpriteShapeProfile at the time of creating. ([1271817](https://issuetracker.unity3d.com/issues/2d-icon-is-missing-on-creating-spriteshapeprofile))
...
*   2D: Fixed SpriteShape Bounding Box does not take into account certain vertices. ([1274400](https://issuetracker.unity3d.com/issues/2d-spriteshape-spriteshape-bounding-box-does-not-take-into-account-certain-vertices))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   2D: Fixed vertical slider handle is not aligned and placed slightly to the right side in the Bone Influence window. ([1260568](https://issuetracker.unity3d.com/issues/2d-animation-vertical-slider-handle-is-not-aligned-and-placed-slightly-to-the-right-side-in-the-bone-influence-window))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Editor: Fixed error in player settings caused by switching platforms. ([1284565](https://issuetracker.unity3d.com/issues/opening-player-setting-throws-exceptions-after-defining-a-scripting-symbol-and-switching-a-platform))
    
*   Editor: Fixed Indexing is not complete if you move a folder while previous indexing is not finished. (1288447)  
    _This is a change to a 2021.1.0a4 change, not seen in any released version, and will not be mentioned in final notes._
...
*   GraphView: Fixed unresponsive GraphView after dropping a node in a stack. ([1275569](https://issuetracker.unity3d.com/issues/graphview-graph-view-becomes-unresponsive-when-adding-a-node-into-the-stack-node))
...
*   SceneManager: Made sure object are cleaned up properly if they are children of GameObject with DontSave flag. ([1157422](https://issuetracker.unity3d.com/issues/gameobjects-with-hideflags-dot-dontsave-raise-an-error-when-reloading-the-scene))
...
*   SceneManager: When starting Unity we now verify that last opened scenes still exist on disk before adding them to the SceneManager. (1286322)  
    _This is a change to a 2021.1.0a2 change, not seen in any released version, and will not be mentioned in final notes._
...
*   Serialization: Fixed crash when opening the Preset window on an object containing SerializeReference with instances accessible from multiple property path. ([1290753](https://issuetracker.unity3d.com/issues/editor-freezes-when-opening-preset-window-of-script-with-serializereference))
...
### Preview of Final 2021.1.0a7 Release Notes
