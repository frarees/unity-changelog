...
- Editor: Fixed an issue where Allocator.Temp memory would internally deallocate memory aggressively resulting in temp memory performance regressions.<br>
    ...
    ([UUM-45113](https://issuetracker.unity3d.com/issues/editor-crashes-when-vr-template-is-loading))    <br>*Fixed in 2023.2.0b7.*

- Editor: Fixed an issue where Unity Editor minimizes for a moment and returned to its initial state when clicked on it in the Windows taskbar.
    ...
    *First seen in 2023.2.0b2.*    <br>*Fixed in 2023.2.0b7.*

- Editor: Fixed intermittent crashes in the menu controller that can occur during domain reloads.
    ([UUM-34847](https://issuetracker.unity3d.com/issues/linux-crash-on-disconnectgtksignal-void-star-gtkcallbackresponse-star-void-star-when-going-through-microgame-tutorials))    <br>*Fixed in 2023.2.0b7.*

- Editor: Fixed intermittent crashes in the menu controller that can occur during domain reloads.
    (UUM-35943)<br>
    *First seen in 2023.2.0a16.*    <br>*Fixed in 2023.2.0b7.*
...
- GI: Fix for memory leak when cancelling a bake
    ([UUM-41988](https://issuetracker.unity3d.com/issues/light-probe-gpu-memory-is-not-deallocated-when-cancelling-a-bake))    <br>*Fixed in 2023.2.0b7.*
...
- IL2CPP: Fix UnsafeUtililty.IsUnmanaged/UnsafeUtililty.IsBlittable returning incorrect results
    (UUM-43422)    <br>*Fixed in 2023.2.0b7.*
...
- PhysX Integration: Sometimes editor crashes after physics assertions
...
- Serialization: Fixed editor crash when creating asset bundle having fully supported inflated types.
    ([UUM-37034](https://issuetracker.unity3d.com/issues/crash-on-typetreequeries-fullyqualifiedtypenameparts-trygetfromreflectiontypestring-when-building-bundles))    <br>*Fixed in 2023.2.0b7.*

- Shadows/Lights: Crash on GfxDevice::ExecuteAsync when light source shadows are enabled with BuiltIn and SRP materials in the Scene with the default Scriptable Render Pipeline
...
- Terrain: Player crashes when Auto connect is enabled for Terrains in the Terrain group
...
- Universal RP: Fixed black screen issue when using URP hardware Dynamic Resolution with DX12
    ([UUM-29260](https://issuetracker.unity3d.com/issues/urp-after-enabling-urp-dynamic-resolution-the-game-view-is-not-rendered))    <br>*Fixed in 2023.2.0b7.*
...
### New 2023.2.0b6 Entries since 2023.2.0b5
...
- Editor: Fixed creating/cloning new VolumeProfiles through Global Settings UI.
    (UUM-45909)<br>
...
- IL2CPP: Fixed an issue wehre the offset of properties within Explicit layout structs with FieldOffset were not correct.
...
- Linux: Fixed an issue where paused unity player from using excessive CPU when "run in the background" option is off.
...
- Mono: Fixee deadlock on taking a memory profiler snapshot.
    (UUM-40688)

- Package Manager: Fixed an issue where multi selection buttons were not viewable due to the horizontal scrollbars.
    ([UUM-45948](https://issuetracker.unity3d.com/issues/package-bulk-action-button-hidden-when-plenty-of-space-to-display))<br>
    *First seen in 2023.2.0b3.*

- Particles: Fixed a potential crash on deletion of particle systems with mesh particles.
...
- Search: Fixed an issue where Search's "Press Tab to filter" was missing an icon.
    ([UUM-44351](https://issuetracker.unity3d.com/issues/search-missing-icon-in-press-tab-to-filter))

- Search: Fixed an issue where Search's placeholder texts was overlapping and escaping the search field.
...
- UI Toolkit: Fixed an issue where USS files were reordering in UI Builder.
    ([UUM-37728](https://issuetracker.unity3d.com/issues/changes-are-not-saved-when-changing-styles-orders-in-the-ui-builder))

- UI Toolkit: Fixed misalignments that were present in the composite controls.
    (UUM-25403)

- URP: Fixed an issue where the Overlay UI disappeared when using the Render Graph and HDR display support in the editor.
    (UUM-45545)<br>
    *First seen in 2023.2.0b3.*
...
- Video: Fixed an issue where video textures such as WebCamTextures may have thrown GraphicsTexture-related errors upon creation and possibly crashed on scene changes or other attempts to delete.
...
- WebGL: Fixed an issue where the application's background image wouldn't appear while loading if compression was enabled.
...
- com.unity.xr.core-utils: [2.2.2](https://docs.unity3d.com/Packages/com.unity.xr.core-utils@2.2//changelog/CHANGELOG.html) &#x2192; [2.2.3](https://docs.unity3d.com/Packages/com.unity.xr.core-utils@2.2//changelog/CHANGELOG.html)

### Preview of Final 2023.2.0b6 Release Notes
...
- Android: Fixed a bug that would cause a `JNI:GetInternetReachability` exception while using APIs like `Application.internetReachability` when running with GameActivity.
...
- Android: Fixes an issue where the Android build system asked for an API level that didn't exist.
...
- Editor: Fixed an Editor crash that would occur because of a shader chunk size of 0 in player settings.
...
- Editor: Fixed an issue to prevent C4D native importer warnings if a project has its own C4D ScriptedImporter.
...
- Editor: Fixed an issue where the control shortcut modifier \(^\) wasn't displayed when introduced inside a custom menu Window for MacOS &amp; Linux platforms.
...
- Editor: Fixed an issue with the Check for Updates window displayed open in the **Window &gt; Panels** dropdown menu.
...
- Editor: Fixed issue with HelpBox not parsing escape sequence correctly.
...
- Editor: Fixed modal windows so they are are not blank when they open.
...
- Editor: Modified the Editor in Windows so the text in dialogues can be copied and pasted.
    (UUM-41283)
...
- Editor: Removed the `scrollview` in the New Scene dialog to fix an issue with scene template.
...
- Graphics: Fixed a crash without error on VK and GL when wrong Graphics Buffer Target was set.
...
- HDRP: Fixed an issue that affected excluders when the camera was underwater.
    (UUM-33247)
...
- iOS: Fixed attached managed debugger window not showing up and that causied application to be in frozen state.
...
- Package Manager: Add a scroll in package manifest description field in Inspector.
    (UUM-42618)
...
- Shaders: Fixed an issue where Shader warmup was not being skipped through SRP initialization. This could negatively affect GPU perforamnce if the user specified a shader preloading asset. This functionality should only occur for OpenGL platforms.
    (UUM-43199)
...
- Universal RP: Added an RTHandle pool to fix an RTHandle reallocation at runtime.
...
- Video: Improve VP8Encoder compression and encoding speed.
