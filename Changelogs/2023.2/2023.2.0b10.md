...
- Editor: Analytics for package now derrives package source from the correct assembly.
    (UUM-47984)<br>
    *First seen in 2023.2.0b2.*    <br>*Fixed in 2023.2.0b11.*

- Editor: UUM-46918: Fix for 2023.2.X: Calling TrackPropertyValue from ListView.bindItem can throw an exception
    (UUM-46918)<br>
    *First seen in 2023.2.0a18.*    <br>*Fixed in 2023.2.0b11.*
...
### New 2023.2.0b10 Entries since 2023.2.0b9
...
- Android: Fixed locations of unaligned load/stores on 32-bit Android issue.
    (UUM-46550)<br>
...
- Apple TV: Fixed on screen keyboard unresponsive to controller when Remote.allowExitToHome was set to false.
...
- DX12: Fixed "ReleaseDynamicVBOFrame: Unexpected frame".<br>
    ...
    ([UUM-28962](https://issuetracker.unity3d.com/issues/releasedynamicvboframe-unexpected-frame-errors-thrown-in-the-console-window-when-creating-a-project-using-3d-sample-scene-hdrp-template))<br>
...
- Editor: Fixed an issue where context menu items would send the Editor to the background.
    ([UUM-36800](https://issuetracker.unity3d.com/issues/contextual-menus-save-layout-popup-isnt-displayed-for-the-user-after-the-contextual-menus-update))<br>
    *First seen in 2023.2.0a14.*

- Editor: Fixed an issue where static batching with transparent shadow was rendering incorrectly.
    (UUM-36082)

- Editor: Fixed an issue where the context menu persisted after domain reload.
    (UUM-36954)<br>
...
- Editor: Fixed an issue whtn the stage priority for Sprite renderer and Canvas renderer did not update the value as expected.
...
- Editor: Fixed an offset of camera position issuew when using the light placement tool.
    (UUM-47863)<br>
...
- Editor: Fixed update of probes validity after an editor restart issue.
    (UUM-42961)<br>
    *First seen in 2023.2.0b4.*

- Editor: \[Android\] Fixed Regex that checks for Play Libraries on 23.2 to avoid errors when using Core-Common libraries instead of only Play Core errors as expected. Regex now expects a series of digits/dots after the term "core" to avoid the confusion.
    ([UUM-44744](https://issuetracker.unity3d.com/issues/android-regex-in-checkplaylibraryadded-incorrectly-identifies-play-core-common-libraries))
...
- Graphics: Fixed a crash in HybridHDRPGfxTest Standalone.
    (UUM-35004)
...
- HDRP: Fixed an issuw where there was no error when the Rendering Layer names in HDRP Global Settings was empty.
...
- Mono: Fixed an Editor crash when mini_get_method fails.
...
- Networking: Fixed an issue where UnityWebRequest would fail to establish TLS connections with Windows Server 2012 and 2016.
...
- Package Manager: Fixed an issue where custom, local and git packages would not follow information from the package.json.
    (UUM-47732)<br>
    *First seen in 2023.3.0a4.*

- Physics: Fixed a crash that occured when ArticulationBody was added next to TerrainCollider that had trees on it.
    ([UUM-46116](https://issuetracker.unity3d.com/issues/crash-on-terraincollider-createtrees-when-entering-the-play-mode))

- Physics 2D: Fixed an issue in both Animation Root Motion rotation and Animation Transform where not rotated a Rigidebody2D correctly.
...
- SpeedTree: Fixed a popping LOD transitions issue for the Crossfade mode for URP.
    (UUM-40046)

- SpeedTree: Fixed an issue where meshes did not calculate UV distribution metrics on import correctly.

- SRP Core: Fixed a stripping.meta corrupted metal file issue.
    ([UUM-46994](https://issuetracker.unity3d.com/issues/file-stripping-dot-meta-in-core-srp-si-corrupt))<br>
...
- Terrain: Fixed a player crash when a terrain tile completely made of holes was viewed from a distance.
    ([UUM-45970](https://issuetracker.unity3d.com/issues/player-crashes-when-auto-connect-is-enabled-for-terrains-in-the-terrain-group))<br>
    *First seen in 2023.2.0b4.*

- TextMeshPro: TextMeshPro component does not perform linear color conversion when the VertexColorAlwaysGammaSpace option is enabled.
...
- UI Toolkit: Fixed a UI Builder Viewport background turns white after docking or undocking the UI Builder Window issue.
    ([UUM-47261](https://issuetracker.unity3d.com/issues/ui-builder-viewport-background-turns-white-after-docking-or-undocking-the-ui-builder-window))<br>
...
- UI Toolkit: Fixed context menu for bound attribute fields showed incorrect items.
    (UUM-46157)<br>
    *First seen in 2023.2.0b4.*
...
- Universal RP: Add Shader Keywords for Soft Shadow quality levels and disable per-light quality level on untethered XR platforms.
...
- Universal RP: Fixed an issue where it was not possible to add a Renderer Feature on a renderer if another feature had a missing/broken script.
...
### Preview of Final 2023.2.0b10 Release Notes
...
- Editor: Fiedx using deferred with a renderer feature requiring rendering layers.
...
- HDRP: Fixed a realtime Reflection probe issue that makes volumetrics clouds wind stop.
...
- HDRP: Fixed an issue where it was not allow for the game to switch HDR on or off during run time.
    (UUM-29770)
...
- HDRP: Fixed and error on water inspector when no SRP is active.
...
- Scripting: Fixed an Editor crash if a coroutine was not persisted.
