...
*   Graphics: Fix a crash when updating HDRP materials from "Edit -> Render Pipeline -> Upgrade HDRP Materials to Latest Version" when ray tracing is used (DirectX 12). ([1217526](https://issuetracker.unity3d.com/issues/editor-crash-when-upgrading-hdrp-material-to-latest-version-with-dxr-enabled-dx12))  
...
*   Packman: Package Manager keeps refreshing packages for a very long time when it is opened for a first time in a project ([1217827](https://issuetracker.unity3d.com/issues/package-manager-keeps-refreshing-packages-for-a-very-long-time-when-it-is-opened-for-a-first-time-in-a-project))
    
*   Packman: \[2019.3\] Package Manager does not list packages from scoped registries ([1217800](https://issuetracker.unity3d.com/issues/2019-dot-3-package-manager-does-not-list-packages-from-scoped-registries))
...
*   Physics: Cloth does not correctly bend and fall when in contact with a Collider ([1219030](https://issuetracker.unity3d.com/issues/cloth-does-not-correctly-bend-and-fall-when-in-contact-with-a-collider))
...
*   Profiling: \[Profiler Standalone\] Profiler stops profiling and throws NullReference Errors on changing Profiler Mode from Playmode to Editor ([1219310](https://issuetracker.unity3d.com/issues/profiler-standalone-profiler-stops-profiling-and-throws-nullreference-errors-on-changing-profiler-mode-from-playmode-to-editor))
    
*   Profiling: \[Profiler\] Standalone Profiler crashes on opening Lighting Settings ([1218485](https://issuetracker.unity3d.com/issues/profiler-standalone-profiler-crashes-on-opening-lighting-settings))
    
*   Project Browser: Unable to select Assets under Project Window in Asset Store ([1218481](https://issuetracker.unity3d.com/issues/project-browser-unable-to-select-assets-under-project-window-in-asset-store))
    
*   Scene Management: GameObject does not follow Camera Aligned Mode axes for translation ([1218738](https://issuetracker.unity3d.com/issues/scene-management-gameobject-does-not-follow-camera-aligned-mode-axes-for-translation))
...
*   Serialization: Editor freezes when trying to serialize generic type with SerializeReference ([1218652](https://issuetracker.unity3d.com/issues/editor-freezes-when-trying-to-serialize-generic-type-with-serializereference))
...
*   Shuriken: Major memory leak occurs when instantiating or dragging a Particle System Prefab with Light module into the Scene view ([1217998](https://issuetracker.unity3d.com/issues/major-memory-leak-occurs-when-instantiating-or-dragging-a-particle-system-prefab-with-light-module-into-the-scene-view))
    
*   UI Elements: Fixed a regression in the Animator graph UI caused by incorrect bounds checking when overriding mouse position. ([1215103](https://issuetracker.unity3d.com/issues/in-animator-window-left-mouse-button-is-not-registered-when-clicking-outside-of-the-default-view))  
...
*   Windows: \[Scene Management\] "Grid and Snap Settings" button in Scene Grid toggle button Alt-Tabs the window instead of opening settings ([1216637](https://issuetracker.unity3d.com/issues/scene-management-grid-and-snap-settings-button-in-scene-grid-toggle-button-alt-tabs-the-window-instead-of-opening-settings))
...
*   Editor: Call OnDestroy/Awake callbacks for ExecuteAlways scripts with the Configurable Play Mode option enabled. ([1209226](https://issuetracker.unity3d.com/issues/parent-class-awake-method-is-not-called-when-reload-scene-setting-is-disabled-and-child-class-has-executeineditmode-attribute))
...
*   Editor: Fixed an issue where the editor would crash on Ubuntu 16.04 when entering play mode. ([1214726](https://issuetracker.unity3d.com/issues/linux-editor-always-crashes-everytime-play-button-is-pressed-on-ubuntu-16-dot-04))  
...
*   Prefabs: Fixed Camera prefab continuously updated on resetting the camera component. ([1214033](https://issuetracker.unity3d.com/issues/camera-camera-prefab-continuously-updated-on-resetting-the-camera-component))  
...
*   Scripting: Fixed FindObjectOfType not returning RenderSettings. ([1213772](https://issuetracker.unity3d.com/issues/drag-a-material-over-scene-view-will-cause-undo-object-may-not-be-null-error))  
    _This is a change to a 2020.1.0a20 change, not seen in any released version, and will not be mentioned in final notes._
...
*   Shaders: Fixied camera replacement shader to work correctly with local keywords. ([1195767](https://issuetracker.unity3d.com/issues/requesting-non-existent-keyword-at-index-exception-is-thrown-after-shader-change))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   Terrain: Fixed issue where an unsupported texture format was reported in headless mod. ([1190805](https://issuetracker.unity3d.com/issues/builds-that-contain-terrain-using-the-nature-slash-terrain-slash-standard-shader-produce-errors-when-run-in-headless-mode))
