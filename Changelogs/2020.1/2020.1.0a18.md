...
*   Graphics - General: Crash on UnityEditor.Handles.Internal\_FinishDrawingCamera when moving around Scene view with baked Occlusion Culling ([1205699](https://issuetracker.unity3d.com/issues/crash-on-unityeditor-dot-handles-dot-internal-finishdrawingcamera-when-moving-around-scene-view-with-baked-occlusion-culling))
...
*   IMGUI: Asset Preview for cube shaper textures does not display ([1206795](https://issuetracker.unity3d.com/issues/asset-preview-for-cube-shaper-textures-does-not-display))
...
*   MacOS: \[Mac\] Crash on MTLIOAccelCommandBufferStorageGrowSegmentList.cold.1 when opening the Scene view in a default HDRP project ([1208854](https://issuetracker.unity3d.com/issues/mac-crash-on-mtlioaccelcommandbufferstoragegrowsegmentlist-dot-cold-dot-1-when-opening-the-scene-view-in-a-default-hdrp-project))
...
*   Mono: Crash on mono\_class\_setup\_methods with Begin MonoManager ReloadAssembly when opening project with Loxodon Framework ([1207588](https://issuetracker.unity3d.com/issues/crash-on-mono-class-setup-methods-with-begin-monomanager-reloadassembly-when-opening-project-with-loxodon-framework))
    
*   OpenGL: Editor crashes when calling Shader.WarmUpAllShaders() with one or more GPU Instanced Materials on OpenGL ([1201344](https://issuetracker.unity3d.com/issues/editor-crashes-when-calling-shader-dot-warmupallshaders-with-one-or-more-gpu-instanced-materials))
...
*   Packman: \[PackMan\] Packages list does not refresh under "All" after removing keyword from search field under "My Assets ([1209830](https://issuetracker.unity3d.com/issues/packman-packages-list-does-not-refresh-under-all-after-removing-keyword-from-search-field-under-my-assets))
...
*   Physics: Crash when using ArticulationBody hierarchy ([1206522](https://issuetracker.unity3d.com/issues/crash-when-using-articulationbody-hierarchy))
...
*   Profiling: \[Performance Regression\] "EditorLoop" takes a significant amount of time to execute ([1203866](https://issuetracker.unity3d.com/issues/editorloop-takes-a-significant-amount-of-time-to-execute))
...
*   Scene Management: \[Improved Prefabs\] Mouse cursor flickers continuously on creating preset of any Prefab object ([1208930](https://issuetracker.unity3d.com/issues/improved-prefabs-mouse-cursor-flickers-continuously-on-creating-preset-of-any-prefab-object))
...
*   Shadows/Lights: Realtime shadows are broken on OpenGL/ES3/Metal/Vulkan in Shadowmask mode ([1206092](https://issuetracker.unity3d.com/issues/shadowmask-shadow-doesnt-know-how-to-be-a-shadow))
...
*   Terrain: Editor freezes on adding branch group in Tree root node ([1206620](https://issuetracker.unity3d.com/issues/terrain-editor-freezes-on-adding-branch-group-in-tree-root-node))
...
### New 2020.1.0a18 Entries since 2020.1.0a17
...
*   Editor: Fixed the issue with folder name getting truncated if it contains dot or period character. ([1203285](https://issuetracker.unity3d.com/issues/folder-name-not-rendered-fully-if-there-is-a-dot-slash-period-dot-character-in-the-name))  
...
*   GI: Fixed a rare case of error 502 thrown when baking with the CPU progressive lightmapper. Also improved performance when baking scenes with LoDs with the CPU lightmapper. ([1144990](https://issuetracker.unity3d.com/issues/cpu-plm-editor-throws-openrl-reported-error-code-502-errors-while-baking-lighting))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Graphics: Fixed ECS RenderMeshSystemV2 ubstantial amount of memory leak. ([1151798](https://issuetracker.unity3d.com/issues/ecs-rendermeshsystemv2-has-substantial-amount-of-memory-leak))
    
*   Graphics: Fixed issue where SRP would allocate memory each frame. ([1165372](https://issuetracker.unity3d.com/issues/lwrp-playerloop-gc-dot-alloc-is-allocating-every-frame-when-using-lwrp-settings))
...
*   Profiler: Target Selection drop-down menu gets stuck on "Autoconnected Player" after failing to connect to a player. ([1193777](https://issuetracker.unity3d.com/issues/profiler-stucks-with-autoconnected-player-option-and-cant-be-switched-back-to-editor-when-androidplayer-is-selected))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Timeline: Fixed Foot IK being applied in Editor when option is disabled. ([1197426](https://issuetracker.unity3d.com/issues/timeline-foot-ik-property-has-no-effect-on-the-animation-when-timeline-preview-is-played))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Video: Fixed split-alpha layout support for H.264/H.265 on Windows/UWP when decode-to-texture is enabled. ([1192954](https://issuetracker.unity3d.com/issues/stereo-video-becomes-horizontally-stretched-when-transcoded-with-keep-alpha-enabled))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Shaders: Introduced "#include\_with\_pragmas", a new directive which allows users to specify snippet parameters in included files when using new shader preprocessor.  
    _This is a change to a 2020.1 change, not seen in any released version, and will not be mentioned in final notes._
...
### Preview of Final 2020.1.0a18 Release Notes
...
*   Asset Import: Fixed AnimationClip fileIds generated from model imported before Unity 3.2 not conserving their fileIds and references when opened in a 2019.1+ project. ([1186494](https://issuetracker.unity3d.com/issues/animation-animation-references-are-lost-from-animation-component-when-project-is-upgraded))
...
*   GI: Fixed transparency in CPU lightmapper. ([1183940](https://issuetracker.unity3d.com/issues/transparency-broken-in-cpu-lightmapper))
...
*   iOS: Fixed an issue where Unity would crash if force quit immediately after launching. ([1145982](https://issuetracker.unity3d.com/issues/ios-crash-on-lookup-core-basic-string-char-core-stringstoragedefault-at-hash-set-dot-h-645-49-during-unity-cleanup))
...
*   iOS: Metal will be added to the required device capabilities list unless GLES2/3 are explicitly added to the GFX APIs list in Player Settings. ([1201094](https://issuetracker.unity3d.com/issues/ios-player-crashes-on-ipad-4th-generation))
...
*   Profiler: Fixed assembly prefix makes reading profiler harder. ([1182855](https://issuetracker.unity3d.com/issues/profiler-assembly-prefix-makes-reading-profiler-harder))
...
*   Video: Fixed asset bundles which include VideoClip have different CRC values when built from a different directory. ([1152507](https://issuetracker.unity3d.com/issues/asset-bundles-which-include-videoclip-have-different-crc-values-when-built-from-a-different-directory))
...
*   Video: \[Windows/UWP\] Fixed GPU memory leak. ([1201213](https://issuetracker.unity3d.com/issues/windows-video-graphics-memory-leak-in-hardware-accelerated-video-decoding))
...
*   Particles: Lifetime by Emitter Speed Module, for controlling how long-lived particles are based on the speed of the emitter at the time they were emitted. This is for instance useful for moving fire.  
    ![description](/sites/default/files/tkkulbj.gif)
