...
*   Templates: Project templates will be updated in subsequent betas to address the following points:
    ...
    *   The _2D_ template currently creates a default scene with a UI canvas: this will be removed in a subsequent build.
    *   The _3D Ultra_ template is missing the "Preview" tag currently.
    *   _All templates_: Lighting settings improvements to come in a subsequent build.
...
*   IL2CPP: Windows and Mac standalone players with IL2CPP scripting backend no longer use stacktrace sentries in development builds. This increases performance significantly in certain scenarios. ([989030](https://issuetracker.unity3d.com/issues/stacktrace-sentries-are-enabled-in-development-builds-of-windows-and-mac-standalone-players-with-il2cpp-scripting-backend))
    
*   IL2CPP: Windows and Mac standalone players with IL2CPP scripting backend now always have null and array bound checks enabled. ([989028](https://issuetracker.unity3d.com/issues/windows-and-mac-standalone-players-with-il2cpp-do-not-have-null-checks-enabled-in-non-development-builds))
...
*   Android: Fixed an issue where the full-screen movie player didn't autorotate as expected. ([962450](https://issuetracker.unity3d.com/issues/android-screen-orientation-can-not-be-changed-when-playing-video-with-handheld-dot-playfullscreenmovie))
    
*   Animation: Fixed case of animator being resampled when enabling/disabling Game Object in Editor scene. (986876)
    
*   Animation: Fixed layout issue when deleting clip in a blend-tree inspector. ([972008](https://issuetracker.unity3d.com/issues/rat-deleting-a-clip-from-a-blend-tree-in-the-inspector-generates-errors), 986882)
    
*   Asset Import: Corrected messaging around Substance Designer import removal. ([986520](https://issuetracker.unity3d.com/issues/typo-in-message-proceduralmaterial-slash-proceduraltexture-are-deprecated-and-no-loguer-available), 986521)
    
*   Asset Import: Fixed case of `AssetPostprocessor.OnAssignMaterialModel` not being called for all models. ([983321](https://issuetracker.unity3d.com/issues/assetpostprocessor-onassignmaterialmodel-not-called-for-meshes-inside-fbx-with-disabled-mesh-renderer))
    
*   Build Pipeline: Fixed crash when loading meshes from asset bundles that contain both compressed and uncompressed meshes. ([984070](https://issuetracker.unity3d.com/issues/editor-crashes-when-loading-a-mesh-asset-with-mesh-compression-from-an-assetbundle), [995368](/False))
    
*   Editor: Fixed regression with Assembly-CSharp.dll and friends referencing editor-only assemblies. ([982271](https://issuetracker.unity3d.com/issues/editor-assemblies-get-referenced-in-main-and-plugins-projects))
    
*   GI: Fixed case of baking pipeline still running if GI systems are disabled. (989061)
    
*   IL2CPP: Enabled dead code stripping for the clang toolchain. ([976218](https://issuetracker.unity3d.com/issues/mac-dead-code-stripping-is-not-enabled-in-il2cpp-builder-in-release-builds))
    
*   IL2CPP: Fix to prevent an unlikely but possible crash when default parameter values are used. ([973052](https://issuetracker.unity3d.com/issues/il2cpp-android-segfault-crash-when-switching-scenes-with-il2cpp-on-android))
    
*   IL2CPP: Fixed a crash in the Unity Linker (`System.InvalidProgramException: Failed to resolve [UnityEngine]System.Resources.NeutralResourcesLanguageAttribute`) when using a managed DLL containing a non-existing attribute. ([969958](https://issuetracker.unity3d.com/issues/uwp-project-with-es2-plugin-fails-to-build-because-of-fatal-error-in-unity-cil-linker))
    
*   IL2CPP: Implemented email sending via `SmtpClient` with .NET 4.6. ([947208](https://issuetracker.unity3d.com/issues/il2cpp-dot-net-4-dot-6-mail-doesnt-send-when-calling-smtpclient-dot-send))
    
*   IL2CPP: Implemented method `EnsureSufficientExecutionStack` for .NET 4.6. ([976623](https://issuetracker.unity3d.com/issues/uwp-when-running-the-app-sufficientexecutionstack-method-is-not-implemented-is-thrown))
    
*   IL2CPP: Improved the performance of code conversion when using many nested generic types. ([975496](https://issuetracker.unity3d.com/issues/il2cpp-freeze-while-assembling-dlls))
    
*   iOS: Fixed ReplayKit recording on iOS 11. ([940872](https://issuetracker.unity3d.com/issues/ios-replaykit-recording-with-replaykit-fails-on-ios-11-latest-beta))
    
*   Particles: Fixed issue where a particle system would sometimes disappear when it was close to the edge of the frustum and the camera was moving. ([964176](https://issuetracker.unity3d.com/issues/particle-system-under-canvas-disappears-sometimes-when-camera-is-moving))
    
*   Particles: Fixed issue where particle systems with negative Transform scale were sometimes rendered incorrectly. ([979638](https://issuetracker.unity3d.com/issues/particles-with-render-mode-mesh-render-alignment-local-and-negative-scale-greater-than-1-are-not-scaled-correctly), [982998](https://issuetracker.unity3d.com/issues/minor-changes-to-y-rotation-of-particle-system-with-x-scale-1-or-below-result-in-huge-visual-difference))
...
*   UI: Fixed issue with UI and prefab positioning when disabling/re-enabling child object. ([977959](https://issuetracker.unity3d.com/issues/turning-off-a-child-object-in-canvas-applying-changes-to-prefab-then-turning-on-again-breaks-its-layout), [986372](/False), [988999](/False))
    
*   Universal Windows Platform: Fixed case of keyboard input not distinguishing between left/right shift keys. ([968801](https://issuetracker.unity3d.com/issues/uwp-builds-do-not-distinguish-between-left-and-right-shift-keys-when-checking-input))
    
*   Windows: Fixed case of default with/height setting not being honored for 'Windowed' mode. ([968584](https://issuetracker.unity3d.com/issues/windows-default-is-native-resolution-overrides-window-width-slash-height-when-full-screen-is-disabled))
...
*   XR: Fixed an issue with lights failing to illuminate scene geometry completely during stereo rendering. ([965494](https://issuetracker.unity3d.com/issues/mixed-reality-spot-or-point-lighting-problem-in-windows-mixed-reality))
    
*   XR: Fixed crash during build when using stereo display. ([977488](https://issuetracker.unity3d.com/issues/build-crashes-when-using-any-virtual-reality-sdk), [987308](/False))
    
*   XR: Fixed crash when connecting simulator device while running in Editor. ([980089](https://issuetracker.unity3d.com/issues/unity-crashes-when-stop-playing-a-scene-after-xrdevice-dot-enabled-equals-true-at-runtime), 989354)
    
*   XR: Fixed crash when Holographic Emulation is set to Remote with no device connected. ([952777](https://issuetracker.unity3d.com/issues/unity-crashes-on-playing-the-scene-at-vrdevicehololens-getdeviceconnected))
    

2018.1.0b5 Release Notes (Full)
-------------------------------
...
*   iOS: Fixed black screen flickering when changing orientation via script. ([953524](https://issuetracker.unity3d.com/issues/ios-when-changing-screen-orientation-via-script-black-frame-appears), [994961](/False))
