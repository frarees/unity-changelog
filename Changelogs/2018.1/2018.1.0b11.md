...
*   GI: "SerializedObject target has been destroyed" appears after environment lighting change. ([978554](https://issuetracker.unity3d.com/issues/imgui-in-playmode-clicking-slash-hovering-on-the-lighting-tab-throws-exception-serialised-object-target-destroyed))
...
*   GI: In built Player the `IsLightmapBakeTypeSupported()` GCAllocs 17B every frame when CPU usage is 0.0%. ([986319](https://issuetracker.unity3d.com/issues/in-built-player-the-islightmapbaketypesupported-gcallocs-17b-every-frame-when-cpu-usage-is-0-dot-0-percent))
    
*   GI: \[PLM\] Supersampling doesn't work. ([1006916](https://issuetracker.unity3d.com/issues/plm-supersampling-doesnt-work))
...
*   Graphics: Editor crashes at `ImageFilters::BlitMultiTap` when stepping into `Graphics.BlitMultiTap()` method. ([1004159](https://issuetracker.unity3d.com/issues/editor-crashes-at-imagefilters-blitmultitap-when-stepping-into-graphics-dot-blitmultitap-method))
...
*   Scripting: Editor crashes when calling `NativeArray.Equals()`. ([1005597](https://issuetracker.unity3d.com/issues/editor-crashes-when-calling-nativearray-dot-equals))
...
*   Android: Warnings are now disabled in ProGuard by default. This fixes build errors with 3rd party plugins. ([935077](https://issuetracker.unity3d.com/issues/android-project-with-iap-plugin-fails-to-build-with-gradle-release-mode-only-and-throws-java-dot-io-dot-ioexception), 986781)
...
*   Audio: Fixed a bug where the matrices passed to spatializer and ambisonic audio plugins could be set to the identity matrix briefly when a sound starts playing. ([991034](https://issuetracker.unity3d.com/issues/native-audio-spatializer-plugin-sometimes-receives-incorrect-source-slash-listener-transforms-when-a-source-starts-playing))
    
*   Editor: A GameView on a Retina monitor with "Low Resolution Aspect Ratio" disabled no longer causes UGUI scaling issues. ([997931](https://issuetracker.unity3d.com/issues/canvas-is-quarter-size-on-retina-display-when-target-platform-is-set-to-ios), 1008227)
...
*   Editor: Fixed case of incorrect message being shown in dialog when changing a gameobject static flag from everything selected to everything but one item. ([1005672](https://issuetracker.unity3d.com/issues/unchecking-a-static-flag-for-an-object-hierarchy-displays-an-incorrect-message))
    
*   Editor: Fixed Metal Editor always having vsync enabled in OSX 10.13. (1009685)
    
*   Editor: Fixed regression where pasting a color in the inspector after having opened the color picker would not paste it to the correct color field. ([1005340](https://issuetracker.unity3d.com/issues/cannot-copy-paste-color-from-one-color-field-to-another-if-they-are-on-the-same-gameobject))
...
*   Editor: Raycasting into a GameView on a Retina monitor with "Low Resolution Aspect Ratio" disabled no longer yields invalid results. ([978659](https://issuetracker.unity3d.com/issues/osx-navmesh-agents-are-broken), 1008228)
    
*   Editor: Tests in assemblies with similar names no longer result in an error when displaying the test list. ([995476](https://issuetracker.unity3d.com/issues/testrunner-two-errors-are-printed-in-console-after-creating-script-in-test-runner-folder))
    
*   GI: Fixed issue where clicking/hovering on the Lighting tab when in playmode would result in "Serialized Object target destroyed" exception being thrown. ([978554](https://issuetracker.unity3d.com/issues/imgui-in-playmode-clicking-slash-hovering-on-the-lighting-tab-throws-exception-serialised-object-target-destroyed))
    
*   Graphics: Fixed an issue where clearing the GI Cache and reloading the project would result in errors being shown in the console. ([975975](https://issuetracker.unity3d.com/issues/enlighten-realtime-light-probes-data-fail-to-load), [980811](https://issuetracker.unity3d.com/issues/errors-appear-in-the-console-after-cleaning-gi-cache-and-reloading-project))
    
*   Graphics: Fixed crash in `MemoryManager::Reallocate` when using Particles/Lines/Trails. ([999559](https://issuetracker.unity3d.com/issues/crash-on-memorymanager-reallocate-when-shooting-objects-with-particles))
    
*   Graphics: Fixed issue where light probes wouldn't load in additively loaded scenes. ([957736](https://issuetracker.unity3d.com/issues/real-time-probes-dont-load-from-additive-loaded-scene))
    
*   Graphics: Fixed issue where lighting data was much bigger in additively loaded scenes vs in a single scene. ([913869](https://issuetracker.unity3d.com/issues/lightingdata-for-additively-loaded-scenes-baked-together-is-much-bigger-than-for-a-single-scene-with-same-geometry))
...
*   OSX: Fixed case of mouse input jumping by a large number at the end of movement. ([992887](https://issuetracker.unity3d.com/issues/mac-mouse-input-jumps-by-a-large-number-at-the-end-of-movement))
    
*   Particles: Fixed crash when particle system has no renderer module. ([995995](https://issuetracker.unity3d.com/issues/crash-in-immediateptr-operator-when-spawning-a-specific-particle-system-object-in-game), 1008128)
    
*   Scripting: Cured various APIs crashes and erroneous return values on .NET backend. ([981757](https://issuetracker.unity3d.com/issues/wsa-isnetworkerror-always-return-true-when-running-an-uwp-app-on-86x-architecture), 987059, 987192, 989205, [989206](https://issuetracker.unity3d.com/issues/wsa-www-method-using-master-solution-configuration-is-receiving-an-empty-data), 991009, 993363)
    
*   Scripting: Fixed a crash when scheduling C# jobs with some very large batch sizes. ([1004878](https://issuetracker.unity3d.com/issues/unity-crashes-at-raycastcommandjob-when-going-from-30000-to-40000-batched-raycasts-per-frame))
...
*   Universal Windows Platform: Fixed a crash in .NET native compiler (`RHB0011: Internal error: 'pchTypeNameEnd[0] == L'&gt;"at'f:\dd\ndp\rh\src\tools\rhbind\makepdb.cpp:998`) in certain scenarios involving plugins. ([991154](https://issuetracker.unity3d.com/issues/regression-uwp-project-fails-to-build-under-net-native-tool-chain))
    
*   Universal Windows Platform: Fixed case of build failing on IL2CPP scripting backend if class library from Mono directory (such as `System.Data.dll`) was copied into the project. ([997876](https://issuetracker.unity3d.com/issues/wsa-reference-rewriter-fails-with-target-system-dot-data-dot-dll-under-il2cpp))
    
*   Universal Windows Platform: Fixed case of Windows Runtime not being enabled when using .NET Standard 2.0 profile. ([1002417](https://issuetracker.unity3d.com/issues/windows-runtime-support-is-broken-with-net-standard-2-api-profile))
    
*   Video: Fixed flickering video playback on iOS/Metal (especially visible on High Sierra). ([972997](https://issuetracker.unity3d.com/issues/video-is-flickering), 986039)
    
*   Web: Fixed abort on iOS on UnityWebRequest timeout. ([1001438](https://issuetracker.unity3d.com/issues/unitywebrequest-ios-unitywebrequest-dot-getassetbundle-sometimes-never-finishes-on-ios-with-bad-network-connection), 1004326)
    
*   Windows: Added missing parentheses in the description for Windows Standalone player with IL2CPP scripting backend.
    
*   Windows: Fixed case of resolution dialog not showing the monitor list and crashing on pressing 'Play' button on some uncommon monitor setups. ([996015](https://issuetracker.unity3d.com/issues/combo-box-select-monitor-is-empty-on-windows-build))
...
*   XR: Fixed case of `GestureRecognizer` manipulation event containing incorrect data after resuming from sleep. ([976544](https://issuetracker.unity3d.com/issues/hololens-manipulation-event-data-is-wonky-after-resuming-from-sleep))
...
*   XR: Fixed case of `InteractionSourcePressedEventArgs` and `InteractionSource` reporting incorrect data on airtap. ([986569](https://issuetracker.unity3d.com/issues/hololens-interactionsourcepressedeventargs-and-interactionsource-returns-a-incorrect-data-on-airtap))
    
*   XR: Fixed log spam when app starts disconnected from Windows MR HMD, and crash when HMD is reconnected. ([950796](https://issuetracker.unity3d.com/issues/windowsmr-rendering-2017-plugging-a-hmd-with-a-running-mr-app-causes-performance-to-drop-from-a-warning-spam), 1008178)
    

2018.1.0b11 Release Notes (Full)
--------------------------------
