...
*   2D: Fixed case of error message "A script behaviour has a different serialization layout when loading" being thrown when a Tilemap is loaded in players with code stripping. ([969827](https://issuetracker.unity3d.com/issues/mobile-error-a-script-behaviour-has-a-different-serialization-layout-when-loading-when-instantiating-tiled-grid-prefab))
    
*   2D: Fixed case of Tilemap not setting Tiles with the correct data in players with code stripping. ([965608](https://issuetracker.unity3d.com/issues/webgl-tilemap-tilemap-dot-settile-does-not-set-the-tile-on-webgl-build))
    
*   2D: Fixed crash after building and loading an Asset Bundle with Tilemap in the Editor. ([976411](https://issuetracker.unity3d.com/issues/unity-crashes-at-validateandupdatetileassetdirty-after-building-asset-bundle-and-entering-play-mode))
    
*   2D: Fixed crash when Tilemap is loaded with invalid Tiles in the Editor. ([972214](https://issuetracker.unity3d.com/issues/editor-crashes-after-entering-slash-exiting-play-mode-while-having-tile-palette-open-and-closing-the-tile-palette-window))
...
*   Build Pipeline: Fixed crash when loading meshes from asset bundles that contain both compressed and uncompressed meshes. ([984070](https://issuetracker.unity3d.com/issues/editor-crashes-when-loading-a-mesh-asset-with-mesh-compression-from-an-assetbundle))
...
*   Editor: Using `Application.internetReachability` with OnGUI no longer results in errors in the Console. ([967806](https://issuetracker.unity3d.com/issues/using-application-dot-internetreachability-with-ongui-causes-errors-in-the-console), 993500)
...
*   GI: Fixed issue where `Lightmapping.BakeMultipleScenes` executed via batch mode with the `-quit` argument would fail to create lightmap textures. ([949305](https://issuetracker.unity3d.com/issues/lightmapping-dot-bakemultiplescenes-executed-via-batch-mode-with-quit-argument-does-not-create-light-map-files))
...
*   Network: Fixed crash during GC collection when using `Ping` class. ([972519](https://issuetracker.unity3d.com/issues/ping-causes-lag-on-android-devices), 984998)
...
*   Scripting Upgrade: Fixed Android crash when script debugger is enabled. ([973794](https://issuetracker.unity3d.com/issues/android-script-debugging-causes-android-to-crash-on-startup-with-net-4-dot-6))
    
*   Scripting Upgrade: Fixed Android crash when `NullReferenceException` is raised. ([954427](https://issuetracker.unity3d.com/issues/android-crash-on-builds-with-net-4-dot-6-scripting-runtime-selected-when-null-reference-exception-appears))
    
*   Scripting Upgrade: Fixed case of profiler not counting GC allocations. (988906)
    
*   Scripting Upgrade: Fixed cause of `EntryPointNotFoundException` when calling `HTTPClient.GetAsync` on Android. ([935292](https://issuetracker.unity3d.com/issues/android-dot-net-4-dot-6-httpclient-dot-getasync-fails-to-get-address-with-unable-to-find-libc-error))
    
*   Scripting Upgrade: Fixed crash on exit when script debugger is enabled. (974228)
    
*   Scripting Upgrade: Fixed crash when using named pipes. ([942966](https://issuetracker.unity3d.com/issues/unity-crashes-when-external-named-pipe-server-is-used))
    
*   Scripting Upgrade: Fixed issue where `Socket.Send` would fail silently on Windows. ([984723](https://issuetracker.unity3d.com/issues/windows-system-dot-net-dot-sockets-dot-socketexception-when-saving-is-run-through-post-slash-put-requests-on-third-party-library))
    
*   Scripting Upgrade: Fixed issue with dynamic assemblies never being unloaded when entering/exiting playmode. ([923165](https://issuetracker.unity3d.com/issues/parental-assembly-gets-unloaded-when-dynamically-loading-child-assembly-from-bytes-with-net-4-dot-6))
    
*   Scripting Upgrade: Fixed issue with Windows Standalone 32-bit randomly failing to start. (980742)
    
*   Services: Performance Reporting: we now report the device model field for managed exceptions. ([980806](https://issuetracker.unity3d.com/issues/device-model-is-not-reported-to-performance-reporting-service-for-managed-exceptions), 980814)
...
*   UI: Fixed issue where the position and rotation specified when calling `Object.Instantiate` were not applied correctly for a RectTransform. ([980300](https://issuetracker.unity3d.com/issues/instantiating-prefab-with-recttransform-places-it-in-00-z-position-instead-of-parent-position), 992285)
...
*   Web: Fixed file URI with host component in UnityWebRequest. ([971557](https://issuetracker.unity3d.com/issues/file-transfer-protocol-fails-to-load-resources), 991605)
    
*   XR: Fixed crash during build when using stereo display. ([977488](https://issuetracker.unity3d.com/issues/build-crashes-when-using-any-virtual-reality-sdk), 987308)
    
*   XR: Fixed crash on HoloLens when trying to use touch keyboard. (992326)
    
*   XR: Fixed screenshot capture in VR. ([956227](https://issuetracker.unity3d.com/issues/screencapture-dot-capturescreenshot-is-not-working-in-mixed-reality-when-running-from-a-build), 986618)
    

2018.1.0b6 Release Notes (Full)
-------------------------------
