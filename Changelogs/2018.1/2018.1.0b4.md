...
*   Cache Server: Fixed an issue where using a Cache Server and reimporting Assembly Definition files would fail to trigger rebuilding of the assembly. ([975773](https://issuetracker.unity3d.com/issues/assembly-definition-files-do-not-force-assembly-creation-on-reimport-all-if-cache-server-is-used), 983744)
...
*   Editor: Removed "Launch Unity" option from Windows installers, to prevent Unity from running as an elevated process. ([975351](https://issuetracker.unity3d.com/issues/files-cannot-be-dragged-from-file-explorer-to-project-view), 988272)
    
*   Editor: The "Show All on Dashboard" button on the Collab history window now points to the correct web endpoint. ([958606](https://issuetracker.unity3d.com/issues/collab-service-window-show-all-on-dashboard-leads-to-list-of-projects-page-on-developer-dashboard), 987279)
    
*   iOS: Fixed problem with switching between input fields with different keyboard/input parameters (987956)
    
*   Physics: Fixed case of double full stop on the dropdown boxes in physics settings. ([958814](https://issuetracker.unity3d.com/issues/double-full-stop-dot-at-the-end-of-the-sentence-in-the-projectsettings-slash-physics-tooltips))
    
*   Physics: Fixed crash in PhysX when performing box query on Android. ([878407](https://issuetracker.unity3d.com/issues/android-physx-crash-when-calling-physics-dot-overlapboxnonalloc-or-overlapbox))
    
*   Physics: Fixed crash that could occur when entering playmode, after importing a package that changes physics settings. ([958230](https://issuetracker.unity3d.com/issues/regression-crash-on-physicsmanager-setcollidertransform-while-entering-playmode-after-reloading-scene-that-had-changes-on-disk))
    
*   Physics: Fixed issue where errors would be thrown when disabling a collider on an inactive object and then destroying the object or exiting playmode. ([968882](https://issuetracker.unity3d.com/issues/vuforia-gameobject-transforms-are-not-being-disabled-when-the-player-is-stopped), 985758)
...
*   Web: Fixed case of file URI unescaping with multibyte chars in UnityWebRequest/WWW. ([978521](https://issuetracker.unity3d.com/issues/reading-local-file-with-unitywebrequest-which-contains-number-and-multibyte-character-in-its-path-fails), 986236)
