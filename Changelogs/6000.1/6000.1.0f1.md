...
### Final 6000.1.0f1 Release Notes
...
- 2D: Fixed an issue where the warning 'GetControlID at event ValidateCommand returns a controlID different from the one in the Layout event' was logged when undoing the deletion of an Edited Freeform 2D Light.
...
- Editor: Fixed an issue where AssetDatabase.GetAssetBundleDependencies returned list didn't include a dependency when it was a Prefab Asset.
...
- Editor: Fixed an issue where enum fields were not displayed on click in macOS when Tap to Click was enabled.
...
- Editor: Fixed an issue where late binding of SpriteAtlas objects didn't work when loading from content archives.
...
- Editor: Fixed an issue where the About box said license was 'Unity Indie' after returning all licenses.
    (UUM-84063)
...
- Editor: Fixed an issue where the graphics jobs toggle didn't work correctly in Project Settings when building for standalone platforms.
...
- Editor: Fixed an issue where the `AddBlitPass` RenderGraph function did not work for XR multiview array textures.
...
- Editor: Fixed an issue where users of a cloud project with the "Inherited from Organization" role couldn't see the project in the Project Binding view.
    (UUM-82760)
...
- Editor: Fixed an issue where `AnyTargetMaterialHasChildren` was called for non-material assets being deleted from the Project Browser.
...
- Editor: Fixed an issue with opening Editor log when -logFile is a relative path.
...
- Editor: Fixed an issue with same static batching data when building a player from the Editor or from command line.
...
- Editor: Fixed an issue with the `TextureImporter.GetAutomaticFormat API` where it returned an unexpected format if the provided platform name referred to a platform without an installed build support module.
    (UUM-95944)

- Editor: Fixed an issue with the `TextureImporter.GetAutomaticFormat` API where it returned an unexpected format if the provided platform name referred to certain platforms \(such as iOS\).
    (UUM-93298)
...
- Editor: Fixed an issue with `applyToCollection` attribute condition when using custom property drawer.
...
- Editor: Fixed an issuse where a corrupted UXML file opened in the UI Builder.
    (UUM-71448)

- Editor: Fixed an isue by adding a shared parameter indicator "*" to the name "Use incremental GC".
...
- Editor: Fixed dropdown items were not highlighted when setting padding in .unity-base-dropdown__item and hovering the mouse over the label.
...
- Editor: Fixed errors related to incompatible keyword spaces when loading a project using ray tracing shaders. The errors were generated when using closest hit shaders and keywords in a fallback shader.
...
- Editor: Fixed GradientPicker removing all swatches when dragging one outside.
...
- Editor: Fixed the modified time for `.meta` files when creating a new script file in an embedded package.
...
- Editor: Fixed the placement of the Prefab overrides comparison popup and other popups on multi-monitor setups with varying display scaling.
...
- Editor: Fixed the size of search icons.
...
- Editor: Fixed the size of search icons.
    ([UUM-93350](https://issuetracker.unity3d.com/issues/search-inspector-section-icon-is-less-sharp-slash-more-pixelated-than-other-editor-icons))
...
- Editor: Improved the Add Component window to truncate long item names with an ellipsis.
...
- Editor: Removed Edit menu items added by Apple Intelligence from view.
    (UUM-96160)
...
- Editor: \[Android\]  Fixed an issue by adding a warning informing users that UnityEditor.AndroidCreateSymbols enumeration is now obsolete and updated documentation on Unity 6000.1.
    (UUM-93309)
...
- Editor: \[APV\] Fixed an issue where it was impossible to switch between multiple Baking Set when multiple scenes are loaded additively.
...
- Editor: \[Device Simulator\] Fixed issue to return the same value of Screen.currentResolution.refreshRateRatio.value as in Game view.
...
- GI: Fixed a bad Reflection Probe rendering of Volumetric Clouds in HDRP.
...
- GI: Fixed an issue by removing the clamping of the environment sample count in the Lighting window to a maximum of 2048. Users can now specify higher sample counts if needed.
...
- GI: Reverted a change to packing made in Unity 6, which based lightmap packing order on the positions of lightmapped objects in the scene. This didn't have the intended effect, and significantly worsened utilization of lightmap space.
...
- Graphics: Fixed a crash in the Vulkan Editor that would occur when texture uploads happen during an active render pass.
...
- Graphics: Fixed a crash that occurred when using UIElements in URP with Vulkan.
...
- Graphics: Fixed a NullReferenceException that occured when baking a scene with multiple APVs, terrain, and trees.
...
- Graphics: Fixed an error that stated "A renderer material mask can't store more than 128 materials."
...
- Graphics: Fixed an issue where shadow culling did not work for non-Mesh Renderer types, resulting in unnecessary draw calls being generated.
...
- Graphics: Fixed an issue where the cmpression of NPOT textures with mipmaps was not supported by the GPU. Texture2D.Compress function has been updated to detect that and not compress the texture, issuing an error message, so that it is consistent with the texture import pipeline.
...
- Graphics: Fixed an issue where the `AddCopyPass` RenderGraph function did not work with XR multiview array textures.
    (UUM-93821)

- Graphics: Fixed an issue where zero-byte files were created when baking Adaptive Probe Volumes \(APV\).
    (UUM-85411)

- Graphics: Fixed an issue where `GraphicsStateCollection::GetVariants()` caused a memory allocation crash in the Editor.
...
- Graphics: Fixed an issue with shadows for additional lights in URP in custom shaders.
...
- Graphics: Fixed lens flare's incorrect position when Camera's Viewport Rect has a nonzero offset, or if hardware dynamic resolution is enabled.
...
- Graphics: Fixed seams appeared between non-infinite instanced quads in the water surface.
...
- Graphics: Updated info in an error message to give users more context that the max allowed render texture size was exceeded when inspecting an asset in the Editor.
...
- HDRP: Fixed an error that occurred when using more than 200 Local Volumetric Fog.
...
- HDRP: Fixed an issue where underwater collider bounds checks were inaccurate when the collider was rotated.
...
- IL2CPP: Fix a UnityLinker crash that occurred when `--enable-report` is used on a large project.
...
- IL2CPP: Fixed an issue where builds failed if the project's root directory contained files that matched project assembly names.
    ([UUM-91340](https://issuetracker.unity3d.com/issues/il2cpp-build-fails-when-there-are-empty-files-in-projects-root-directory))

- IL2CPP: Fixed an issue where engine module test assemblies could potentially be stripped entirely.
    (UUM-86462)
...
- IL2CPP: \[setlocale\]\(https://www.qnx.com/developers/docs/7.1/\#com.qnx.doc.neutrino.lib_ref/topic/s/setlocale.html\) fixes the performance on QNX for the first CultureInfo call.CultureInfo call tested on NXP iMX8QM:~240 ms without fix.~70 ms with the fix.
    (UUM-86398)
...
- Input System: Fixed an issue where the Input System became unresponsive after the game or application had been running for a long time or when using high-frequency input devices.
...
- iOS: Fixed an issue where if the "Prepare iOS for Recording" player setting was true and the "Mute Other Audio Sources" player setting was false, we would stop an external app's audio when first starting the Unity app.
...
- iOS: Improved the responsiveness of motion devices \(Accelerometer, Gyroscope, Attitude sensor\) after device orientation changes when using Input System.
...
- Linux: Fixed an assertion failure on ExFAT file systems when *Build and Run* is executed.
...
- Linux: Fixed"Shutdown worker was forced killed because it kept running. Worker x" was not appearing in the console.
    (UUM-83635)

- macOS: Fixed a bundle hashing exception that contains an empty file.
    (UUM-84930)
...
- Package Manager: Fixed a crash when package contains a dependency with null version in the package manifest.
...
- Package Manager: Fixed an issue where dropdown menus sometimes opened outside the Package Manager window.
...
- Package Manager: Fixed an issue where switching Projects during the import of a complete Project prevented all assets from importing, causing the Project to open incomplete.
...
- Package Manager: Improved the loading time of the Package Manager window.
    (UUM-90165)
...
- Package Manager: Switching Project when importing complete project conflict with opened project
    (UUM-100277)
...
- Particles: Fixed a potential crash that occurred when a sub-emitter had multiple parent emitters.
...
- Particles: Fixed an issue by clarifing the unexpected behavior on ParticleSystem.Play\(\) API documentation.
...
- Physics: Fixed a PhysX error that stated "Failed to create Convex Mesh from source mesh" which appeared when looking around the Scene view while having the Physics Debug tab open.
...
- Physics: Fixed an issue where calling `Physics.IgnoreCollision(colA, colB, false)` did not trigger PhysX's collision filter to re-evaluate contacts, which allowed the two shapes to still ignore collision until filtering data was updated.
...
- Physics: Secured Physics.IgnoreCollisions\(\) API to prevent having a collider ignore itself.
    (PHYS-405)

- Physics: Updating tooltip for DynamicFriction field in PhysicsMaterial asset.
...
- Prefabs: Fixed an issue where undoing changes to Prefab variants in Prefab Mode in Context caused the dummy parent object to be lost during `MergePrefab`. This issue resulted in the Transform being moved to the scene root and the Transform hierarchy being re-calculated.
...
- Prefabs: Fixed Handle PrefabInstances as Added GameObjects with deleted parent in the source Prefab.
...
- Scene/Game View: Fixed an issue with the Camera Gizmo flickering in the Scene view and texture stretching in the Camera Overlay.
...
- Scripting: Fixed a crash on reimport after High Definition RP package removal.
...
- Scripting: Fixed a freeze that occurred when using `InstantiateAsync` with very large objects.
    (UUM-95656)
...
- Scripting: Fixed an issue where Script dependencies were lost in Player builds when there is DefineConstrains on an Asmdef using ExtraDefines.
...
- Scripting: Fixed an issue where `AsyncInstantiate` continued unfinished operations after exiting Play mode.
...
- Search: Fixed an issue where searching for GameObjects with inherited components did not work correctly when filtering by base types.
...
- Serialization: Fixed an issue where DataEquals of SerializedProperty returned false in some cases where refReferenceId and otherReferenceId had a garbage value that made the refManagedReferenceTypeTree and otherManagedReferenceTypeTree null and corrupted the serialized data.
...
- Serialization: Fixed an issue where the UniversalRenderPipelineGlobalSettings.asset file created unnecessary changes in the Git diff during Player builds.
...
- Serialization: Remove forced upgrade prompt for Fixed Timestep. Binary serialized time manager settings are now properly migrated.
    ([UUM-91212](https://issuetracker.unity3d.com/issues/fixed-timestep-setting-gets-resets-to-default-when-upgrading-project))
...
- Shadergraph: Enabled custom interpolators for custom sprite lit targets. The shader's *Sprite Custom Lit* material mode now changes the sprite to the desired color instead of remaining black.
...
- Shadergraph: Fixed  sticky note context menu shortcut display text.
...
- Shadergraph: Fixed a null reference exception that occurred when the Shader Graph Editor was open while entering Play mode.
...
- Shadergraph: Fixed an issue by adding SHADERGRAPH_PREVIEW_MAIN define specifically for main previews.
...
- Shadergraph: Fixed an issue by exposing aniso setting on blackboard sampler state properties.
    ([UUM-90178](https://issuetracker.unity3d.com/issues/samplerstate-property-missing-anisotropic-filtering))

- Shadergraph: Fixed an issue that a shader graph is reverted to its last saved state when entering Play Mode without saving changes.
...
- Shadergraph: Fixed an issue where copy or pasting a property from a Graph to a Subgraph would make it display Graph only settings.
    (UUM-91040)
...
- Shadergraph: Fixed an issue where graphs containing groups were unintentionally modified when opening the Shader Graph Editor.
    ([UUM-76080](https://issuetracker.unity3d.com/issues/shader-graph-node-reference-samples-become-dirty-on-opening))

- Shadergraph: Fixed an issue where horizontal scrollbars was not working as expected in the Blackboard or Graph Inspector.
...
- Shadergraph: Fixed an issue where latin alphabet for variable names were not allowed.
...
- Shadergraph: Fixed an issue where some resources were not properly disposed of when entering Play mode in the Editor.
...
- Shadergraph: Fixed an issue where the Blackboard did not scroll to display newly created properties.
...
- Shadergraph: Fixed an issue where the property sheet displayed an incorrect type mismatch error for preview properties.
...
- Shadergraph: Fixed an issue where using HDR colors with custom Render Texture targets caused errors.
...
- Shadergraph: Fixed Color properties now default to opaque \(alpha = 1\).
    ([UUM-93088](https://issuetracker.unity3d.com/issues/default-value-of-shadergraph-color-property-is-0-by-default))

- Shadergraph: Fixed Forum link in info replaced with link to unity discussions.
...
- Shadergraph: Paste option in dropdown menus is now disabled when there is nothing valid to paste.
...
- Shadergraph: Updated Shader Graph to mark a shader graph dirty when toggling checkboxes in the Graph Settings.
    (UUM-83383)

- Shadergraph: Updated the Custom Function Node so that previews are hidden unless the first output is a previewable type, such as a vector or float.
...
- Shaders: Fixed an Editor crash that occurred during shader fallback resolution when importing a specific asset bundle.
...
- Shaders: Fixed an issue where bad shader code generated on metal sometimes when using CubeMapFaceID.
...
- Shaders: Fixed an issue where incorrect line numbers are reported in preprocessor errors after block comments.
    (UUM-87896)
...
- Shaders: Fixed shaders sp they now produce errors when they encounter the same keyword in different directives.
...
- Shaders: Fixed some instances where the Complex Lit base shader format would get interpreted as a different shader format.
    (UUM-89026)
...
- SRP Core: Fixed missing STP shaders and visual artifacts when targeting GLCore renderer.
    (UUM-84661)
...
- SRP Core: Fixed the serialization errors are thrown when "com.unity.render-pipelines.core" was added as a custom package.
...
- Terrain: Fixed a Terrain Detail objects were not rendering in the build when the Terrain is generated at runtime.
...
- Terrain: Fixed an issue in which grass details always used the default rendering layer mask.
...
- Terrain: Fixed an issue so that Terrain Trees with custom Tree Height and any materials without a "_TreeInstanceScale" property return to their correct height when zoomed in. Prior to this fix, trees materials without _TreeInstanceScale would always render at scale 1, even if the tree instance had height data.
...
- Terrain: Fixed terrain billboard grass rendering when dynamic batching was enabled in the URP asset.
...
- Terrain: Fixed terrain detail rendering issues when the render layer feature was enabled under DirectX 11 \(DX11\).
...
- Text: Ensure default editor fontAssets are properly initialized.
    (UUM-98848)

- Text: Ensure the SamplingPoint size for color font is properlyset.
    (UUM-71284)
...
- Text: Fixed an issue to ensure to reset focus when done editing.
...
- Text: Fixed an unstable tooltip of TextElement due to invalid usage of textInfo in IsElided.
    (UUM-81536)
...
- Text: Marked the color selection API as obsolete.
...
- Text: Replaced the HashSet initializer list with Array initializer list.
    (UUM-100735)
...
- UI: Fixed an issue by changing names for Windows architecture to differentiate between 64 and 32-bit.
    (UUM-87932)

- UI Elements: Fixed an issue by preventing the rename and create template option in the context menu when multiple elements are selected within UIBuilder hierarchy.
...
- UI Elements: Fixed an issue by setting character limit on UIBuilder library search field.
    ([UUM-76898](https://issuetracker.unity3d.com/issues/search-field-in-ui-builders-library-has-no-character-limit))

- UI Elements: Fixed an issue by updating the pointer counter logic for collection views.
...
- UI Elements: Fixed an undo order issue in UIBuilder.
...
- UI Elements: Fixed missing Undo support for rename operations in the UI Builder hierarchy.
    ([UUM-76676](https://issuetracker.unity3d.com/issues/renaming-control-in-ui-builder-hierarchy-does-not-add-to-undo-history))

- UI Elements: Fixed the indentation for decorator drawers.
...
- UI Toolkit: Fixed a memory leak that occurred when minimizing and maximising UI Builder.
...
- UI Toolkit: Fixed a NullReferenceException error that occurred when selecting a Script element in the UI Builder Hierarchy by addressing issues in the default attribute field factory in the UI Builder.
...
- UI Toolkit: Fixed a rendering bug that caused some fields and buttons in the UI Builder's Inspector became corrupted and invisible, due to reading `worldClip` in `GeometryChangeEvent`.
...
- UI Toolkit: Fixed a `NullReferenceException` that occurred when assigning a new instance of `PanelSettings` to a `UIDocument`.
...
- UI Toolkit: Fixed an issue in UI Builder where reported a circular dependency when the reference document was removed.
...
- UI Toolkit: Fixed an issue that displayed the "Import of asset setup artifact dependency to but dependency isn't used and therefore not registered in the asset database" warning when importing UXML.
...
- UI Toolkit: Fixed an issue where a BaseField's tooltip woult not appear when hovering its label, unless the BaseField has a tooltip set on its label directly.
    (UUM-68606)
...
- UI Toolkit: Fixed an issue where a layout in GenericDropwdownMenu could cause "Layout update is struggling" error messages.
...
- UI Toolkit: Fixed an issue where attribute overrides were not applied to direct children of a template instance.
...
- UI Toolkit: Fixed an issue where click would not work after an element was removed from its panel without releasing its pointer capture.
...
- UI Toolkit: Fixed an issue where docking multiple tabs together in the Editor with touch devices was not working.
...
- UI Toolkit: Fixed an issue where dropdown menu positions appeared in the wrong place.
...
- UI Toolkit: Fixed an issue where the batch state was not applied at the beginning of each command list when rendering world-space UI.
    (UUM-93882)

- UI Toolkit: Fixed an issue where the Dynamic Atlas Viewer did not update correctly when reopening the UI Toolkit Debugger window.
...
- UI Toolkit: Fixed an issue where the EditorWindow creator wizard didn't use the same xmlns prefix as the UI Builder.
...
- UI Toolkit: Fixed an issue where the root visual element could cause commands to be created for it when using world-space render mode. The commands must be stored in a default command list.
    (UUM-91743)
...
- UI Toolkit: Fixed an issue where transitions using `transition-property: all;` were not canceled in certain cases.
...
- UI Toolkit: Fixed an issue with elements being dragged and dropped after opening the context menu in the UI Builder hierarchy view.
    (UUM-69261)
...
- UI Toolkit: Fixed an isue by shanging editor PopupField style to match IMGUI and have the same height as other fields.
...
- UI Toolkit: Fixed an `IndexOutOfRangeException` that occurred when performing an Undo operation after moving an element in the UI Builder.
...
- UI Toolkit: Fixed exceptions that would occur when displaying a non-serializable field in the UI Builder inspector. A warning will now display.
    (UUM-90113)
...
- UI Toolkit: Fixed ScrollView mouse wheel scrolling so it now takes into account the dimensions of other elements in the scroll view hierarchy.
...
- UI Toolkit: Fixed the EndLayoutGroup error sometimes displaying in the console when using the UI Toolkit Live Reload option.
...
- UI Toolkit: Fixed UsageHints.DynamicColor did not prevent a re-tessellation when the color had an initial alpha of 0 or if the background tint was white.
...
- UI Toolkit: Fxed an issue where UI Toolkit did not support tiled slicing.
    ([UUM-92721](https://issuetracker.unity3d.com/issues/background-repeat-breaks-when-using-sprite-slicing-in-uitk))

- UI Toolkit: Hide the renderer and shader of UIDocument in Inspector when rendering in worldspace.
...
- UI Toolkit: Modified asset paths so they can now contain the guid only part. It should take the form "?guid=&lt;guid&gt;".<br>
    For example:<br>
...
- Undo System: Changed mark array size to keep duplicates within a single undo action.
    (UUM-84565)

- Undo System: Fixed a crash that occurred when events were added to the undo stack during the initial project load sequence, causing the undo stack to become desynchronized.
...
- Undo System: Fixed a crash that occurred when rapidly undoing changes to DrivenRectTransform components.
...
- Undo System: Remove undo groups which contain no actions automatically.
    (UUM-88038)
...
- Universal Windows Platform: Fixed an issue with Universal Windows Platform \(UWP\) project build when Visual Studio version is set to Latest Release.
...
- URP: Rendering Debugger - Material -Rendering Layers. Adding runtime support.
    (UUM-96354)
...
- VFX Graph: Fixed an issue that caused an unexpected compilation warning.
    (UUM-92778)
...
- VFX Graph: Fixed an issue where calling SetTexture missed setting something dirty.
...
- VFX Graph: Fixed an issue where CustomRenderTexture could not be used in VFX Graph object fields.
    (UUM-85711)
...
- VFX Graph: Fixed an issue where the VFX Graph template window appeared empty when the Terrain Tools package was installed.
    ([UUM-95871](https://issuetracker.unity3d.com/issues/vfx-template-window-is-empty-because-of-an-exception))

- VFX Graph: Fixed an issue with Undo and Redo while changing ShaderGraph reference.
...
- VFX Graph: Fixed errors that occurred when deleting Integration Update Rotation and Trigger blocks in the VFX Graph.
...
- VFX Graph: Fixed exception when converting a selection that contained an empty group to a subgraph.
...
- VFX Graph: Fixed issues with selection undo and redo functionality in VFX Graph.
...
- Video: Fixed an issue where a *AVFoundationVideoMedia error while reading* error was thrown when streaming from a URL with a signed token in the VideoPlayer.
...
- Video: Fixed on Windows to allow an arbitrary amount of webcam \(virtual and/or physical\) devices to be used.
...
- Video: Fixed the output of stale frames by the Video Player after a seek.
...
- VisionOS: Fixed an issue by adding GCEventInteraction support to allow for game controller to work with visionOS UI as well as game content.
    (UUM-87703)
...
- Web: Fixed an is where RangeError: Array buffer allocation failed was thrown when running URP sample on Chrome with compression.
...
- Web: Fixed an issue by correcting the name of the background image file in the output directory when using "Name Files as Hashes".
    ([UUM-96178](https://issuetracker.unity3d.com/issues/background-image-name-is-not-being-hashed-when-using-a-webgl-build-with-the-hashed-names-option))
...
- Web: Fixed freeze on startup when decompression fallback is enabled and multithreading was disabled.
...
- WebGL: Fixed an issue where `PlayDelayed()` played audio immediately in WebGL builds, ignoring the specified delay.
...
- Windows: Fixed a crash in the Windows player when embedded into an executable that does not export the `D3D12SDKPath` symbol.
...
- Windows: Fixed an issue where the window lost focus during game startup.
...
- Windows: Fixed an issue with monitor handle association to display details, ensuring correct behavior for all resolutions.
...
- Windows: Fixed an issue with the Video Player that occurred when reading video files with a long path prefix.
    (UUM-78606)
...
#### Final 6000.1.0f1 Package changes

#### Packages updated

- com.unity.sentis: [2.1.0](https://docs.unity3d.com/Packages/com.unity.sentis@2.1//changelog/CHANGELOG.html) to [2.1.2](https://docs.unity3d.com/Packages/com.unity.sentis@2.1//changelog/CHANGELOG.html)
...
- com.unity.2d.animation: [10.1.3](https://docs.unity3d.com/Packages/com.unity.2d.animation@10.1//changelog/CHANGELOG.html) to [10.2.0](https://docs.unity3d.com/Packages/com.unity.2d.animation@10.2//changelog/CHANGELOG.html)

- com.unity.2d.common: [9.0.6](https://docs.unity3d.com/Packages/com.unity.2d.common@9.0//changelog/CHANGELOG.html) to [9.1.0](https://docs.unity3d.com/Packages/com.unity.2d.common@9.1//changelog/CHANGELOG.html)
...
- com.unity.2d.psdimporter: [9.0.3](https://docs.unity3d.com/Packages/com.unity.2d.psdimporter@9.0//changelog/CHANGELOG.html) to [10.1.0](https://docs.unity3d.com/Packages/com.unity.2d.psdimporter@10.1//changelog/CHANGELOG.html)

- com.unity.2d.spriteshape: [10.0.6](https://docs.unity3d.com/Packages/com.unity.2d.spriteshape@10.0//changelog/CHANGELOG.html) to [10.1.0](https://docs.unity3d.com/Packages/com.unity.2d.spriteshape@10.1//changelog/CHANGELOG.html)

- com.unity.2d.tilemap.extras: [4.0.2](https://docs.unity3d.com/Packages/com.unity.2d.tilemap.extras@4.0//changelog/CHANGELOG.html) to [4.3.0](https://docs.unity3d.com/Packages/com.unity.2d.tilemap.extras@4.3//changelog/CHANGELOG.html)

- com.unity.2d.aseprite: [1.1.6](https://docs.unity3d.com/Packages/com.unity.2d.aseprite@1.1//changelog/CHANGELOG.html) to [1.2.4](https://docs.unity3d.com/Packages/com.unity.2d.aseprite@1.2//changelog/CHANGELOG.html)

- com.unity.adaptiveperformance: [5.1.0](https://docs.unity3d.com/Packages/com.unity.adaptiveperformance@5.1//changelog/CHANGELOG.html) to [5.1.3](https://docs.unity3d.com/Packages/com.unity.adaptiveperformance@5.1//changelog/CHANGELOG.html)
...
- com.unity.addressables: [2.2.2](https://docs.unity3d.com/Packages/com.unity.addressables@2.2//changelog/CHANGELOG.html) to [2.4.3](https://docs.unity3d.com/Packages/com.unity.addressables@2.4//changelog/CHANGELOG.html)
...
- com.unity.collab-proxy: [2.5.2](https://docs.unity3d.com/Packages/com.unity.collab-proxy@2.5//changelog/CHANGELOG.html) to [2.7.1](https://docs.unity3d.com/Packages/com.unity.collab-proxy@2.7//changelog/CHANGELOG.html)

- com.unity.ide.rider: [3.0.31](https://docs.unity3d.com/Packages/com.unity.ide.rider@3.0//changelog/CHANGELOG.html) to [3.0.36](https://docs.unity3d.com/Packages/com.unity.ide.rider@3.0//changelog/CHANGELOG.html)
...
- com.unity.inputsystem: [1.11.1](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.11//changelog/CHANGELOG.html) to [1.14.0](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.14//changelog/CHANGELOG.html)
...
- com.unity.mobile.android-logcat: [1.4.3](https://docs.unity3d.com/Packages/com.unity.mobile.android-logcat@1.4//changelog/CHANGELOG.html) to [1.4.5](https://docs.unity3d.com/Packages/com.unity.mobile.android-logcat@1.4//changelog/CHANGELOG.html)
...
- com.unity.render-pipelines.core: [17.0.3](https://docs.unity3d.com/Packages/com.unity.render-pipelines.core@17.0//changelog/CHANGELOG.html) to [17.1.0](https://docs.unity3d.com/Packages/com.unity.render-pipelines.core@17.1//changelog/CHANGELOG.html)

- com.unity.render-pipelines.high-definition: [17.0.3](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.0//changelog/CHANGELOG.html) to [17.1.0](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.1//changelog/CHANGELOG.html)

- com.unity.render-pipelines.high-definition-config: [17.0.3](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition-config@17.0//changelog/CHANGELOG.html) to [17.1.0](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition-config@17.1//changelog/CHANGELOG.html)

- com.unity.render-pipelines.universal: [17.0.3](https://docs.unity3d.com/Packages/com.unity.render-pipelines.universal@17.0//changelog/CHANGELOG.html) to [17.1.0](https://docs.unity3d.com/Packages/com.unity.render-pipelines.universal@17.1//changelog/CHANGELOG.html)
...
- com.unity.services.authentication: [3.3.3](https://docs.unity3d.com/Packages/com.unity.services.authentication@3.3//changelog/CHANGELOG.html) to [3.4.1](https://docs.unity3d.com/Packages/com.unity.services.authentication@3.4//changelog/CHANGELOG.html)
...
- com.unity.shadergraph: [17.0.3](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.0//changelog/CHANGELOG.html) to [17.1.0](https://docs.unity3d.com/Packages/com.unity.shadergraph@17.1//changelog/CHANGELOG.html)

- com.unity.splines: [2.6.1](https://docs.unity3d.com/Packages/com.unity.splines@2.6//changelog/CHANGELOG.html) to [2.8.0](https://docs.unity3d.com/Packages/com.unity.splines@2.8//changelog/CHANGELOG.html)
...
- com.unity.visualeffectgraph: [17.0.3](https://docs.unity3d.com/Packages/com.unity.visualeffectgraph@17.0//changelog/CHANGELOG.html) to [17.1.0](https://docs.unity3d.com/Packages/com.unity.visualeffectgraph@17.1//changelog/CHANGELOG.html)

- com.unity.visualscripting: [1.9.4](https://docs.unity3d.com/Packages/com.unity.visualscripting@1.9//changelog/CHANGELOG.html) to [1.9.6](https://docs.unity3d.com/Packages/com.unity.visualscripting@1.9//changelog/CHANGELOG.html)

- com.unity.xr.arcore: [6.0.3](https://docs.unity3d.com/Packages/com.unity.xr.arcore@6.0//changelog/CHANGELOG.html) to [6.1.0](https://docs.unity3d.com/Packages/com.unity.xr.arcore@6.1//changelog/CHANGELOG.html)

- com.unity.xr.arfoundation: [6.0.3](https://docs.unity3d.com/Packages/com.unity.xr.arfoundation@6.0//changelog/CHANGELOG.html) to [6.1.0](https://docs.unity3d.com/Packages/com.unity.xr.arfoundation@6.1//changelog/CHANGELOG.html)

- com.unity.xr.arkit: [6.0.3](https://docs.unity3d.com/Packages/com.unity.xr.arkit@6.0//changelog/CHANGELOG.html) to [6.1.0](https://docs.unity3d.com/Packages/com.unity.xr.arkit@6.1//changelog/CHANGELOG.html)

- com.unity.xr.core-utils: [2.3.0](https://docs.unity3d.com/Packages/com.unity.xr.core-utils@2.3//changelog/CHANGELOG.html) to [2.5.2](https://docs.unity3d.com/Packages/com.unity.xr.core-utils@2.5//changelog/CHANGELOG.html)

- com.unity.xr.interaction.toolkit: [3.0.5](https://docs.unity3d.com/Packages/com.unity.xr.interaction.toolkit@3.0//changelog/CHANGELOG.html) to [3.1.1](https://docs.unity3d.com/Packages/com.unity.xr.interaction.toolkit@3.1//changelog/CHANGELOG.html)
...
- com.unity.xr.meta-openxr: [2.0.1](https://docs.unity3d.com/Packages/com.unity.xr.meta-openxr@2.0//changelog/CHANGELOG.html) to [2.1.0](https://docs.unity3d.com/Packages/com.unity.xr.meta-openxr@2.1//changelog/CHANGELOG.html)

- com.unity.xr.oculus: [4.3.0](https://docs.unity3d.com/Packages/com.unity.xr.oculus@4.3//changelog/CHANGELOG.html) to [4.5.1](https://docs.unity3d.com/Packages/com.unity.xr.oculus@4.5//changelog/CHANGELOG.html)

- com.unity.xr.openxr: [1.12.1](https://docs.unity3d.com/Packages/com.unity.xr.openxr@1.12//changelog/CHANGELOG.html) to [1.14.2](https://docs.unity3d.com/Packages/com.unity.xr.openxr@1.14//changelog/CHANGELOG.html)

- com.unity.learn.iet-framework: [4.0.2](https://docs.unity3d.com/Packages/com.unity.learn.iet-framework@4.0//changelog/CHANGELOG.html) to [4.1.3](https://docs.unity3d.com/Packages/com.unity.learn.iet-framework@4.1//changelog/CHANGELOG.html)

- com.unity.learn.iet-framework.authoring: [1.2.2](https://docs.unity3d.com/Packages/com.unity.learn.iet-framework.authoring@1.2//changelog/CHANGELOG.html) to [1.3.2](https://docs.unity3d.com/Packages/com.unity.learn.iet-framework.authoring@1.3//changelog/CHANGELOG.html)
...
- com.unity.multiplayer.playmode: [1.3.0](https://docs.unity3d.com/Packages/com.unity.multiplayer.playmode@1.3//changelog/CHANGELOG.html) to [1.4.2](https://docs.unity3d.com/Packages/com.unity.multiplayer.playmode@1.4//changelog/CHANGELOG.html)

- com.unity.transport: [2.3.0](https://docs.unity3d.com/Packages/com.unity.transport@2.3//changelog/CHANGELOG.html) to [2.5.1](https://docs.unity3d.com/Packages/com.unity.transport@2.5//changelog/CHANGELOG.html)

- com.unity.dedicated-server: [1.3.0](https://docs.unity3d.com/Packages/com.unity.dedicated-server@1.3//changelog/CHANGELOG.html) to [1.4.2](https://docs.unity3d.com/Packages/com.unity.dedicated-server@1.4//changelog/CHANGELOG.html)
...
- com.unity.ai.navigation: [2.0.4](https://docs.unity3d.com/Packages/com.unity.ai.navigation@2.0//changelog/CHANGELOG.html) to [2.0.7](https://docs.unity3d.com/Packages/com.unity.ai.navigation@2.0//changelog/CHANGELOG.html)
...
- com.unity.services.tooling: [1.1.0](https://docs.unity3d.com/Packages/com.unity.services.tooling@1.1//changelog/CHANGELOG.html) to [1.3.0](https://docs.unity3d.com/Packages/com.unity.services.tooling@1.3//changelog/CHANGELOG.html)
...
- [com.unity.project-auditor@1.0.1](https://docs.unity3d.com/Packages/com.unity.project-auditor@1.0//changelog/CHANGELOG.html)

- [com.unity.web.stripping-tool@1.0.0](https://docs.unity3d.com/Packages/com.unity.web.stripping-tool@1.0//changelog/CHANGELOG.html)

- [com.unity.meta-instant-games-sdk@1.0.0](https://docs.unity3d.com/Packages/com.unity.meta-instant-games-sdk@1.0//changelog/CHANGELOG.html)
