...
*   2D: Sprite packer causes Out of memory error [(897066)](https://issuetracker.unity3d.com/issues/sprite-packer-causes-out-of-memory-error)
...
*   Animation: Fixed animation layering when WriteDefault is set to off. [(907400 909361)](https://issuetracker.unity3d.com/issues/animator-scale-animation-affects-additive-layer-without-in-slash-out-transition)
*   Editor: Fix UI Component Icons [(909700)](https://issuetracker.unity3d.com/issues/ui-script-icons-are-default-script-icons)
*   Graphics: Fixed RenderToCubemap when using a HDR enabled Camera. This fixes prevents error message "Releasing render texture whose render buffer is set as Camera's target buffer with Camera.SetTargetBuffers!". [(902409)](https://issuetracker.unity3d.com/issues/rendertocubemap-is-not-hdr-compatible-and-causes-camera-not-to-be-rendering-in-the-scene-anymore)
*   Linux: Fix uninitialized screen dimensions at startup with some window managers [(880426)](https://issuetracker.unity3d.com/issues/linux-input-dot-mouseposition-returns-infinity-or-nan-in-fullscreen-mode)
...
*   VR: Enable termination of Cardboard applicaiton when pressing the <- button during the splash screen. (911205)
...
*   VR: Fixed VR does not render if a renderTexture whose format is set to RGB565 is created, crash is possible (912063)

2017.1.0b8 Release Notes (Full)
-------------------------------
