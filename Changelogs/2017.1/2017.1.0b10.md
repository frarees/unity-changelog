...
*   Android: Fix H264 videos don't loop on Android 4.1 and 4.2 devices [(893527)](https://issuetracker.unity3d.com/issues/videoplayer-h264-videos-dont-loop-on-android-4-dot-1-and-4-dot-2-devices)
...
*   Editor: Fixed "too many GUIClip pushed" error on ExitGUIException [(912350)](https://issuetracker.unity3d.com/issues/when-undoing-the-creation-of-new-asset-error-appears-gui-error-you-are-pushing-more-guiclips-than-you-are-popping)
*   Editor: Fixed exceptions thrown when changing GameView settings on a different display [(909648)](https://issuetracker.unity3d.com/issues/exceptions-are-thrown-when-clicking-on-aspect-ration-options-for-display-2-8)
...
*   Editor: Fixed lights attached to the scene view camera not lighting the scene when lights are toggled off. [(909992)](https://issuetracker.unity3d.com/issues/light-attached-to-scene-view-camera-does-not-light-up-the-scene-if-lights-are-toggled-off)
*   Editor: Restore SRGB write state after internal override [(901588)](https://issuetracker.unity3d.com/issues/scene-in-the-oculus-rift-headset-is-darker-compared-to-the-editor-when-single-pass-is-enabled)
...
*   Graphics: Fix for GetTransformInfoDoNotExpectUpToDate in renderer bounding volume updating [(899009)](https://issuetracker.unity3d.com/issues/in-order-to-call-gettransforminfoexpectuptodate-dot-dot-dot-error-message-appears-while-toggling-skinned-mesh-renderer-component)
*   Graphics: Fixed Assert in Editor and crash in Standalone when disabling/enabling SkinnedMeshRenderer in very specific situations eg from animation during an OnBecameVisible callback. [(901468)](https://issuetracker.unity3d.com/issues/standalone-build-crashes-in-skinnedmeshrenderer-rendererbecamevisible)
...
*   IL2CPP: Correct a possible error from the UnityLinker that can occur when a .pdb file already exists in the output directory [(915778)](https://issuetracker.unity3d.com/issues/il2cpp-ios-unitylinker-dot-exe-did-not-run-properly-error-when-building-with-code-stripping-enabled)
*   IL2CPP: Do not throw a managed exception when the maximum recursion depth is reached during metadata initialization [(905659)](https://issuetracker.unity3d.com/issues/android-il2cpp-notsupportedexception-in-static-constructor-when-scene-finishes-loading)
*   IL2CPP: Fixed an issue which caused IL2CPP to fail to convert assemblies if two non-existing methods with the same declaring type were referenced from any assemblies [(917343, 917348)](https://issuetracker.unity3d.com/issues/il2cpp-crashes-when-two-non-existing-methods-with-same-declaring-types-and-parameter-counts-are-referenced-from-a-dll)
*   iOS: Fixed corrupted orientation view when forcing interface orientation via script (using Screen.orientation) on iOS10 [(835745)](https://issuetracker.unity3d.com/issues/ios-changing-the-screen-orientation-via-a-script-sometimes-results-in-corrupted-view-on-ios-10)
*   Particles: Fix case where velocity was ignoring shape transform [(908252)](https://issuetracker.unity3d.com/issues/particles-do-not-fill-cone-shape-correctly-when-cone-scale-is-changed)
*   Particles: Fixed another crash on Android hardware caused by invalid particle data (912280)
*   Particles: Restore "Show Selected" preview mode [(901440)](https://issuetracker.unity3d.com/issues/all-particle-systems-from-the-same-particle-effect-are-played-regardless-of-the-show-selected-setting-in-the-particle-editor)
*   Physics: Fix an issue where ContactFilter2D collision normal limits failed. [(912158)](https://issuetracker.unity3d.com/issues/osx-contactfilter2d-dot-usenormalangle-does-not-respond-to-collisions)
*   Physics: Fix an issue with Raycast not always correctly detecting BoxCollider2D or CapsuleCollider2D. [(917323)](https://issuetracker.unity3d.com/issues/raycast-doesnt-detect-collision-with-a-box-collider-2d-that-has-edge-radius-greater-than-0)
*   Physics: Fixed case of CharacterController starting to ignore collisions with certain objects after a number of scene reloads. [(809914)](https://issuetracker.unity3d.com/issues/charactercontrollers-consistently-ignore-collisions-with-random-colliders)
*   Scripting: Fix APIUpdater crash when checking C# scritps with deep object initialization syntax. (902973)
*   Scripting: Fix assembly corruption in assemblies containing references to UnityEditor.Animations.AvatarMask. [(904030)](https://issuetracker.unity3d.com/issues/dll-breaks-after-upgrading-to-5-dot-6-and-up)
*   Scripting: Fixed compatibility define. UNITY\_64 is now defined again in managed code in addition to the new PLATFORM\_ARCH\_64 macro. [(904483)](https://issuetracker.unity3d.com/issues/nvidia-ansel-build-failed-to-load-a-dll-at-runtime)
...
*   Timeline: Copy & Pasting Track from Timeline A to Timeline B results in Track being duplicated in Timeline A [(914607)](https://issuetracker.unity3d.com/issues/timeline-copy-and-pasting-track-from-timeline-a-to-timeline-b-results-in-track-being-duplicated-in-timeline-a)
...
*   Universal Windows Platform: Fixed a crash which happened when retrieving WheelHit.Collider after having previously set it to null on .NET scripting backend [(871541)](https://issuetracker.unity3d.com/issues/wsa-system-dot-executionengineexception-is-thrown-when-running-the-wsa-build)
*   Universal Windows Platform: Fixed a potential stack overflow when building a project with a large custom cursor [(909977)](https://issuetracker.unity3d.com/issues/uwp-editor-crashes-when-building-the-attached-project-which-uses-custom-cursor)
*   Universal Windows Platform: Fixed exceptions thrown in OnGUI callback not being logged on .NET scripting backend [(914797)](https://issuetracker.unity3d.com/issues/wsa-application-dot-logmessagereceived-is-not-calling-a-subscribed-event-on-windows-store-build)
*   Universal Windows Platform: Fixed IL2CPP not being able to build C++ code with VS 2017 Update 2 [(916281)](https://issuetracker.unity3d.com/issues/il2cpp-cannot-build-c-plus-plus-code-with-vs2017-update-2)
*   Universal Windows Platform: Fixed System.Numerics.dll not being referenced by scripts when using IL2CPP scripting backend [(912074)](https://issuetracker.unity3d.com/issues/system-dot-numerics-dot-dll-is-not-referenced-on-il2cpp-scripting-backend)

2017.1.0b10 Release Notes (Full)
--------------------------------
