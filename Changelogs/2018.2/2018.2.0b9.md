2018.2.0b9 Release Notes (diff since 2018.2.0b8)
------------------------------------------------
...
*   XR: Updated Vuforia to version 7.1.35

### Fixes

*   2D: Fixed sprite indexing is rearranged in Atlas when switching platforms while having platform-dependent settings. ([1019559](https://issuetracker.unity3d.com/issues/2d-sprite-indexing-is-rearranged-in-atlas-when-switching-platforms-while-having-platform-dependent-settings))
    
*   Android: Fix for graphic corruption when screen resolution is changed ([785610](https://issuetracker.unity3d.com/issues/android-when-screen-resolution-is-changed-there-is-a-frame-with-graphical-corruption), 1046739)
    
*   Android: Fixed a crash when upgrading an existing application on Android to 64 bit (1047060)
    
*   Android: Fixed VideoPlayer not working on Intel Baytrail ([973427](https://issuetracker.unity3d.com/issues/asus-k011-does-not-show-a-streamingasset-video), 1047072)
    
*   Animation: Fix crash when invalid component type is used to create PropertySceneHandle/PropertyStreamHandle (1032369)
    
*   Animation: Fix crash when trying to reparent a transform bound to a TransformSceneHandle ([1035803](https://issuetracker.unity3d.com/issues/full-body-ik-c-number-jobs-unity-crash-when-trying-to-re-parent-hinge-effector-to-hand-effectors-using-the-animation-c-number-jobs-example))
    
*   Animation: Fixed GetPosition/SetPosition/GetRotation/SetRotation on TransformStreamHandle not returning global space for generic rig with root motion apply
    
*   Animation: Fixed memory leak for animated int (1041233)
    
*   Animation: Fixed memory leak for CloseAnimationStream when no playable is bound to animator (1041237)
    
*   Build Pipeline: Fix for empty "plaftormName" property when serializing BuildReport using EditorJsonUtility on macOS. ([1034097](https://issuetracker.unity3d.com/issues/buildpipeline-buildreport-platformname-property-is-not-set-upon-build-on-macos))
    
*   Build Pipeline: \[BuildPipeline\] Fix for incorrect first valid standalone target index, which is greater than StandaloneOSX index ([1034097](https://issuetracker.unity3d.com/issues/buildpipeline-buildreport-platformname-property-is-not-set-upon-build-on-macos))
    
*   Editor: Fix enum field not stopping event propagation when showing menu ([1033150](https://issuetracker.unity3d.com/issues/blackboard-follows-mouse-after-changing-mode), 1037732)
    
*   Editor: Fix GameObject created in hierarchy when inside an animator blend tree node. ([1043944](https://issuetracker.unity3d.com/issues/entering-a-blendtree-node-creates-defaultavatar-clone-animatorpreview-go-in-the-hierarchy), 1046176)
    
*   Editor: License activation fails when running Unity with command line arguments ([1041582](https://issuetracker.unity3d.com/issues/license-activation-fails-when-running-unity-with-command-line-arguments), 1043475)
    
*   Editor: Reduced enter play mode and recompile time by optimizing EditorCompilationInterface.GetTargetAssembly and EditorCompilationInterface.GetTargetAssembliesWithScripts. Has a high impact on projects with lots of scripts and lots of .asmdef files (1032929)
    
*   Graphics: ComputeShader.Dispatch() will now throw an error if the texture dimensions do not match with the dimensions required by the shader kernel
    
*   Graphics: Fixed crashes when loading RenderDoc onto a running Unity editor instance
    
*   Graphics: Instancing: Fixed corner cases in shader code generation on GL and Vulkan
    
*   Graphics: Metal: Fix compute buffers having garbled debug labels occasionally
    
*   Graphics: Metal: Resource binding handling optimizations
    
*   Graphics: OpenGL: Fixed Graphics.DrawMeshInstancedIndirect not supporting 32-bit indices
    
*   Graphics: Rendering.CommandBuffer: Fixed using GetTemporaryRT() results as RW Textures (UAVs) ([1033216](https://issuetracker.unity3d.com/issues/srp-enablerandomwrite-does-not-affect-texture-created-with-commandbuffer))
    
*   Graphics: RenderTexture.Create() will now throw an error if the user attempts to create a 3D texture with depth bits != 0.
    
*   Graphics: Vulkan: Enable render-to-3D Texture where supported by the implementation
    
*   Graphics: Vulkan: Fixed various validation layer warnings and minor issues
    
*   Graphics: Vulkan: Vulkan shader compilation was missing the SHADER\_STAGE\_ defines that other platforms have, fixed.
    
*   IL2CPP: Change the stack behavior of store-indirect instructions to fix unintended side effects caused by a previous optimization to prevent too many local variables. (1036866)
    
*   IL2CPP: Correct low level IL behavior when storing values to small local integer variables. This can affect the conversion between pointers and integers in unsafe contexts. ([1040777](https://issuetracker.unity3d.com/issues/error-il2cpp-dot-exe-did-not-run-properly-while-building-for-android))
    
*   IL2CPP: Fix an issue that occurs when constrained calls are made on pointer types in unsafe contexts. ([1043225](https://issuetracker.unity3d.com/issues/il2cpp-build-fails-with-system-dot-string-cthdata-dot-range-stringformint-system-dot-int32-attempting-to-constrain-an-invalid-type))
    
*   IL2CPP: Fix deadlock on exit when using OnAudioFilterRead with il2cpp scripting backend. ([1017515](https://issuetracker.unity3d.com/issues/onaudiofilterread-on-uwp-il2cpp-crashes-application-when-trying-to-application-dot-quit))
    
*   iOS: Fix C plugin support ([1022895](https://issuetracker.unity3d.com/issues/ios-xcode-compiler-errors-unknown-type-name-bool-in-unityinterface-dot-h), 1037715)
    
*   Package Manager: Fix AssetDatabase warnings in Console when creating a new project or upgrading an existing project
    
*   Scripting: Fix crash when loading some assemblies due to invalid metadata parsing ([1035692](https://issuetracker.unity3d.com/issues/crash-with-lots-of-plugins-during-script-compilation-after-an-error-in-one-of-the-scripts-has-been-fixed))
    
*   Universal Windows Platform: Add missing manifest capabilities to UWA project settings ([1024081](https://issuetracker.unity3d.com/issues/wsa-missing-wificontrol-in-capabilities-for-uwp))
    
*   Web: Fix UnityWebRequest POST failure with multipart sections ([1026339](https://issuetracker.unity3d.com/issues/generic-slash-unknown-http-error-appears-when-using-unitywebrequest-dot-post), 1037718)
    
*   WebGL: Fix default Data Caching value (enabled) ([1046290](https://issuetracker.unity3d.com/issues/webgl-data-caching-not-enabled-by-default))
    
*   WebGL: Fix default Linker Target value (WebAssembly)
    
*   WebGL: Fix LW Render Pipeline ([1017374](https://issuetracker.unity3d.com/issues/lwrp-template-scene-is-not-rendered-in-webgl-when-built-with-lightweight-rp-template))
    
*   Windows: Change papplication exe to use the product name instead of the folder name ([1024461](https://issuetracker.unity3d.com/issues/when-building-the-game-for-windows-the-exe-takes-the-file-name-of-the-folder-you-save-in-instead-of-the-given-product-name))
    
*   XR: Fix stripping MeshFilter and MeshRenderer classes if they are not used, causing the VR splash screen to crash on an empty project ([1027726](https://issuetracker.unity3d.com/issues/xr-ios-launching-an-ios-app-in-vr-mode-causes-the-app-to-crash))
    
*   XR: PressType on HoloLens now correctly reports Select instead of None. (1016406)
    
*   XR: Resolved issues with keyboard and mouse not working properly during Play in Editor for Windows Mixed Reality applications (954950)
    
*   XR: Resolved issues with User-Presence not working properly in the Editor for Windows Mixed Reality applications. (951118)
    

2018.2.0b9 Release Notes (Full)
-------------------------------
