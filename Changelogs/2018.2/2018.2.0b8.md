2018.2.0b8 Release Notes (diff since 2018.2.0b7)
------------------------------------------------

### Known Issues in 2018.2.0b8

*   Animation: Editor/Player can crash if you reparent a transform bound to a TransformSceneHandle ([1035803](https://issuetracker.unity3d.com/issues/full-body-ik-c-number-jobs-unity-crash-when-trying-to-re-parent-hinge-effector-to-hand-effectors-using-the-animation-c-number-jobs-example))
...
*   GI: Improve Reflection Probe culling performance

### Fixes

*   Android: Fix the issue that GI assets for the first scene have been added to the obb with "Split Application Binary" enabled. ([1037638](https://issuetracker.unity3d.com/issues/android-gi-assets-for-the-first-scene-level0-have-been-added-to-the-obb-when-split-application-binary-is-enabled))
    
*   Animation: Fixed use of AnimationCurve scripting API in threads. ([1041793](https://issuetracker.unity3d.com/issues/scripting-slash-animation-animationcurve-scripting-api-thread-checks-have-changed-after-bindings-upgrade))
    
*   GI: Allow for non-latin characters in the Unity editor install path. ([1038721](https://issuetracker.unity3d.com/issues/unity-crashes-when-project-is-loaded-if-editors-installation-path-contains-non-latin-characters))
    
*   GI: Fix crashes while baking and painting/removing terrain trees. ([1026701](https://issuetracker.unity3d.com/issues/plm-editor-crashes-while-baking-after-removing-terrain-trees-and-switching-ui-tabs-with-plm-set-as-baking-backend))
    
*   GI: Fix the output color for HDR lightmaps in Gamma Space. (1035068)
    
*   Graphics: Fix Editor and PS4 player crash when dispatching a ComputeShader with more than 16 textures bound (1029451)
    
*   Graphics: Fix JobTempAlloc warning message in SRP (1033301)
    
*   Graphics: Fixed crash when loading compressed skinned mesh. ([1041405](https://issuetracker.unity3d.com/issues/unity-2018-dot-2-crash-on-compressedmesh-decompress-when-loading-skinned-and-compressed-mesh))
    
*   Graphics: Fixed error when setting UVs or colors on a skinned mesh. (1031197)
    
*   IL2CPP: Fix exception when child of Text Mesh Pro object is detached ([1035793](https://issuetracker.unity3d.com/issues/uwp-exception-is-thrown-when-child-game-object-is-detached-from-parent))
    
*   Input: Properly return touch pressure on iOS devices which supports it in the new input system.
    
*   Input: Return pressure 1.0 for touches on iOS devices not supporting presurre in the new input system.
    
*   iOS: Paths for native source plugins are not properly added to xcode project when the files are in a package (1025944)
    
*   Package Manager: Editor would crash when canceling package installation ([1011686](https://issuetracker.unity3d.com/issues/packagemanager-editor-crashes-when-canceling-package-installation), 1036770, [1046387](https://issuetracker.unity3d.com/issues/backport-backport-packagemanager-editor-crashes-when-canceling-package-installation))
    
*   Package Manager: Fix Package Manager UI reporting an error when adding/removing a package (1030371)
    
*   Package Manager: Fix reveal in Finder/Explorer when Packages folder selected (1030996)
...
*   Physics: Fix cloth properties icons and tooltips ([1021611](https://issuetracker.unity3d.com/issues/cloth-buttons-in-the-inspector-have-incorrect-details), 1046052)
    
*   Scripting: Fix crash when accessing custom attribute data (1039434)
    
*   Scripting: Fix crash when MonoImporter.SetExecutionOrder is called from RuntimeInitializeOnLoadMethod ([1005502](https://issuetracker.unity3d.com/issues/unity-crashes-when-monoimporter-dot-setexecutionorder-is-called-from-runtimeinitializeonloadmethod))
    
*   Scripting: Raise exception when covariance is used on old scripting runtime ([1036673](https://issuetracker.unity3d.com/issues/dot-net-3-dot-x-editor-crashes-when-using-covariants))
    
*   Scripting: Resolve reference assemblies correctly from any folder in project ([957012](https://issuetracker.unity3d.com/issues/dot-dll-file-not-found-if-its-in-a-different-folder-that-the-referencing-dll-files), [996473](https://issuetracker.unity3d.com/issues/dll-dependencies-are-not-resolved-if-the-dlls-are-stored-in-seperate-folders))
    
*   Scripting Upgrade: Avoid accessing /dev/shm (which fails Mac App Store submission) (1035654)
    
*   Scripting Upgrade: Fix compilation errors in Visual Studio when targeting .NET Standard 2.0 ([1030459](https://issuetracker.unity3d.com/issues/reference-to-httpclient-statement-is-not-found-in-vs2017))
    
*   Scripting Upgrade: Fix ExecutionEngineException when using Property.GetValue ([1030427](https://issuetracker.unity3d.com/issues/uwp-executionengineexception-is-thrown-on-system-dot-reflection-dot-monoproperty-getteradapterframe))
    
*   Scripting Upgrade: WebGL: Correct an error that can occur when BestHTTP is used - "The type initializer for 'System.Globalization.CharUnicodeInfo' threw an exception." ([1034134](https://issuetracker.unity3d.com/issues/dot-net4-dot-5-webgl-build-cant-connect-to-server-with-system-dot-globalization-dot-charunicodeinfo-initialize-error))
    
*   Timeline: Copy/Paste now retains state of clips and track at copy time (969331)
    
*   Timeline: Fix audio playback on scaled control track clips ([1024165](https://issuetracker.unity3d.com/issues/audio-does-not-play-when-playing-in-a-timeline-which-is-controlled-by-another-timeline-with-a-scaled-control-clip))
    
*   Timeline: Fix copy/paste menu not correctly appearing when copying between Timelines (1030316)
    
*   Timeline: Fix Copy/Paste of multiple clips losing their relative position (946400)
    
*   Timeline: Fix for playhead not retaining position when switch timelines (1030358)
    
*   Timeline: Fix playable track curves on copy/paste being part of incorrect asset (1025464)
    
*   Timeline: Fix playback of timeline clips in animation window (1034748)
    
*   Timeline: Fix playhead jumping ahead when timeline is played in editor ([1032103](https://issuetracker.unity3d.com/issues/timeline-playhead-jumps-ahead-when-timeline-play-mode-is-enabled-after-importing-webvr-assets))
    
*   Timeline: Fix playhead right context menu not appearing correctly (1041731)
    
*   Timeline: Fix selection change when creating a new recording clip (1043952)
    
*   Timeline: Fix split command creating a 1 frame clip (1029628)
    
*   Timeline: Fix trim operation on empty animation track clips ([1040941](https://issuetracker.unity3d.com/issues/timeline-the-size-of-an-empty-animation-clip-in-the-timeline-doesnt-increase-when-trying-to-drag-its-left-border))
    
*   Universal Windows Platform: Fixed GI folders getting picked up as language resources by Visual Studio (976639)
    
*   Universal Windows Platform: Fixed subscribing to Windows Runtime events on IL2CPP scripting backend with .NET Standard 2.0 profile (1043563)
    
*   Video: Distorted audio on OSX. ([1002426](https://issuetracker.unity3d.com/issues/osx-videoplayer-audio-from-unityengine-dot-video-dot-videoplayer-is-distorted-when-audio-output-mode-is-direct), 1044009)
    
*   Video: Seeking backwards in VideoPlayer video takes considerably more time than seeking forwards ([950054](https://issuetracker.unity3d.com/issues/seeking-backwards-in-videoplayer-video-takes-considerably-more-time-than-seeking-forwards), 1044007)
    
*   Windows: Fixed "Failed to create decoder for ScreenSelector.png" in development builds when no screen selector is specified in player settings ([1019970](https://issuetracker.unity3d.com/issues/deployment-management-failed-to-create-decoder-exception-thrown-while-playing-development-build))
    
*   Windows: Fixed an exception in RecreateDisplayList when there are no monitors attached (1017288)
    
*   XR: A/B buttons
    

2018.2.0b8 Release Notes (Full)
-------------------------------
