2018.2.0b7 Release Notes (diff since 2018.2.0b6)
------------------------------------------------

### Known Issues in 2018.2.0b7
...
*   Editor: Optimised Version Control icon overlay drawing in the Project Window
    
*   Graphics: backport of case 1036664 ( https://ono.unity3d.com/unity/unity/pull-request/67880/\_/2018.2/SRP-Batcher/fixes )
    
*   Package Manager: - Show selected asset full path in project browser bottom bar (two column)
    
    *   Add new Assets -> Copy Path menu item

### Fixes

*   Asset Import: Added validation for invalid constraints in FBX files. ([1038630](https://issuetracker.unity3d.com/issues/editor-crashes-in-modelimporter-importconstraints-when-trying-to-import-constraints-from-animation))
    
*   Asset Import: Fixed random animation corruption when importing the animation multiple times. ([1005576](https://issuetracker.unity3d.com/issues/reimporting-prefab-with-optimal-animation-compression-settings-multiple-times-corrupts-animation))
    
*   Editor: Changed default physics layermask in LineRenderer create points mode to be Everything instead of Nothing
    
*   Editor: Changed LineRenderer editor so when the LineRenderer is disabled, the simplify preview is not drawn.
    
*   Editor: Disables icon filtering on integer-scaled monitor displays ([1025829](https://issuetracker.unity3d.com/issues/macos-play-slash-pause-buttons-have-a-small-white-outline-when-the-play-button-is-pressed))
    
*   Editor: Fixed a Scriptable Build Pipeline crash when building asset bundles without path entries in OSX
    
*   Editor: Fixed argument exception when selecting new points in a LineRenderer in Edit mode when the points list was collapsed. ([1034699](https://issuetracker.unity3d.com/issues/shuriken-argumentexception-is-thrown-on-console-when-editing-points-on-the-line-in-linerenderer-component))
    
*   Editor: Fixed bug where FPS navigation in LookDev view did not work when holding right mouse button
    
*   Editor: Fixed bug where Scene View FPS navigation stopped working after maximizing and un-maximizing Scene View (1036595)
    
*   Editor: Fixed several multiple monitor issues on windows high dpi setups ([1018591](https://issuetracker.unity3d.com/issues/hidpi-offset-with-the-colorpicker-when-it-and-the-editor-are-on-differents-monitors), 1040964)
    
*   Editor: Fixed text bluriness on Windows High-dpi with non-integer scale ratios. (1035448)
    
*   Editor: Test Runner: PrebuildSetupAttribute will find types in arbitrary assemblies, instead of a predefined list ([1033694](https://issuetracker.unity3d.com/issues/prebuildsetupattribute-cannot-find-types-in-arbitrary-assemblies))
    
*   Graphics: Fixed rare crash in SRP culling code.
    
*   Graphics: FIxed uninitialised variable in texture (mipmap) streaming manager (1044022)
    
*   Graphics: Various graphics bugs when new SRP Batcher is ON in SampleGame, Fontainebleau and BOTD demos (1036664)
    
*   Scripting: Fix compilation errors in Visual Studio when targeting .NET Standard 2.0 ([1030459](https://issuetracker.unity3d.com/issues/reference-to-httpclient-statement-is-not-found-in-vs2017))
    
*   Scripting: Fix Vector2/Vector3 Angle functions to not round value on nearly parallel vectors (1038707)
    
*   Scripting Upgrade: Fixes an issue where editor scripts could fail to recompile on windows.
    
*   Scripting Upgrade: Made API updater version control integration more robust to unexpected error (improved handling on Windows). ([995171](https://issuetracker.unity3d.com/issues/unauthorizedaccessexception-when-script-updater-tries-to-update-non-checked-out-file-causes-infinite-compilation-loop))
    
*   Timeline: Fixed a crash when creating a control clip with an empty timeline (1033891)
    
*   XR: Fix eye texture allocation on Oculus mobile platforms.
    

2018.2.0b7 Release Notes (Full)
-------------------------------
