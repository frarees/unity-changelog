2018.2.0b2 Release Notes (diff since 2018.2.0b1)
------------------------------------------------

### Known Issues in 2018.2.0b2
...
*   Editor: \[SpeedTree\] Applying SpeedTree changes, through popup which appears when playing the scene, crashes editor ([1022504](https://issuetracker.unity3d.com/issues/speedtree-applying-speedtree-changes-through-popup-which-appears-when-playing-the-scene-crashes-editor))
...
*   Timeline: Added an option to disable audio scrubbing in Timeline

### Fixes

*   Animation: Reverted keyframing of root motion in Timeline.
    
*   Asset Import: Bring back the option to import lightmaps with push pull dilation. The option can be turned on in scripts, by setting the TextureImporter.pushPullDilation = true;. ([1014724](https://issuetracker.unity3d.com/issues/lighting-differences-between-unity-5-dot-4-4-and-unity-2017-dot-2))
    
*   Asset Import: Fix a crash when importing framework plugins inside Assets/Plugins/iOS folder. ([1021172](https://issuetracker.unity3d.com/issues/windows-fileaccessor-read-crashes-when-importing-unity-ads), 1025453)
    
*   Editor: Exit after displaying fatal error notice on linux. ([1018887](https://issuetracker.unity3d.com/issues/cannot-close-the-popup-and-unity-editor-after-crashing-out-of-memory))
    
*   Editor: Fixed Project Browser search only showing results from current folder even when Assets search mode was enabled ([1024947](https://issuetracker.unity3d.com/issues/other-folder-contents-dont-appear-in-search-if-a-particular-folder-is-selected-in-projectview), 1027301)
    
*   Editor: Fixed regression where arrow keys no longer moved the scene view camera. ([1022564](https://issuetracker.unity3d.com/issues/shortcut-manager-scene-view-camerra-does-not-move-using-arrow-keys))
    
*   Editor: Make linux keycode reporting more consistent with other platforms
    
*   Editor: \[HiDPI\] Displaying a tooltip on any scale above 100% constantly repaint the window in which it's contained (1016434)
    
*   Graphics: Fix occasional crash when using marquee selection to select dynamic geometry such as Particles, Lines or Trails (1021655)
    
*   Graphics: Instancing LoDFade parameters now contain both quantized and non quantized value to match non-instanced version.
    
*   Graphics: Various Vulkan performance and correctness fixes
    
*   Timeline: Fix a memory leak of AnimationPlayableAssets when using Timeline with AssetBundles ([1001595](https://issuetracker.unity3d.com/issues/switching-between-animations-with-assetbundle-cause-animationplayableasset-remain-in-memory))
    
*   UI: Fixing issue with disabled canvas renderer changing parent canvas that is then deleted.
    
*   UI: Fixing issue with setting position followed by setting parent losing proper positioning. ([1021484](https://issuetracker.unity3d.com/issues/children-layout-breaks-when-changing-the-hierarchy-layer-and-the-transform-of-their-parent))
    

2018.2.0b2 Release Notes (Full)
-------------------------------
