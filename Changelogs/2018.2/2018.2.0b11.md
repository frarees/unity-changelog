2018.2.0b11 Release Notes (diff since 2018.2.0b10)
--------------------------------------------------

### Known Issues in 2018.2.0b11
...
*   Scripting: 2018.2 Performance regression in BehaviourManager::Update ([1046516](https://issuetracker.unity3d.com/issues/2018-dot-2-performance-regression-in-behaviourmanager-update), 1051447)
...
*   Editor: Profiler, Frame Debugger and Physics Debugger windows are now located in "Window/Analysis" instead of "Window/Debug", Test runner is located in General

### Improvements

*   Licenses: Added two parameters ('createManualActivationFile' and 'manualLicenseFile') to allow activation of offline license via command line.

### Fixes

*   : (1053850)
    
*   Asset Import: Stop avatar reference set in model importer causing false dependencies if it's not being used ([1007037](https://issuetracker.unity3d.com/issues/asset-bundle-dependencies-are-included-from-an-asset-linkage-in-an-animation-meta-file))
    
*   Editor: Added icon for LookAtConstraint component.
    
*   Editor: LW Pipeline Assets updated in template for new lightweight shadow features (1052133)
    
*   Kernel: Fix crash when existing playmode after loading a scene from an asset bundle opened using LoadFromStream API. ([1023820](https://issuetracker.unity3d.com/issues/editor-crashes-when-loading-scene-bundle-from-memory-stream-or-file-stream), 1035387)
    
*   Kernel: Fix incorrect triggering of Assert "Multiple dependency jobs should not be in ScheduleDependencies"
    
*   Mobile Rendering : \[Mobile\] Rendering Performance decrease in Unity 5.6 (compared to 5.4) ([954315](https://issuetracker.unity3d.com/issues/ios-rendering-performance-decrease-in-unity-5-dot-6-compared-to-5-dot-4))
    
*   OSX: Fixed caseof maximize green dot button doing zoom instead of toggling fullscreen. (1021777)
    
*   Profiler: Loading profiler sessions now contains all samples ([1047081](https://issuetracker.unity3d.com/issues/load-profiling-session-does-not-contain-all-samples), 1048795)
    
*   Testing: Improved player test result responses
    
*   Timeline: Fixed animations not showing up as sub-objects of Timeline (1050938)
    
*   Timeline: Fixed Play button behaviour in Timeline Window when time is past timeline duration (1047998)
    

2018.2.0b11 Release Notes (Full)
--------------------------------
