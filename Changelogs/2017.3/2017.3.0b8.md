...
*   Android: Changed "Bundle Identifier" to "Package Name" in associated error messages ([954779](https://issuetracker.unity3d.com/issues/bundle-identifier-error-message-contains-outdated-terminology))
    
*   Android: Fix building with target sdk 21-23 (956658)
    
*   Android: Fix corner case crash caused by the permissions popup ([934782](https://issuetracker.unity3d.com/issues/killing-application-during-permission-request-message-results-in-crash-with-android-dot-app-dot-fragment$instantiationexception))
    
*   Android: Project export - no longer overwriting res/ directory when exporting to keep local changes to layouts etc. intact
    
*   Animation: Fixed Asserts when loading AssetBundle with AnimationClip asynchronously ([943438](https://issuetracker.unity3d.com/issues/loading-an-asset-that-has-an-animator-component-from-an-asset-bundle-with-loadasync-throws-assertion-failed-error))
    
*   Animation: Fixed Copy/Paste in component not copying animated values when keyframing ([950809](https://issuetracker.unity3d.com/issues/values-of-gameobject-transform-cannot-be-copied-when-previewing-animations), 962629)
    
*   Animation: Fixed framing of selected edges in the Animator Window not properly centering view in certain cases. (952197)
    
*   DX12: Fixed crashes that can happen on DirectX12 with graphics jobs enabled to do with DynamicVBOs. ([951091](https://issuetracker.unity3d.com/issues/standalone-build-crashes-with-d3d12-and-graphic-jobs))
    
*   Editor: Fix crash when input processing happens during destruction ([946958](https://issuetracker.unity3d.com/issues/macos-fatal-error-when-quitting-getmanagerfromcontext-pointer-to-object-of-manager-inputmanager-is-null-table-index-1))
    
*   Editor: fixed issue where public methods with PreferenceItem attribute are not registered. ([958573](https://issuetracker.unity3d.com/issues/preferenceitem-attribute-does-not-create-a-section-in-the-preferences-window-if-a-public-static-method-is-used), 963194)
    
*   Graphics: Cull projectors correctly to match Editor scene cameras to fix i.e. Preview of a material color is changed to the color of the material used on a Projector ([954828](https://issuetracker.unity3d.com/issues/preview-of-the-other-material-color-is-changed-to-the-color-of-the-material-used-for-projector))
    
*   Graphics: Don't crash when creating sprite with unsupported texture format ([944413](https://issuetracker.unity3d.com/issues/editor-crashes-at-prcore-blitimageremapnoscale-when-creating-sprite-with-textures-format-set-to-r8-rg16-rgb9e5float-yuy2))
...
*   Graphics: On DirectX11 or DirectX12 fixed potential hang when using different sized render targets and multiple cameras. ([950907](https://issuetracker.unity3d.com/issues/directx-on-camera-dot-render-freezes-the-editor-when-clear-flags-is-set-to-depth-only-or-dont-clear))
    
*   Graphics: Vulkan: Fix crash
    
*   iOS: Fixed crash in Handheld.PlayFullScreenMovie when playback ends ([949361](https://issuetracker.unity3d.com/issues/ios11-crash-at-fullscreenvideoplayer-dot-mm-223-using-handheld-dot-playfullscreenmovie-when-movie-ends))
...
*   iOS: Fixed support of wide colors on new devices ([955685](https://issuetracker.unity3d.com/issues/iphone8-unsupported-wide-colors))
    
*   iOS: SystemInfo.supportedRenderTargetCount now correctly returns 8 for devices that support it ([949032](https://issuetracker.unity3d.com/issues/ios-systeminfo-dot-supportedrendertargetcount-returns-incorrect-number-of-render-targets))
    
*   Linux: Fix screen selector crash in freetype ([956475](https://issuetracker.unity3d.com/issues/linux-player-standalone-x86-64-builds-with-resolution-window-enabled-crash-in-funlockfile))
    
*   Shaders: Fixed case where some user shader keywords were handled incorrectly leading to compilation errors when building for standalone. ([944334](https://issuetracker.unity3d.com/issues/keywordenum-uv1-uv2-in-shaders-causes-a-compilation-error-when-building-for-standalone))
...
*   Universal Windows Platform: Fix building VS project after manually switching it to Windows SDK 15063 on .NET scripting backend ([940555](https://issuetracker.unity3d.com/issues/uwp-build-fails-with-system-dot-io-dot-filenotfoundexception-thrown-after-switching-target-version-from-older-to-newer-in-vs))
    
*   Universal Windows Platform: Fixed an assert when trying to access missing material properties on debug builds ([945336](https://issuetracker.unity3d.com/issues/uwp-player-crashes-on-material-gettexture-with-debug-configuration-when-custom-shader-with-texture2d-reference-is-used-in-ui))
    
*   Universal Windows Platform: Fixed Reference Rewriter complaining about missing operators == and != on System.Type on .NET scripting backend ([955651](https://issuetracker.unity3d.com/issues/wsa-reference-rewriter-fails-to-process-system-dot-type-inequality-operator))
    
*   Universal Windows Platform: Fixed StateMachineBehaviour not working on private fields before loading any scenes on .NET scripting backend ([937501](https://issuetracker.unity3d.com/issues/uwp-classes-implementing-the-iserializationcallbackreceiver-interface-do-not-deserialize-properly-in-a-statemachinebehaviour))
    
*   Universal Windows Platform: Fixed Unity not referencing facade class libraries when targeting UWP and using IL2CPP with .NET 4.6 API Compatibility level. This used to manifest with compilation errors such as "The type 'TextReader' is defined in an assembly that is not referenced. You must add a reference to assembly 'System.IO, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a'." ([949209](https://issuetracker.unity3d.com/issues/wsa-unity-does-not-add-system-dot-io-as-a-reference-assembly-when-il2cpp))
    
*   Video: Fatal Error "CheckDisalowAllocation" when using DontDestroyOnLoad WebCamTexture with Unity Remote camera ([921560](https://issuetracker.unity3d.com/issues/fatal-error-checkdisalowallocation-when-using-dontdestroyonload-webcamtexture-with-unity-remote-camera), 963960)
    
*   Video: On OSX, Editor crash when previewing/playing video on older OS version ([932957](https://issuetracker.unity3d.com/issues/macos-editor-crash-when-previewing-slash-playing-video-on-older-macos), 961028)
    
*   Video: On OSX, Video Player component rendering video with artifacts ([908289](https://issuetracker.unity3d.com/issues/mac-video-player-component-rendering-video-with-artifacts), 961026)
    
*   Video: Specific h264 Video Clip stops playing/lags after a few seconds while playing it on a lower-spec hardware ([930947](https://issuetracker.unity3d.com/issues/specific-h264-video-clip-stops-playing-slash-lags-after-a-few-seconds-while-playing-it-on-a-specific-hardware), 961027)
    
*   Windows: Fixed DictationRecognizer asserting when stopping or starting the speech recognition twice in a row ([918146](https://issuetracker.unity3d.com/issues/wsa-windows-speech-dictationrecognizer-throws-assertion-failed-on-expression-succeeded-hr-when-user-cancels-the-input))
...
*   XR: Fix camera aspect ratio after disabling VR ([892084](https://issuetracker.unity3d.com/issues/android-vrcardboard-aspect-ratio-not-preserved-when-switching-from-vr-cardboard-to-non-vr-vrsettings-dot-enabled-equals-false))
