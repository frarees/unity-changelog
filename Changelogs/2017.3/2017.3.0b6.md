...
*   Cache Server: \[macOS\]\[High Sierra\] Operating System freezes when changing Cache Server Mode multiple times ([954747](https://issuetracker.unity3d.com/issues/macos-high-sierra-operating-system-freezes-when-changing-cache-server-mode-multiple-times))
...
*   Android: Fix header availability in UnityWebRequest on Android when response uses chunked transfer ([956599](https://issuetracker.unity3d.com/issues/unitywebrequest-getresponseheaders-method-returns-null-on-android-devices))
    
*   Animation: Fix an irrelevant assert in Animation Component ([943954](https://issuetracker.unity3d.com/issues/assert-in-play-mode-when-culling-type-is-set-to-based-on-renderers-m-rendererdata-dot-m-isvisibleinscene-and-and-isinscene-dot-dot-dot))
    
*   Animation: FIxed an issue where Playable graphs would react differently if configured in coroutines vs Update (927919)
    
*   GI: Fix a crash in 'RenderTexture::DiscardContents' when opening the scene. ([935328](https://issuetracker.unity3d.com/issues/crash-in-proberenderer-rendercube-rendertexture-discardcontents-when-opening-the-scene))
    
*   GI: Fix a crash in ApplyMaterialPassWithCache when generating lighting and Lightmap Static option is enabled. ([946517](https://issuetracker.unity3d.com/issues/crash-on-applymaterialpasswithcache-when-generating-lighting-and-lightmap-static-is-enabled))
...
*   GI: Fix exception being thrown when the backgroung color of a Reflection Probe is changed. ([939947](https://issuetracker.unity3d.com/issues/reflectionprobe-exception-thrown-when-the-background-color-is-modified))
    
*   GI: Fix for real-time lightmaps still being present after Precomputed Real-time GI is disabled. ([888351](https://issuetracker.unity3d.com/issues/realtime-gi-realtime-lightmaps-dont-disappear-when-precomputed-realtime-gi-is-disabled))
    
*   GI: Fix Lightmap Static tree prefabs not being able to sample lighting through light probes. (946727)
    
*   GI: Fix project build errors when Reflection Probes Projection setting in Tier1(Graphics Settings) is unchecked. ([925552](https://issuetracker.unity3d.com/issues/unchecking-reflection-probes-box-projection-under-low-tier1-in-graphics-settings-throws-build-errors))
    
*   GI: Fix real-time lightmaps performance regression. (944363)
    
*   GI: Fix Reflection Probe sphere preview not being rendered in Scene View. ([928456](https://issuetracker.unity3d.com/issues/reflection-probe-preview-is-not-rendered-in-scene-view))
    
*   GI: Fix terrain artifacts caused by Enlighten terrain mesh triangulation being different from the original mesh triangulation. ([754298](https://issuetracker.unity3d.com/issues/terrain-with-baked-gi-get-wrong-shadow-on-other-objects))
    
*   Graphics: Added error message for graphics APIs that do not support texture wrap mode "mirror once" (Android Vulkan, Android GLES3 and WebGL). ([912323](https://issuetracker.unity3d.com/issues/texture-wrap-mode-mirror-once-is-wrong-on-android-vulkan-android-gles3-and-webgl-player))
...
*   Graphics: Emit error messages instead of assert when the screen position is out of view frustum. ([947342](https://issuetracker.unity3d.com/issues/screen-position-out-of-view-frustrum-when-scene-camera-moves-far-away-from-the-world-origin))
    
*   Graphics: Fix incorrect definition of \_ShadowCoord in AutoLight.cginc leading to unexpected compilation errors or extra useless interpolator. ([924464](https://issuetracker.unity3d.com/issues/incorrect-definition-of-shadowcoord-in-autolight))
...
*   Graphics: Fixed D3D11 driver assert message and potential crash " Invalid mask passed to GetVertexDeclaration() when using post-effect" ([932940](https://issuetracker.unity3d.com/issues/assertion-failed-invalid-mask-passed-to-getvertexdeclaration-when-using-post-effect))
    
*   Graphics: Fixed object with "Dynamic Occludee" property being disabled rendering in Preview window when selecting a different object. ([944223](https://issuetracker.unity3d.com/issues/unchecking-dynamic-occludee-and-selecting-another-mesh-causes-inital-mesh-to-remain-in-previewer))
    
*   Graphics: Fixed OpenGL errors in the Editor due to mismatched of color and depth MSAA samples.
    
*   Graphics: Fixed specific case where not all requested shader variants ending up in an asset bundle. ([948053](https://issuetracker.unity3d.com/issues/shader-with-corresponding-shadervariantcollection-built-as-an-assetbundle-and-loaded-in-the-scene-loses-variants))
    
*   Graphics: Fixed Tree shadows being culled when zoomed in on Occlusion Visualization mode. ([941334](https://issuetracker.unity3d.com/issues/occlusionculling-regression-tree-shadows-are-culled-when-zooming-in-in-vizualization-mode))
    
*   Graphics: Fixed Vulkan multiple display support.
    
*   Physics: Fix colliders missing pose updates when they were inactive during the scene load ([945332](https://issuetracker.unity3d.com/issues/moving-a-collider-via-script-does-not-update-its-position-if-it-was-disabled-on-start-and-enabled-after))
    
*   Physics: Fix physics settings being always marked as dirty up on the editor start, even when nothing was changed (948866)
    
*   Physics: Fix Physics.ComputePenetration returning incorrect result when a collider was scaled and had it's centre offset ([951233](https://issuetracker.unity3d.com/issues/computepenetration-return-incorrect-value-when-transform-scaled-and-center-of-collider-is-moved))
    
*   Physics: Fixes GameObject which has a disabled cloth component not following parent's transform ([946307](https://issuetracker.unity3d.com/issues/gameobject-which-has-a-disabled-cloth-component-does-not-follow-parents-transform))
...
*   Scripting: Fix crash in debugger when trying to stop while a single step operation is in progress ([951901](https://issuetracker.unity3d.com/issues/editor-stops-responding-when-stepping-into-method-and-stopping-debugging-in-visual-studio-2017))
    
*   Scripting: Save/resume entire 128-bits of xmm argument registers (952069)
    
*   Scripting Upgrade: Fix crash on OSX when socket error ENXIO is encountered ([944464](https://issuetracker.unity3d.com/issues/macos-crash-in-exc-crash-sigabrt-when-trying-to-use-system-dot-net-dot-sockets-dot-socket))
    
*   Scripting Upgrade: Fix crash when struct has array field of same type ([807575](https://issuetracker.unity3d.com/issues/unit-tests-of-a-fixed-point-math-library-cause-a-hard-crash))
    
*   Shaders: Disable instancing support when performing surface shader analysis. (935126, [941827](https://issuetracker.unity3d.com/issues/time-to-import-shader-files-doubled-from-5-dot-5-x-to-5-dot-6-x-and-2017-dot-x))
    
*   Shaders: Fixed incorrect translation of shaders using resinfo with mask operators. (943340)
    
*   Shaders: Fixed incorrect translation to GLSL of compute shaders using bfi instructions with mask operators. ([927339](https://issuetracker.unity3d.com/issues/hlslcc-bfi-instructions-with-mask-operators-are-incorrectly-translated-to-glsl))
    
*   Shaders: Fixed surface shader compilation error 'unknown semantics "SV\_InstanceID" specified for "instanceID" (d3d9)'. (935126)
...
*   Timeline: Fixed a crash from control tracks causing cycles in evaluation ([949374](https://issuetracker.unity3d.com/issues/crash-on-recursiveplayabletraverser-traverse-when-having-gameobjects-with-control-tracks-as-a-source-gameobjects-of-each-other))
