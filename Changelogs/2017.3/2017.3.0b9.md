...
*   OSX: Fix Editor crash when using GLCore on High Sierra with Intel 6xxx series GPU ([956196](https://issuetracker.unity3d.com/issues/unity-editor-crashes-when-creating-3d-objects-when-game-view-is-visible-and-default-skybox-is-enabled-on-13-inch-macbook-pro), 963864)
...
*   AI: Fix NavMesh.SamplePoint failing for meshes in range when another navmesh was below the point and out of range ([935433](https://issuetracker.unity3d.com/issues/navmesh-dot-sampleposition-produces-unreliable-results))
    
*   Android: Fix android video player multithreaded rendering ([931038](https://issuetracker.unity3d.com/issues/android-videoplayer-stuttering-and-dropping-frames))
    
*   Android: Fix android video player playback starting to lag after activating input field ([934841](https://issuetracker.unity3d.com/issues/android-application-lag-terribly-when-returning-from-input-field-slash-soft-keyboard-entry-on-android))
...
*   Editor: Fixed an issue where performing a drag and drop operation to a GameObject that is being edited in the Preview window of Timeline would apply changes to its associated Prefab that cannot be reverted. ([925381](https://issuetracker.unity3d.com/issues/drag-and-drop-applies-changes-to-a-prefab-with-timeline-when-in-preview-mode-which-cannot-be-reverted), 964916)
    
*   Editor: It should no longer be possible to start multiple PlayMode and EditMode test runs at the same time from the test runner ui. ([945000](https://issuetracker.unity3d.com/issues/testrunner-playmode-test-leaves-behind-inittestscene-dot-dot-dot-scene-assets-after-test-runs))
    
*   Graphics: Fix crashes related to removing optimized skinned mesh renderers ([964674](https://issuetracker.unity3d.com/issues/editor-crashes-after-deleting-animator-component-for-object-which-has-skinned-mesh-renderer-with-update-when-offscreen-enabled), [964683](https://issuetracker.unity3d.com/issues/editor-crashes-after-exiting-play-mode-when-scene-contains-couple-skinned-mesh-renderers), 964778)
    
*   iOS: Fix provisioning profile selection. ([955679](https://issuetracker.unity3d.com/issues/ios-invalidoperationexception-when-trying-to-set-provisioning-profile-in-player-settings-using-browse))
    
*   iOS: Fixed the Screen.dpi() method from the Trampoline code in order to return the correct number of DPI in iPhone 8, iPhone 8+ and iPhone X. Also fixed the iPhone 8+ and iPhone X model checking. ([960914](https://issuetracker.unity3d.com/issues/ios-a11-screen-dot-dpi-method-return-incorrect-number-of-dpi-on-iphone-8-8-plus), 962792)
...
*   Physics: When an animation disables a Collider2D, stop the collider from crashing or otherwise getting into a bad state. ([957044](https://issuetracker.unity3d.com/issues/setting-collider-dot-enabled-to-false-in-animation-does-not-disable-the-collider))
    
*   Terrain: Fix a crash when a splat texture only has a normal map ([963350](https://issuetracker.unity3d.com/issues/crash-in-splatdatabase-recalculatebasemap-when-accessing-scene-that-contains-terrain-with-missing-texture))
    
*   tvOS: Fix icon asset catalog setup when certain multi-layer icons are missing ([959867](https://issuetracker.unity3d.com/issues/appletv-building-a-project-that-doesnt-have-all-icons-set-up-for-appletv-fails-with-ioexception))
    
*   WebGL: Fixed missing slash in WebRequest formatted url ([940942](https://issuetracker.unity3d.com/issues/www-returned-url-missing-slashes-webgl-build))
    
*   XR: Fixed a thread affinity issue on certain mobile devices that could affect performance when resuming VR apps.
    

2017.3.0b9 Release Notes (Full)
-------------------------------
