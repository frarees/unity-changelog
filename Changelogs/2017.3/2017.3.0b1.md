...
*   Editor: Inspector doesn't immeadiately reflect changes in arrays when an element is deleted ([627322](https://issuetracker.unity3d.com/issues/inspector-doesnt-immeadiately-reflect-changes-in-arrays-when-an-element-is-deleted))
...
*   Animation: Couldn't set the transition time in Animator.CrossFade() ([941945](https://issuetracker.unity3d.com/issues/animation-reverting-an-animator-controller-in-the-middle-of-transition-reverts-to-a-different-point))
    
*   Animation: Further fixed alpha channel being animated when in linear color mode ([935087](https://issuetracker.unity3d.com/issues/material-alpha-value-that-is-animated-in-materials-color-property-is-returned-incorrectly-when-using-linear-color-space))
    
*   Asset Import: Check for normal maps when importing embedded materials ([944362](https://issuetracker.unity3d.com/issues/normalmap-settings-do-not-appear-when-importing-fbx-file))
...
*   Editor: Fixed editor crashes in Transform::CountNodesDeep when loading a new scene ([919023](https://issuetracker.unity3d.com/issues/editor-crashes-in-transform-countnodesdeep-when-loading-a-new-scene))
...
*   Graphics: Added support for vertex light in forward when shadowmask mode is used. ([917094](https://issuetracker.unity3d.com/issues/forward-realtime-lights-set-to-not-important-do-not-work-with-shadowmask))
    
*   Graphics: Fixed crash when certain variables of CustomRenderTexture are used in script. ([942563](https://issuetracker.unity3d.com/issues/crash-when-certain-variables-of-a-new-customrendertexture-are-used-in-script))
...
*   Graphics: Fixed not being able to set any Mesh on a Skinned Mesh Renderer. ([946068](https://issuetracker.unity3d.com/issues/skinnedmeshrenderer-property-sharedmesh-is-null-when-created-via-script))
...
*   Multiplayer: NetworkTransport.SendMulticast causes Unity to crash ([938980](https://issuetracker.unity3d.com/issues/networktransport-dot-sendmulticast-causes-unity-to-crash))
    
*   Scripting: Fix crash if delegate is created on un-inflated generic type ([763091](https://issuetracker.unity3d.com/issues/crash-when-adding-an-attribute-on-a-generic-class))
    
*   Scripting: Fix debugger error of "Unable to step" when trying to step over dead code blocks ([912607](https://issuetracker.unity3d.com/issues/visual-studio-throws-the-unable-to-step-error-when-stepping-over-the-code))
    
*   Scripting Upgrade: Do not use the AppleTlsProvider on macOS and iOS, as Unity doesn't implement a backend for it. Instead, use the default TLS provider. ([938287](https://issuetracker.unity3d.com/issues/mobile-il2cpp-dot-net-4-dot-6-argumentnullexception-when-running-azure-storage-sdk))
    
*   Scripting Upgrade: Implement stack overflow handling on Windows ([922636](https://issuetracker.unity3d.com/issues/unity-crashes-instead-of-throwing-stackoverflowexception-error-dot-net-4-dot-6))
    
*   Universal Windows Platform: Fixed "Failed to locate" exception in UWP Build Settings dialog. (949019)
    
*   Video: Fixed bug where multiple VideoClip resources could not be played on Standalone players. ([943395](https://issuetracker.unity3d.com/issues/regression-windows-build-does-not-play-videos-loaded-from-the-resources))
    
*   Video: Fixed crash in VideoClipImporter Preview when using Legacy MovieTexture ([945085](https://issuetracker.unity3d.com/issues/editor-crash-on-prcore-blitimage-when-playing-video-in-the-preview))
...
*   Web: Fix possible freeze on iOS, when running multiple UnityWebRequests with custom download handler scripts ([936801](https://issuetracker.unity3d.com/issues/ios-9-application-freezes-after-minimising-and-re-opening-it-when-downloading-with-webrequest))
...
*   XR: Fixed the issue with target eye not working with stereo instancing or multiview ([895976](https://issuetracker.unity3d.com/issues/sps-android-target-eye-test-breaks-for-single-pass-stereo-on-gear-vr-and-daydream))
    
*   XR: Fixed VRStats for Oculus ([947215](https://issuetracker.unity3d.com/issues/assetdatabase-dot-importpackagecompleted-callback-doesnt-fire-if-scripts-have-been-recompiled-during-the-import-process))
    

2017.3.0b1 Release Notes (Full)
-------------------------------
