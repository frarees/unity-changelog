...
*   [\[723801\]](http://issuetracker.unity3d.com/issues/unity-crashes-when-changing-assets-outside-the-editor) Animation: Editing .anim text file while Unity is running might crash the editor
...
*   [\[799288\]](http://issuetracker.unity3d.com/issues/regression-in-baked-transparency) GI: Baked transparency regressed in Enlighten 3.03 un2
*   [\[688985\]](http://issuetracker.unity3d.com/issues/baked-lighting-data-lost-after-rebake-in-copied-scene) GI: Copying a scene loses the baked lighting. Workaround is to manually "Build Lighting."
...
*   [\[789557\]](/Issue) Animation: A warning that was erroneously displayed in AnimationClip is now displayed in ModelImporter.
*   [\[788452\]](/Issue) Animation: Fixed a case where adding an IsActive property to a legacy animation would cause a crash when sampling.
*   [\[783143\]](/Issue) Animation: Fixed a crash triggered when playing an AnimatorControllerPlayable with an invalid asset.
*   [\[766978\]](/Issue) Animation: Fixed an issue where an assert would pop when importing animations on a model where the skinned mesh was not on the root joint.
*   [\[785852\]](/Issue) Animation: Fixed an issue where interrupted transitions would cause empty states to continue to output animation.
*   [\[789784\]](/Issue) Animation: Fixed an issue where the transition preview wouldn't reappear when valid parameters were set (after it having disappeared when invalid parameters were set).
*   [\[789586\]](/Issue) Animation: Fixed case of animation clip being invalidated when adding an animation event.
*   [\[796729\]](/Issue) Animation: Fixed case of CullingMode not getting properties applied when changed during playmode in Inspector.
*   [\[788132\]](/Issue) Animation: Fixed case of GetHumanPose crashing when using it with a humanoid avatar with unsupported hierarchy.
*   [\[778887\]](/Issue) Animation: Fixed case of playable not updating when animator controller is already set in animator.
*   [\[786827\]](/Issue) Animation: Fixed case of rotation values set in inspector being restricted to 0-360 degrees when recording animation.
*   [\[784470\]](/Issue) Animation: Fixed case of transition not evaluating when exit time is close to 1.0.
*   [\[773437\]](/Issue) Animation: Fixed crashes in AudioSource.GetCustomCurve.
*   [\[793808\]](/Issue) Animation: Fixed cut letter in the 'Dopesheet' button, in the animation window.
*   [\[789053\]](/Issue) Animation: Fixed issue whereby the Z position of RectTransform couldn't be animated.
*   [\[780836\]](https://issuetracker.unity3d.com/issues/assetbundles-crashes-in-a-built-player-with-5-dot-3-4) Asset Bundles: Fixed crash that could be triggered when calling AssetBundle.LoadAllAssetAsync() on a streamed scene asset bundle.
...
*   [\[787114\]](https://issuetracker.unity3d.com/issues/mse-entering-slash-exiting-playmode-makes-active-scene-a-scene-which-is-on-top-of-hierarchy) Editor: EDITOR ONLY: When having multiple scenes open when entering playmode, the active scene is now loaded and activated first, no matter where in the list it is. When exiting playmode the previous active scene is now correctly made the active scene again.
...
*   [\[792560\]](/Issue) Editor: Fixed gfxsettings when importing from an old unitypackage.
...
*   [\[678975\]](/Issue) Graphics: Fixed issue whereby building a player with -nographics would cause some image effects to become disabled in the build.
*   [\[739115\]](/Issue) Graphics: Repeated loading of PNG/JPG images into a Texture2D no longer fails.
...
*   [\[794384\]](/Issue) OpenGL: Work-around for Mac Intel driver crash when trying to use tessellation shaders on a GPU that can't do it.
...
*   [\[782556\]](/Issue) SpeedTree: Fixed an issue where billboards didn't scale correctly when batched.
*   [\[784423\]](/Issue) Terrain: Fixed an issue where deleting in-use terrainData while GI is being baked for the terrain would crash the editor.
...
*   Core: Optimizations made to SetParent() and Destroy(), with potentially dramatic performance increases. Please let us know how much it helps on your projects.
*   Shaders: Added Material.SetFloatArray and other array related functions, in addition to already existing Shader.SetFloatArray and MaterialPropertyBlock.SetFloatArray.

### Fixes

*   (Issue)Asset Bundles: Fixed progress reported by AssetBundleCreate request (which may come from UnityWebRequest).
*   [\[733441\]](/Issue) Dynamic Geometry: Fixed issue where billboard particles were casting shadows that were aligned to the view, rather than to the light direction.
*   [\[759485\]](/Issue) Dynamic Geometry: Fixed issue where small particles could cast the wrong sized shadows.
*   [\[798293\]](/Issue) Dynamic Geometry: Fixed issue where Trails and Lines were being accidentally batched together.
*   [\[780050\]](https://issuetracker.unity3d.com/issues/scenemanager-console-log-reference-to-scenemanager-dot-onsceneloaded-is-not-correct) Editor: Corrected warning message about deprecated OnLevelWasLoaded to correctly refer to sceneLoaded.
*   \[697872, 633905\] Editor: Fixed an issue whereby a stacktrace was not provided for logging or exceptions triggered from threads.
*   [\[792341\]](https://issuetracker.unity3d.com/issues/second-build-fails-when-animation-window-is-opened) Editor: Fixed case of "EnsureRunningOnMainThread is not allowed to be called during serialization" error being thrown.
*   [\[588531\]](https://issuetracker.unity3d.com/issues/project-fails-to-update-if-directory-is-deleted) Editor: Fixed case of folders being recreated when the meta file is still present after deletion.
*   [\[795700\]](https://issuetracker.unity3d.com/issues/graphicsemulation-empty-menu-item-appears-on-each-platform-switch) Editor: Fixed case of the shader hardware tier emulation messing up the menu structure in some cases.
*   [\[792544\]](/Issue) Editor: Fixed occasional crashes on DX9/DX11 when resizing editor windows.
*   [\[771941\]](https://issuetracker.unity3d.com/issues/duplicating-large-amounts-5000-of-items-in-the-hierarchy-takes-a-few-minutes) Editor: Fixed performance regression when duplicating many objects.
*   (Issue)GI: Changes to enable L2 spherical harmonics for light probes and to use the correct light probes SH axes and scaling (rebaking light probes required). Makes objects lit by lightprobes match static lightmapped objects. Additionally upgraded Enlighten to SDK-3.03.UN2.50261.
*   [\[725776\]](/Issue) GI: Fixed case of "Error adding system: Data not available." being thrown.
*   [\[773204\]](https://issuetracker.unity3d.com/issues/crash-on-transform-getlocaltoworldmatrixnoscale-when-game-object-is-deleted) GI: Fixed crash on "Transform::GetLocalToWorldMatrixNoScale" when game object is deleted in GI scene view mode.
*   [\[794551\]](/Issue) Graphics: Change to expose PlayerSettings.graphicsJobs editor scripting API.
*   [\[794402\]](/Issue) Graphics: Fixed a crash when using instanced shader on SkinnedMeshRenderer.
*   (Issue)Graphics: Fixed case of LightEvent command buffers going out of sync in heavy multithreading scenarios.
*   [\[787939\]](/Issue) Graphics: Fixed case of Unity crashing after adding a Graphics Raycaster component.
*   [\[798205\]](/Issue) Graphics: Fixed issue with calling CommandBuffer.ReleaseTemporaryRT followed by CommandBuffer.GetTemporaryRT with the same name.
*   (Issue)Graphics: Fixed performance degradation in Transform.position, rotation and localToWorldMatrix (Get).
*   [\[797773\]](/Issue) Graphics: Re-enabled LightEvent.AfterShadowMap command buffers for point lights.
*   [\[798012\]](/Issue) Image Effects: Fixed case of OnRenderImage source argument being null for image effects applied to the scene view in some cases.
*   [\[790833\]](https://issuetracker.unity3d.com/issues/ios-application-crashes-when-unitywebrequest-dot-abort-is-called-right-before-the-downloading-starts) iOS: Fixed crash when Abort was called on UnityWebRequest before downloading had begun.
*   \[792062\] iOS: Fixed crash when scrolling multiline input field on some devices.
*   [\[771132\]](https://issuetracker.unity3d.com/issues/trying-to-run-project-using-unityremote) iOS: Fixed Editor crash when iOS device is attached as a Unity Remote.
*   [\[782592\]](https://issuetracker.unity3d.com/issues/ios-application-freezes-when-trying-to-load-many-wwws-simultaneously) iOS: Fixed issue where generating too many WWW requests or UnityWebRequests at once would cause the app to lock up.
*   [\[761024\]](/Issue) iOS: Fixed performance regression caused by Physics/Job system changes.
*   [\[778222\]](/Issue) OpenGL: Fixed occasional crash when docking editor windows in Mac Editor.
*   [\[792881\]](/Issue) OpenGL: Fixed possible crashes when switching graphics API in the editor.
*   \[787566\] Scripting: Fix to prevent reimporting assets that do not support the cache server, when switching platforms.
*   UnityConnect: Fixed exception hashing algorithm to honor the intended character counts rather than the first 32.
*   \[779987\] Visual Studio: Fixed case of Visual Studio 2015 and MonoDevelop both opening the same script when Visual Studio is set as the script editor.
*   [\[778119\]](/Issue) VR: Single Pass VR: Fixed the following Image Effects from the Standard Assets: Antialiasing, Bloom, BloomAndFlares, BloomOptimized, ColorCorrectionCurves, ColorCorrectionLookup, ColorCorrectionRamp, ContrastEnhance, ContrastStretch, EdgeDetection, Fisheye, Grayscale, NoiseAndScratches, ScreenOverlay, SepiaTone, SunShafts, TiltShift, Twirl, VignetteAndChromaticAberration, Vortex.

* * *

5.4.0b21 Release Notes (Full)
-----------------------------
