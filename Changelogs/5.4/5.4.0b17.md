...
*   Asset Bundles: Build size stats are once again printed for normal asset bundles (fixing a regression introduced in 5.3.1).
*   Editor: Fix to ensure that sceneLoaded is called for first scene when entering play mode. I.e. this behaviour is now consistent with OnLevelWasLoaded.
*   Editor: Fix to prevent adding a scene multiple times to the scene list via click and drag.
*   Editor: Fixed serialization of SceneSetup.
*   Editor: Selecting a scene asset no longer shows "Message" and "Is Warning" fields in the inspector.
*   IL2CPP: Fix to emit proper C++ code for COM marshaling of methods that have at least one parameter that cannot be marshaled.
*   IL2CPP: Fix to ensure that correct C++ code is generated for IL code that returns an integer as an IntPtr from a method.
*   IL2CPP: Fixes to ensure generation of correct C++ code for assemblies compiled with Visual Studio when a method returning an IntPtr returns an integer value.
*   IL2CPP: Fixes to ensure generation of correct C++ code for the OnSerialize method injected by UNET in classes deriving from NetworkProximityChecker.
*   Scripting: Fix to avoid allocating GC memory during Coroutine iteration.
*   Windows: Fixed default position of the standalone Windows player window when it is larger than the primary monitor.
*   Windows: Fixed Screen.orientation reporting Autorotation on start-up.
*   Windows Store: SystemInfo.deviceModel will now report model name and manufacturer.

* * *

5.4.0b17 Release Notes (Full)
-----------------------------
