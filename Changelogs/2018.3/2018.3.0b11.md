### Known Issues in 2018.3.0b11 under investigation
...
*   AI: Fixed an issue with NavMeshObstacle carving that could create an unexpected boundary where one of the hull's corners or edges sits precisely on a NavMesh polygon edge. ([1078553](https://issuetracker.unity3d.com/issues/a-navmeshobstacle-placed-at-a-specific-position-creates-a-boundary-on-an-unobstructed-navmesh), 1097941)
...
*   Animation: Fixed crash when playing a playable graph and setting an override controller. ([1087755](https://issuetracker.unity3d.com/issues/mecanim-animation-valuesfromclip-crashes-when-entering-to-play-mode))
...
*   Editor: Fixed an issue where pressing CMD + Z when changing the project name causes a fatal error. ([1092471](https://issuetracker.unity3d.com/issues/macos-fatal-error-occurs-in-the-launcher-when-cmd-plus-z-is-used-when-changing-the-project-name), 1095490)
    
*   Editor: Fixed DynamicVBO crashing when switching renderers back and forth in player settings. (1092405)
    
*   Editor: Fixes mouse position being slightly offset from cursor in GameView. ([1068201](https://issuetracker.unity3d.com/issues/input-dot-mouseposition-is-offset-from-the-cursor), 1092348)
...
*   Editor: NET\_LEGACY is now only set when NET 2.0 or .NET 2.0 Subset profile is used.
    
*   Editor: ScriptableObject apply default references when created from ObjectFactory API and UI. ([1090072](https://issuetracker.unity3d.com/issues/scriptable-object-loses-references-when-new-sample-tile-is-created), 1099366)
...
*   GI: Progressive Lightmapper (CPU) : Fix first bake AO in non auto-mode. ([1096065](https://issuetracker.unity3d.com/issues/ao-on-the-first-bake-after-startup-is-broken))
    
*   Graphics: Fix Crash when creating RenderTexture with XR formats. (1089864)
    
*   Graphics: Fixed crash with Texture Mipmap streaming scene view debug mode when light maps are recalculated. ([1095546](https://issuetracker.unity3d.com/issues/dereferencing-null-pptr-crash-when-using-non-directional-lightmap-and-texture-streaming-scene-view), 1100073)
...
*   iOS: Fix occasional stalls in Memory Profiler that happen in development build. ([1000164](https://issuetracker.unity3d.com/issues/ios-player-freeze-on-writelock-atomic-compare-exchange-weak-explicit-on-some-devices), [1024354](https://issuetracker.unity3d.com/issues/ios-random-freeze-with-heavy-cpu-usage-while-destroy-is-being-used-rapidly-on-gameobject-dot-deactivate-on-weaker-devices))
...
*   Linux: Resolve an issue where double-clicking on Linux Standalone Players did not function in Nautilus. ([1047075](https://issuetracker.unity3d.com/issues/linux-executable-doesnt-launch-and-has-incorrect-icon), 1100864)
...
*   Particles: Fixed a memory leak when pausing Play Mode in the Editor with particle billboards visible. ([1098914](https://issuetracker.unity3d.com/issues/performance-memory-leak-when-pausing-billboarded-particles-in-editor-play-mode-and-switching-to-the-scene-view))
    
*   Particles: Fixed rendering corruption on particles. ([1092374](https://issuetracker.unity3d.com/issues/realtime-reflection-probes-cause-rendering-artifacts))
...
*   Prefabs: Fix leaks of Editor objects created by Overrides dropdown comparison view. ([1051224](https://issuetracker.unity3d.com/issues/editor-becomes-unresponsive-when-entering-play-mode-after-applying-prefab-overrides-and-re-adding-the-prefab-to-the-scene), 1098910)
...
*   Prefabs: Fix Transform.DetachChildren() to not hang the Editor if called on a Prefab instance. An error is now logged with info about restructuring a Prefab instance is not allowed. ([1089493](https://issuetracker.unity3d.com/issues/editor-freezes-when-trying-to-select-child-prefab-which-has-been-previously-deleted), 1098865)
...
*   UI: Fix crash if null texture in GUIStyle.scaledbackground. ([1074485](https://issuetracker.unity3d.com/issues/unity-crashes-on-guistyle-drawbackground-when-opening-custom-imgui-window), 1098087)
...
### Preview of Final 2018.3.0b11 Release Notes
...
*   Package Manager: Package Manager UI now refreshes after all add/remove actions. ([1064839](https://issuetracker.unity3d.com/issues/shadergraph-package-keeps-loading-on-package-manager-window-when-installed-or-removed))
...
*   Terrain: Fixed an issue where the UI elements would distort on "Import Heightmap" window when shrinking it horizontally and vertically. ([1053066](https://issuetracker.unity3d.com/issues/ui-elements-distort-on-import-heightmap-window-when-shrinking-it-horizontally-and-vertically))
...
*   Terrain: Fixed issue causing "Details are not being painted according to the selected brush" ([1068717](https://issuetracker.unity3d.com/issues/terrain-details-are-not-being-painted-according-to-the-selected-brush))
    
*   Terrain: Fixed issue causing "Unable to paint across Terrain Tiles" (1068933)
    
*   Terrain: Fixed issue where mesh renderer would draw shadows on terrain when painted as a tree, even when the cast shadow was set to "OFF". ([1050886](https://issuetracker.unity3d.com/issues/mesh-renderer-draw-shadows-on-terrain-when-painted-as-a-tree-even-when-the-cast-shadow-is-set-to-off))
...
*   UI: Fixed crash in AddUIVertexTriangleSteam caused by use of temporary variables ([738798](https://issuetracker.unity3d.com/issues/vertexhelper-adduivertextrianglestream-causes-crash))
    
*   UI: Fixed crash when instantiating GameObjects containing Canvas's in coroutines. ([1046498](https://issuetracker.unity3d.com/issues/unity-editor-crashes-when-instantiating-gameobjects-containing-canvas-and-sprites-in-coroutines))
...
*   UI: Fixed issue with Enabling "Content Size Fitter" Constraints and then removing them, greying out the "Width"/"Height" of the GO untill Scene Reload ([1013029](https://issuetracker.unity3d.com/issues/enabling-content-size-fitter-constraints-and-then-removing-them-greys-out-the-width-slash-height-of-the-go-untill-scene-reload))
    
*   UI: Fixed issue with Font Sizes that are too big for Rect Transform causes Argument Out Of Range ([1037923](https://issuetracker.unity3d.com/issues/using-font-sizes-that-are-too-big-for-rect-transform-causes-argument-out-of-range-2018-dot-3))
    
*   UI: Fixed issue with RectTransform messing up when dragging Canvas prefab onto another Canvas ([1047865](https://issuetracker.unity3d.com/issues/rect-transform-component-is-messed-up-when-dragging-canvas-prefab-on-other-canvas-object))
...
*   UI: Fixed RectTransfrom position being set to 0 after attaching to disabled canvas ([1030212](https://issuetracker.unity3d.com/issues/gui-element-with-rect-transform-position-is-set-to-0-dot-0-0-after-attaching-it-to-the-disabled-canvas))
...
*   UI: Fixed Undoing Add Component action on Canvas changing UI object positions depending on Anchor Presets ([1047799](https://issuetracker.unity3d.com/issues/undoing-add-component-action-on-canvas-changes-ui-object-positions-depending-on-anchor-presets))
    
*   UI: Fixed warning when Setting Aspect Ratio Fitter to "Fit in Parent" ([1014834](https://issuetracker.unity3d.com/issues/setting-aspect-ratio-fitter-to-fit-in-parent-causes-warning-in-the-console))
...
*   UI: Fixing issue with 16bit chars not displaying correctly. ([1067159](https://issuetracker.unity3d.com/issues/korean-ime-certain-combinations-of-korean-letters-get-deleted-from-the-inputfield-upon-typing))
...
*   UI: Fixing race condition that lead to batches being rendered with no bounds ([1038949](https://issuetracker.unity3d.com/issues/ios-delayed-call-gameobject-dot-setaction-makes-ui-disappear))
...
*   UI: Moved access to the TMP Settings to the new unified settings. ([1056436](https://issuetracker.unity3d.com/issues/project-settings-options-are-missing))
