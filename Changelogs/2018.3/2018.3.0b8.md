### Known Issues in 2018.3.0b8 under investigation
...
### New 2018.3.0b8 Entries since 2018.3.0b7
...
*   2D: Editor crashes in GameObjectHierarchyProperty::CountRemaining when undoing the creation of Sprite Mask object. ([1074128](https://issuetracker.unity3d.com/issues/editor-crashes-in-gameobjecthierarchyproperty-countremaining-when-undoing-the-creation-of-sprite-mask-object), 1078534)
...
*   2D: Fixed TilemapRenderer freeze in the Android player when no section of a Tilemap is visually on-screen. ([1072948](https://issuetracker.unity3d.com/issues/android-tilemap-causes-a-temporary-freeze-of-the-game), 1091994)
    
*   2D: Inactive Tilemaps no longer appear in the Tile Palette Active Target list. ([1085462](https://issuetracker.unity3d.com/issues/tile-palette-active-tilemap-list-shows-layers-with-inactive-gameobjects), 1091995)
...
*   Animation: Fixed Animator.applyRootMotion toggle resetting state machine when in game. ([1080216](https://issuetracker.unity3d.com/issues/animator-resets-to-the-default-layer-when-changing-applyrootmotion-during-runtime), [1087729](https://issuetracker.unity3d.com/issues/physics-simulation-causes-models-to-jitter-and-sometimes-fly-off-the-screen))
    
*   Animation: Fixed root motion not being evaluated when overriding with a script. ([1086070](https://issuetracker.unity3d.com/issues/applybuiltinrootmotion-does-not-evaluate-when-being-manually-called-in-a-script-overriding-root-motion))
    
*   Asset Import: Fixed "Format" setting is missing and ArgumentException occurs for texture preset platforms. ([1081058](https://issuetracker.unity3d.com/issues/editor-format-setting-is-missing-and-argumentexception-occurs-for-texture-preset-platforms), 1091711)
...
*   Editor: Fix that errors are thrown when a prefab with more that 50 game components is selected in the Hierarchy window. ([1085685](https://issuetracker.unity3d.com/issues/errors-are-thrown-when-a-prefab-with-more-that-50-game-components-is-selected-in-the-hierarchy-window), 1090995)
    
*   Editor: Fixed an issue where red text could appear in the Project search bar along with "style not found" console warnings, while searching in play mode. ([1086419](https://issuetracker.unity3d.com/issues/red-text-in-project-search-bar-when-searchingwhile-in-play-mode), 1091550)
    
*   Editor: Fixed an issue where usage of Obsolete APIs could spam the console window with infinite null reference exceptions. ([1085675](https://issuetracker.unity3d.com/issues/shader-causes-infinite-nullreferenceexception-errors-even-after-deletion), 1092288)
    
*   Editor: Fixed arrow keys logging errors in editor log slowing down editor.
    
*   Editor: Fixed issue by restricting the insertion of menu items into a given submenu to a prescribed limit of 1000 items and displays a warning about the same. ([1039181](https://issuetracker.unity3d.com/issues/assertion-failed-failed-to-insert-item-error-is-thrown-when-scripts-count-exceed-the-limit-of-1167-scripts))
...
*   Editor: Fixed `Find References in Scene` showing objects that are not referencing the asset when using prefabs. ([970903](https://issuetracker.unity3d.com/issues/find-references-in-scene-shows-objects-that-are-not-referencing-the-asset))
    
*   Editor: GenericMenu.allowDuplicateNames now works for subMenus. ([1074892](https://issuetracker.unity3d.com/issues/enabled-genericmenu-dot-allowduplicatenames-doesnt-allow-duplicates-in-submenus), 1091738)
    
*   Editor: Removing .meta files from empty 3D and 2D project templates so unique GUIDs are generated. (1089085)
    
*   GI: Crash in CalculateSurfaceArea after opening project with 32bit compressed mesh. Crash was caused by missing index format test in the Mesh code. ([1072951](https://issuetracker.unity3d.com/issues/crash-in-calculatesurfacearea-after-opening-the-project-with-certain-mesh))
...
*   IL2CPP: Correctly handle memory mapped files with an offset. ([1082118](https://issuetracker.unity3d.com/issues/memorymappedfile-class-crashes-on-il2cpp-builds))
...
*   IL2CPP: Generate proper C++ code for unsafe C# code which accesses an array element via a pointer. ([1081052](https://issuetracker.unity3d.com/issues/il2cpp-unity-editor-fails-to-build-when-auto-generating-a-c-number-array-of-pointers-in-c))
    
*   iOS: Fixed an issue where the screen could be cut off/corrupted when launching from an associated domain link. (1093921)
    
*   Kernel: Improves performance of job stealing in all worker threads by aligning data to cache line boundaries (avoids false sharing)
...
*   Prefabs: Fix 'Open Prefab Asset' context menu item causing issues when used on a model Prefab. Now, show 'Open Model' context menu item for model Prefabs instead. ([1079961](https://issuetracker.unity3d.com/issues/selecting-open-prefab-asset-on-a-prefab-variant-made-from-an-fbx-throws-nullreference-exceptions))
    
*   Prefabs: Fix issues with incorrect display of removed components on Prefab instances in the Inspector in cases where components are null (due to missing script) or are set to HideInInspector on either the Asset or on the instance. ([1082570](https://issuetracker.unity3d.com/issues/setting-hideflags-dot-hideininspector-on-a-prefabs-component-shows-the-component-as-transform-removed-instead-of-hiding-it), [1083899](https://issuetracker.unity3d.com/issues/missing-components-dont-appear-in-the-inspector-when-in-prefab-mode-and-a-misleading-message-is-shown))
...
*   Prefabs: Prefab Mode: Fix error thrown in Console when creating a new script in Prefab Mode and exiting Prefab Mode while compiling ([1083386](https://issuetracker.unity3d.com/issues/error-is-thrown-in-console-when-creating-a-new-script-in-prefab-mode-and-returning-to-scene))
    
*   Scripting: Fix crash with message box "GetThreadContext failed". (1082246)
    
*   Scripting Upgrade: Fix ArgumentException: "GCHandle value belongs to a different domain" during WebRequest. ([1082650](https://issuetracker.unity3d.com/issues/x86-linux-player-webrequest-returns-error-argumentexception-gchandle-value-belongs-to-a-different-domain))
...
*   Scripting Upgrade: Fixed random crash on x86 Linux with TLS connections ("GCHandle value belongs to a different domain"). ([1082650](https://issuetracker.unity3d.com/issues/x86-linux-player-webrequest-returns-error-argumentexception-gchandle-value-belongs-to-a-different-domain))
    
*   Shaders: Fix shader compiler crash when handling SV\_DepthGreaterEqual/SV\_DepthLessEqual (1077443)
    
*   Video: Fixed Video component volume controls are not working. ([1077684](https://issuetracker.unity3d.com/issues/video-component-volume-controls-are-not-working), 1079708)
...
*   Vulkan: Fixed crash on standalone Windows player when minimizing the player window. ([1086940](https://issuetracker.unity3d.com/issues/vulkan-crash-when-minimizing-player-window))
    

### Preview of Final 2018.3.0b8 Release Notes
