2018.3.0b4 Release Notes
========================

Known Issues in 2018.3.0b4
--------------------------
...
*   Physics: Physics performance regression ([1086243](https://issuetracker.unity3d.com/issues/query-performance-significantly-dropped))
...
*   SceneManager: Selecting Animator component's Optimize Transform Hierarchy setting freezes the editor and causes a memory leak (1079977)
    

Known Issues - won't be fixed in 2018.3
---------------------------------------

*   Gr: Reinstalling Render Pipeline Package results in Errors until Editor is restarted ([1075234](https://issuetracker.unity3d.com/issues/hdrp-lwrp-reinstalling-render-pipeline-package-results-in-broken-lighting-and-argumentnullexception-errors), 1078081)
    
*   IL2CPP: Building Development Build fails on empty project when Script Debugging is enabled and Scripting Backend is IL2CPP (1082125)
...
New 2018.3.0b4 Entries Since 2018.3.0b3
---------------------------------------
...
2018.3.0b4 Entries Already Released in 2018.2
---------------------------------------------

### Fixes

*   Animation: Fix regression when changing hierarchy under an Animator ([1080559](https://issuetracker.unity3d.com/issues/2018-dot-2-regression-animation-binding-breaks-after-activating-gameobject))
    
*   Asset Import: Fixed crash when re-importing assets for some global settings ([1079532](https://issuetracker.unity3d.com/issues/unity-crashes-on-std-tree-std-tmap-traits-core-basic-string-char-core-stringstoragedefault-when-reimporting-assets), 1081905)
    
*   Editor: Disable Drag & Drop of packaged textures onto GameObjects in the scene because the generated material cannot be saved. ([1057779](https://issuetracker.unity3d.com/issues/materials-are-not-accessible-when-created-from-textures-in-packages-folder), 1082419)
...
*   Graphics: Fix for texture mip map streaming crash (1081114)
...
*   Scripting: Fix error in CSharpNameParser when parsing conditional directives ([1077417](https://issuetracker.unity3d.com/issues/error-in-csharpnameparser))
    
*   Scripting: Fix incorrect Transform.hierarchyCount and Transform.hierarchyCapacity properties ([1079998](https://issuetracker.unity3d.com/issues/transform-dot-hierarchycount-always-returns-0-regardless-of-the-number-of-transforms-in-the-transforms-hierarchy-data-structure), 1082622)
    
*   Scripting Upgrade: Fix crash when calling Application.Unload ([1060891](https://issuetracker.unity3d.com/issues/application-dot-unload-crashes-with-the-4-dot-x-scripting-runtime))
    
*   Scripting Upgrade: Fix missing crash callstacks on OSX (1074280)
    
*   Scripting Upgrade: Fix potential deadlock on OSX (1077203)
    
*   Scripting Upgrade: Fixed SslStream/UnityTlsContext not reporting gracefully closed Streams on read (1076788)
    
*   Timeline: Fixed 'Find Source Asset' clip operation for externally referenced PlayableAssets ([1079560](https://issuetracker.unity3d.com/issues/timeline-find-source-asset-option-is-disabled-on-playable-track-timeline))
    
*   Timeline: Fixed Editor Playback stopping when a track warning appears (1080143)
    
*   Timeline: Fixed Track Selection when other menus are open (1071296)
    
*   Universal Windows Platform: Fixed an assert triggered when Xbox gamepads are disconnected in some situations (1081997)
    
*   Universal Windows Platform: Fixed build error in Editor when using Entities package for UWP ([1067528](https://issuetracker.unity3d.com/issues/uwp-building-an-application-on-uwp-fails-when-entities-package-is-installed), 1082003)
...
*   2D: Fixed a crash when an invalid Sprite is set in the Tilemap while the TilemapRenderer is in Individual mode. ([1078047](https://issuetracker.unity3d.com/issues/2018-dot-3-rendernodequeue-reset-or-buildrenderobjectdata-crashes-when-opening-a-scene-or-entering-play-mode), 1078801)
    
*   2D: Fixed calculation of bounds of a TilemapRenderer on an Isometric grid. ([1077673](https://issuetracker.unity3d.com/issues/tilemap-painted-tilemap-disappears-from-scene-view-when-dragging-the-scene-view-to-the-right), 1084074)
    
*   2D: Setting the TilemapRenderer to Individual mode with no Tiles rendered no longer causes a memory leak. (1084071)
    
*   Android: Fix crash caused by BC6H texture assets ([1064440](https://issuetracker.unity3d.com/issues/android-crash-on-start-if-auto-generate-lighting-is-enabled))
    
*   Android: Fix crash on startup on Android 4.3 and earlier ([1069896](https://issuetracker.unity3d.com/issues/android-empty-project-crashes-on-samsung-galaxy-s3-device))
    
*   Editor: Fixed a syntax error in the C# test script template. (1082970)
    
*   Editor: Fixed adb inheriting handles on Windows when building for Android ([1076356](https://issuetracker.unity3d.com/issues/adb-prevents-editor-logs-from-being-deleted-slash-renamed-on-windows))
    
*   Editor: Fixed alert sound in Game View while in play mode when key is pressed (macOS). ([1080334](https://issuetracker.unity3d.com/issues/macos-funk-error-sound-plays-when-pressing-any-non-shortcut-key-in-play-mode))
    
*   Editor: Fixed missing File > Close menu item on macOS. ([1078378](https://issuetracker.unity3d.com/issues/macos-close-window-cmd-plus-w-option-doesnt-work-on-any-of-the-editor-windows))
    
*   GI: Added better discoverbility and design for when working with Lightmaps Previews
    
*   Graphics: Change default QualitySettings.asyncUploadBufferSize to 16MB. Adding QualitySettings.asyncUploadPersistentBuffer option to control if the async upload system should retain the ring buffer while no loading operations are active (1078079)
    
*   Particles: Particle Subemitters didn't inherit parent's rotation correctly while the parent object was disabled in Play mode ([1063249](https://issuetracker.unity3d.com/issues/particle-subemitters-dont-inherit-parents-rotation-correctly-while-the-parent-object-is-disabled-in-play-mode), 1084250)
    
*   Particles: The sub emitter inherit lifetime option was being ignored. ([1075149](https://issuetracker.unity3d.com/issues/the-lifetime-of-the-particles-do-not-follow-the-lifetime-of-the-parent-particle-when-inherit-lifetime-is-set-in-the-sub-emitters), 1084253)
    
*   Terrain: Fix various lightmap debug rendering for terrains with instancing enabled. (1070522, [1082011](https://issuetracker.unity3d.com/issues/terrain-doesnt-work-with-lighting-debug-views-when-draw-instanced-is-enabled))
    
*   UI: Fixing issue with selectable Alpha not calculating correctly ([1080385](https://issuetracker.unity3d.com/issues/ui-button-alpha-value-is-maximum-255-at-any-value-except-0))
    
*   Universal Windows Platform: Fixed build window only allowing selecting minimum OS version that is identical to target SDK version ([1079835](https://issuetracker.unity3d.com/issues/uwp-projects-only-allow-specifying-current-target-sdk-as-minimum-supported-os-version))
    
*   Video: Fixed errors logs in Video previews ([1072989](https://issuetracker.unity3d.com/issues/video-playing-video-in-preview-throws-error-in-console), 1082640)
    

Preview of Final 2018.3.0b4 Release Notes
-----------------------------------------
