### Known Issues in 2018.3.0b12 under investigation
...
### New 2018.3.0b12 Entries since 2018.3.0b11
...
*   Scripting: ENABLE\_VSTU when VisualStudio support has been initialized.
...
*   2D: Fixed issue where Sorting Groups did not update in Play Mode when Play Mode was paused in the Editor. ([1047148](https://issuetracker.unity3d.com/issues/editing-a-sprite-atlas-dirties-the-scene-but-doesnt-check-out-the-scene-in-vcs), 1102015)
    
*   Android: Fix Android VideoPlayer does not support even framerates and shows different frame on some Android devices. ([1096444](https://issuetracker.unity3d.com/issues/android-xr-unity-fails-to-build-apk-if-split-application-binary-and-virtual-reality-supported-is-enabled))
...
*   Android: Using aab (Android App Bundle) extension only when building with Gradle. ([1058839](https://issuetracker.unity3d.com/issues/android-appbundle-setting-stays-active-when-the-build-system-is-changed-from-gradle-to-internal))
...
*   Editor: Fix random windows editor crash after resetting audio mixer from inspector menu. ([1097134](https://issuetracker.unity3d.com/issues/audio-editor-crash-on-resetting-audio-mixer-group-properties))
...
*   Editor: Fixed Timeline/Play default shortcut not set to the Space key. ([1098991](https://issuetracker.unity3d.com/issues/timeline-play-hotkey-spacebar-is-not-working-anymore))
    
*   Facebook: Fix for SDK's DLL stripping issue with Facebook platform's WebGL builds.
    
*   GI: Fix an issue were hashing didn't capture a change of scenes when creating a LightingData assset in auto-mode. ([1094637](https://issuetracker.unity3d.com/issues/environment-lighting-is-not-applied-when-loading-a-duplicated-scene-via-scenemanager-dot-loadscene))
...
*   Graphics: Fix RGBA4444 texture support when using OpenGL ES. ([1077689](https://issuetracker.unity3d.com/issues/opengles-ios-android-built-project-rgba16-format-textures-are-converted-to-rgba32-format), 1079023)
    
*   Graphics: Fixed Graphics.CopyTexture failing to copy textures into a Texture2DArray for compressed TextureFormats. ([1094698](https://issuetracker.unity3d.com/issues/graphics-dot-copytexture-fails-to-copy-textures-into-a-texture2darray-for-certain-textureformats))
    
*   Graphics: Fixed Graphics.CopyTexture() freezing the Editor when Texture Quality is set to Half Res with DirectX. ([1030483](https://issuetracker.unity3d.com/issues/directx-graphics-dot-copytexture-freezes-the-editor-when-texture-quality-is-set-to-half-res))
    
*   Graphics: Fixed issue with Graphics Jobs which could result in mesh particles flickering. (1100352)
    
*   Graphics: Vulkan: Fixed multidisplay rendering. ([1079634](https://issuetracker.unity3d.com/issues/vulkan-crashes-in-player-on-vk-icdnegotiateloadericdinterfaceversion-when-using-multidisplay))
    
*   IL2CPP: Fixed IL2CPP build failing if Visual Studio 2019 is installed on the machine.
    
*   IL2CPP: Properly handle a closed delegate used to invoke a virtual method on a value type. ([1096431](https://issuetracker.unity3d.com/issues/il2cpp-player-crashes-when-delegating-a-structs-virtual-method))
    
*   IL2CPP: Support the AttributeType property of of the TypeInfo object. ([1094621](https://issuetracker.unity3d.com/issues/il2cpp-certain-linq-query-operation-return-null))
    
*   iOS: Fixed inverted screen orientation on a specific(~45 degrees) angle. ([1076400](https://issuetracker.unity3d.com/issues/ios-screen-dot-orientation-is-inverted-after-changing-screen-orientations-when-device-is-tilted-at-~45-degrees), 1096710)
...
*   Prefabs: Errors spams the console if auto-save is enabled and trying to save a Variant Prefab with a missing base Prefab. ([1100637](https://issuetracker.unity3d.com/issues/error-message-spams-the-console-while-editor-is-trying-to-save-prefab-asset-into-deleted-nested-folder), 1100777)
    
*   Prefabs: Fix 'Save Assets' window pops up after every prefab adjustment in Prefab Mode when 'Verify Saving Assets' and 'Auto Save' is both checked. ([1096454](https://issuetracker.unity3d.com/issues/save-assets-window-pops-up-after-every-prefab-adjustment-in-prefab-mode-when-verify-saving-assets-and-auto-save-is-checked), 1100775)
    
*   Prefabs: Fix applying array element properties beyond length of source array so it asks to apply the entire array instead of producing errors. ([1094332](https://issuetracker.unity3d.com/issues/exceptions-are-thrown-while-attempting-to-apply-property-modification-via-context-menu), 1100782)
...
*   Prefabs: Fix for crash when trying to revert added component that other added component depends on. ([1093613](https://issuetracker.unity3d.com/issues/improved-prefabs-crashes-as-override-window-doesnt-check-component-dependency), 1095499)
...
*   Prefabs: Fix null-reference exception when switching aspect ratio in Game View. ([1099985](https://issuetracker.unity3d.com/issues/imgui-null-reference-exception-is-thrown-on-selecting-different-option-from-dropdown-in-aspect-ratio-and-tilepallete), 1100453)
...
*   UI: Fixes InputField gets corrupted and ArgumentOutOfRangeException is thrown when using certain emojis. ([1050713](https://issuetracker.unity3d.com/issues/inputfield-gets-corrupted-and-argumentoutofrangeexception-is-thrown-when-using-certain-emojis))
...
### Preview of Final 2018.3.0b12 Release Notes
...
*   2D: Image components now correctly display valid Sprite data when instantiated from a Prefab and triggering a Sprite Atlas request. ([1038179](https://issuetracker.unity3d.com/issues/image-components-not-displaying-sprite-data-when-instantiated-from-a-prefab-and-trigger-a-spriteatlas-request))
    
*   2D: The "Invalid texture used for cursor" warning message is no displayed in the console when switching between the Tile Palette edit modes. ([1051717](https://issuetracker.unity3d.com/issues/invalid-texture-used-for-cursor-warning-shown-when-tiles-are-selected-in-tile-palette-window))
...
*   2D: The Tilemap editing preview is now cleared when the user switches to another tab from Scene view. ([1043365](https://issuetracker.unity3d.com/issues/game-view-displays-unplaced-tile-when-it-is-visible-on-the-scene-and-other-tab-is-selected))
...
*   Android: Fixed an issues where textures are not displayed when Compress using ETC1(split alpha channel) was checked. ([1067933](https://issuetracker.unity3d.com/issues/android-mobile-does-not-render-texture-when-compress-using-etc1-split-alpha-channel-is-checked-in-android-override-settings))
...
*   iOS: Changing iPhone X orientation manually to upside-down portrait is no longer permitted, as it's not supported by the device. ([1064081](https://issuetracker.unity3d.com/issues/ios-forcing-portrait-upside-down-crashes-application-on-iphone-x))
...
*   iOS: Fix for incorrect 'Success' message when a build fails during post processing stage. ([1057069](https://issuetracker.unity3d.com/issues/android-ios-tvos-build-fails-but-build-succeeded-message-is-present))
...
*   iOS: We no longer allow building for Apple TV/iOS Simulator if only Metal is added to the graphics API list (as the Simulator does not support Metal). ([1064041](https://issuetracker.unity3d.com/issues/tvos-simulator-simulator-builds-crash-due-to-default-graphics-being-set-to-metal-only))
...
*   Scripting: Fixed case of 'Error in CSharpNamespaceParser\` being thrown when importing C# scripts containing trailing whitespaces. ([1056395](https://issuetracker.unity3d.com/issues/error-in-csharpnamespaceparser-when-importing-c-number-scripts))
...
*   Scripting: Visual Studio Project generation: Will now regenerate project when enabled built-in packages have been changed. ([1030664](https://issuetracker.unity3d.com/issues/builtin-modules-changing-module-state-doesnt-update-the-star-dot-csproj-file))
