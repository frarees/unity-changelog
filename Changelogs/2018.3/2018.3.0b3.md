2018.3.0b3 Release Notes
========================

Known Issues in 2018.3.0b3
--------------------------
...
New 2018.3.0b3 Entries Since 2018.3.0b2
---------------------------------------
...
2018.3.0b3 Entries Already Released in 2018.2
---------------------------------------------

### Improvements

*   XR: We now auto-open VR settings when a VR Device is added to the list.

### Fixes

*   AI: `RemoveAllNavMeshData()` now waits for `NavMeshObstacles` to finish carving, in order to prevent a crash. (1075239)
...
*   Android: Fixed TLS-related crash. (1080019)
    
*   Editor: Fix to make sure that modules depending on disabled modules are also disabled.
    
*   Editor: Fixed case if incorrect button style in the texture inspector.
    
*   Editor: Fixed case of drag and drop not working on Preset Manager from Unified Settings. ([1069618](https://issuetracker.unity3d.com/issues/preset-manager-from-unified-settings-doesnt-allow-drag-and-drop-anymore))
    
*   Editor: Fixed case of the `FloatField` being too large in a `VectorXField`. (1079656)
    
*   Editor: Fixed invalid texture and color blending with UIElements. (1078812)
...
*   Editor: Linux: Fixed keypad period and delete for input fields.
    
*   Editor: Removed spurious space between Edit Mode buttons. (1080018)
    
*   IL2CPP: Changed load indirect instructions to always use a temporary instead of pushing a pointer expression on the value stack, which can cause incorrect behavior in certain cases. ([1077373](https://issuetracker.unity3d.com/issues/il2cpp-c-number-reference-parameter-causes-indexoutofrangeexception-on-il2cpp-backend))
    
*   IL2CPP: Fix for an issue that can be encountered when trying to set a field value via reflection that has a previously unused type. ([1078167](https://issuetracker.unity3d.com/issues/il2cpp-assertion-error-being-fired-when-setting-fieldinfo-dot-setvalue-to-null))
    
*   IL2CPP: Fix to prevent an error during code conversion when both a pinned and unpinned pointer types are used as local variables and the debugger is enabled. (1070548)
    
*   IL2CPP: Fixed an issue that allowed debugger stack-frame data to get out of synchronization with the debugger thread and cause crashes when debugging players. ([1064723](https://issuetracker.unity3d.com/issues/ios-android-random-crashes-while-debugging-with-il2cpp-debugger-in-visual-studio))
    
*   IL2CPP: Fixed an issue with `UnsafeUtility.SizeOf()` crashing IL2CPP when called with an enum type.
    
*   IL2CPP: Fixed an unhandled exception in IL2CPP. Adding support for pointer types when executing an initobj instruction.
    
*   IL2CPP: For asynchronous delegate invocation, we no longer assume that a class with a method named "Invoke" is a delegate. ([1075581](https://issuetracker.unity3d.com/issues/il2cpp-async-operation-causes-nullreferenceexception-in-the-build-when-scripting-backend-is-il2cpp))
    
*   Prefabs: Fixed issue whereby function `PrefabUtility.CreatePrefab` would ignore `ReplacePrefabOptions.ReplaceNameBased`. (1078397)
    
*   SceneManager: Fixed crashes caused by incorrectly activated children of inactive parents. ([1051704](https://issuetracker.unity3d.com/issues/crash-on-particlesystemgeometryjob-schedulejobs-when-deleting-gameobject), 1081784)
    
*   Timeline: Fixed assembly reload causing a locked timeline to become deselected sometimes. (1078546)
    
*   Timeline: Fixed case of objects getting offset when recording on animation tracks with scale and position. ([1075970](https://issuetracker.unity3d.com/issues/object-is-getting-offset-while-recording-timeline-animation-when-scale-and-position-is-animated))
    
*   Timeline: Fixed hang when using custom playable assets that return a default of 0 duration. ([1075761](https://issuetracker.unity3d.com/issues/unity-freezes-when-changing-the-length-of-a-timeline-clip-with-0-duration))
    
*   Timeline: Fixed jittering during recording on animation tracks when recording scale before position. ([1076023](https://issuetracker.unity3d.com/issues/timeline-animated-object-jumps-around-while-animating-its-position-or-rotation-when-scale-was-animated-first))
...
*   Prefabs: Select, Revert All and Apply All are now supported when multiple Prefab instances are selected. (1075528)

### Fixes

*   2D: Fixed case of error message being thrown when deleting an asset before Sprite Editor Window applies changes to the asset. (1080953)
    
*   Editor: Fixed persistence of checked state in user-added menu items. ([1038153](https://issuetracker.unity3d.com/issues/selecting-custom-menuitem-from-component-tab-results-in-cant-be-checked-because-doesnt-exist-error-message))
    
*   Editor: Fixed play mode shortcut regression. ([1075121](https://issuetracker.unity3d.com/issues/hotkeys-are-not-ignored-even-when-game-view-is-focused-in-play-mode))
    
*   Particles: Fixed regression when using non-uniform scale on particle meshes. ([923170](https://issuetracker.unity3d.com/issues/setting-scaling-mode-to-hierarchy-when-particle-system-or-parent-is-scaled-non-uniformly-produces-random-rotation))
    
*   Prefabs: Fixed implementation of `PrefabUtility.ApplyObjectOverride` so that it only calls `ApplyModifiedProperties` once per object (apply target plus each object that needs to have its overrides cleared). (1073443)
    
*   Prefabs: Imported components on Model asset prefabs are once again shown (previously they were hidden because we don't want to show components for normal prefab assets).
    
*   Video: Fixed case of invalid Time position during seek operations in VideoPlayer. ([1052848](https://issuetracker.unity3d.com/issues/videoplayer-timeline-goes-to-0-when-seeking), 1080030)
    
*   XR: Fixed case of Single Pass Instanced rendering not handling downsampled Render Texture viewports correctly. (1068220)
    

Preview of Final 2018.3.0b3 Release Notes
-----------------------------------------
