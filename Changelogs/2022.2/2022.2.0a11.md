...
*   Asset Bundles: Unity closes when corrupted AssetBundle is opened with AssetBundle Browser ([1407033](https://issuetracker.unity3d.com/issues/unity-closes-when-corrupted-assetbundle-is-opened-with-assetbundle-browser))
    
*   Graphics: File could not be read error is thrown when a 16 bit .tif file is present in the project ([1404047](https://issuetracker.unity3d.com/issues/file-could-not-be-read-error-is-thrown-when-a-16-bit-tif-file-is-present-in-the-project))
    
*   Editor: Game View is not displayed after setting language pack in Editor ([1420291](https://issuetracker.unity3d.com/issues/gameview-is-not-displayed-after-setting-language-pack-in-unityeditor))
    
*   Editor: Impossible to change the Gradient's location value in the Visual Effects Graph when using the keyboard ([1420954](https://issuetracker.unity3d.com/issues/impossible-to-change-the-gradients-location-value-in-the-visual-effects-graph-when-using-the-keyboard))
    
*   Graphics: Random-write to UAV texture from shadowcaster shader causes GPU hang when filtering Scene Hierarchy objects ([1417589](https://issuetracker.unity3d.com/issues/random-write-to-uav-texture-from-shadowcaster-shader-causes-gpu-hang-when-filtering-scene-hierarchy-objects))
    
*   Editor: Unity Crashes when using serializedObject.CopyFromSerializedPropertyIfDifferent() and the provided property is an ".Array.size" ([1413250](https://issuetracker.unity3d.com/issues/unity-crashes-when-using-serializedobject-dot-copyfromserializedpropertyifdifferent-and-the-provided-property-is-an-dot-array-dot-size))
...
*   HDRP: Basic Indoors template Scene is completely white when loaded with Volume Exposure Mode set to Fixed ([1381761](https://issuetracker.unity3d.com/issues/hdrp-basic-indoors-template-scene-is-completely-white-when-loaded-with-volume-exposure-mode-set-to-fixed))
    
*   HDRP: \[RTX\] RTR does not seem to reflect what they should in a player ([1410966](https://issuetracker.unity3d.com/issues/hdrp-rtx-rtr-does-not-seem-to-reflect-what-they-should-in-a-player))
...
*   IMGUI: \[OSX\] Editor windows appear laggy and fail to repaint sometimes after performing various actions ([1409537](https://issuetracker.unity3d.com/issues/osx-editor-windows-appear-laggy-and-fail-to-repaint-sometimes-after-performing-various-actions))
...
*   Inspector Framework: Editor crashes when a GameObject is drag and dropped into a Script field ([1416878](https://issuetracker.unity3d.com/issues/unity-crashes-when-a-gameobject-is-drag-and-dropped-into-script-field))
...
*   Scene Management: Removing a Prefab and undoing actions makes GameObject being unable to be dragged into Project view ([1417571](https://issuetracker.unity3d.com/issues/removing-a-prefab-and-undoing-actions-makes-gameobject-being-unable-to-be-dragged-into-project-view))
...
*   UI Toolkit: Inspector fields alignment regression ([1422697](https://issuetracker.unity3d.com/issues/uitoolkit-inspector-fields-alignment-regression))
...
*   Web Platform: \[BuildError\] Constant il2cpp build error after OSX 12.3 Upgrade ([1412113](https://issuetracker.unity3d.com/issues/webgl-builderror-constant-il2cpp-build-error-after-osx-12-dot-3-upgrade))
    

### New 2022.2.0a11 Entries since 2022.2.0a10

#### Features

*   Graphics: Added a visual debug tool for buffer load/store actions.
    
*   UI Toolkit: Improved the UI/UX of the inspector displaying appropriate visual cues accordingly to the statuses of properties and providing more details about how their (properties) values are bound and resolved.
    
*   URP: All builtin URP shaders and URP ShaderGraph shaders will support Mesh LOD CrossFade, which you can select in `UniversalRenderPipelineAsset.lodCrossFadeDitheringType` property.
...
*   Android: Add Initial launch size metadata into AndroidManifest.xml for Chrome OS devices with Android R or higher installed (1360064)
...
*   Graphics: Fix crash on some platforms when calling Graphics.Blit(null, destination) with a destination texture larger than the backbuffer. ([1374709](https://issuetracker.unity3d.com/issues/crash-when-opening-project-with-weather-maker))
...
*   HDRP: Fixed error on LitTessellation material editor when Planar or Triplanar UVMapping is selected. (1404759)  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   IL2CPP: Corrected the behavior of `RuntimeInformation.IsOSPlatform` on desktop platforms. ([1399596](https://issuetracker.unity3d.com/issues/windows-fails-to-identify-as-windows-when-building-the-project-using-il2cpp-scripting-backend))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   macOS: Fixed `CopyTexture` to `RenderTexture` so it doesn't fail to update actual contents. ([1411741](https://issuetracker.unity3d.com/issues/metal-mac-black-2d-texture-when-using-readpixels-and-copytexture))  
...
*   Particles: Fix for glitching particle effects that showed up when using graphics jobs. (1417465)  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   Prefabs: Fixed "ArgumentException: Could not cast AssetImporter to PrefabImporter" error message appears while clicking on prefab's hierarchy. ([1413702](https://issuetracker.unity3d.com/issues/argumentexception-could-not-cast-assetimporter-to-prefabimporter-error-message-appears-while-clicking-on-prefabs-hierarchy))  
    _First seen in 2022.2.0a9._
    
*   Prefabs: Fixed two prefab bugs and made sure data loads correctly with the latest version. (1410570, 1410577)
    
*   Profiler: Added missing text to the Profiler Module Editor. (1417549)  
...
*   Scripting: Fixed when namespace handling for MonoBehaviour with newlines/tabs/spaces. ([1407718](https://issuetracker.unity3d.com/issues/monobehaviour-scripts-cause-compilation-errors-when-the-namespace-contains-a-line-break))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Serialization: Fix issue where a PropertyModification targetting a serialize reference instance which was no longer being present could cause memory corruption. (1415837)
...
*   Shaders: Fixed incorrect shadow caster pass being used when multiple pipelines were in a single shader. (1411270)  
    _First seen in 2022.2.0a9._
    
*   Shaders: Fixed so when you change line endings in shaders or include files, it doesn't cause recompilation of variants. ([1415557](https://issuetracker.unity3d.com/issues/line-ending-changes-in-shaders-or-includes-cause-full-shader-recompiles))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   Shaders: Fixed the compute shader compilation progress bar so it doesn't get stuck. (1415495)  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   VFX Graph: Fixed material inspector so it displays in outputs with shader graph. (1388531)  
    _First seen in 2022.2.0a1._
...
*   VFX Graph: VFX Graph gizmo could not be manipulated anymore (1398351)
...
*   XR: Fixed the Oculus Quest software keyboard to show up on InputField interactions. (1417991)  
    _This has already been backported to older releases and will not be mentioned in final notes._
    

#### New 2022.2.0a11 Package Changes since 2022.2.0a10
...
### Preview of Final 2022.2.0a11 Release Notes
...
#### Preview of Final 2022.2.0a11 Package changes
