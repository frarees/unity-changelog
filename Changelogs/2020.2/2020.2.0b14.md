...
*   IMGUI: Memory leak when continuously triggering repaint on a custom Editor window which redraws meshes on every repaint ([1293317](https://issuetracker.unity3d.com/issues/memory-leak-when-continuously-triggering-repaint-on-a-custom-editor-window-which-redraws-meshes-on-every-repaint))
    
*   Linux: InputSystem's Mouse delta values do not change when the Cursor lockState is set to Locked ([1248389](https://issuetracker.unity3d.com/issues/linux-inputsystems-mouse-delta-values-do-not-change-when-the-cursor-lockstate-is-set-to-locked))
    
*   Linux: Lego Microgame crashes the Linux Editor at "ScanFilter::Merge" when selecting a WebGL build location ([1295952](https://issuetracker.unity3d.com/issues/lego-microgame-crashes-the-linux-editor-at-scanfilter-merge-when-selecting-a-webgl-build-location))
...
*   Scripting: This one line fix should be added to the 2020.2/staging changes of the fix for 1288026 to make it complete. That fix landed 6 days ago.  
    ...
    _This is a change to a 2020.2.0b14 change, not seen in any released version, and will not be mentioned in final notes._  
    _Fixed in 2020.2.0b15._
...
*   Editor: fixed an issue where mouse dragging on macOS would impact editor performance. ([1281005](https://issuetracker.unity3d.com/issues/macos-mouse-input-gets-interrupted-when-changing-values-in-inspector-by-dragging-mouse))  
    _This is a change to a 2020.2.0a21 change, not seen in any released version, and will not be mentioned in final notes._
    
*   Editor: Fixed an issue where the bundle identifier could not be set in the generated Xcode project for iOS. ([1284759](https://issuetracker.unity3d.com/issues/ios-bundle-identifier-field-in-player-settings-no-longer-works-for-ios))  
...
*   Graphics: Fixed an issue where SRP would not firing visibility callbacks when an object is indirectly became visible due to shadow culling. ([1286377](https://issuetracker.unity3d.com/issues/assertion-failed-on-expression-errors-when-switching-shared-mesh-rapidly))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Graphics: Fixed missing type check from VFX component sheet, error possible if. ([1258022](https://issuetracker.unity3d.com/issues/crash-on-vfxpropertysheetserializedcomponent-readbackfromruntime-when-opening-the-visual-effect-graph-window))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   GraphView: Fixed an issue where the GraphView would become unresponsive after dropping a node in a stack. ([1275569](https://issuetracker.unity3d.com/issues/graphview-graph-view-becomes-unresponsive-when-adding-a-node-into-the-stack-node))  
...
*   macOS: Fixed an issue where non-development Apple silicon builds not running. ([1293633](https://issuetracker.unity3d.com/issues/non-development-apple-silicon-builds-dont-run))  
    _This is a change to a 2020.2.0 change, not seen in any released version, and will not be mentioned in final notes._
...
*   Scripting: Fixed a HttpClient WebException thrown when requesting with PostAsync. ([1276200](https://issuetracker.unity3d.com/issues/httpclient-webexception-when-doing-a-request-with-postasync))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   Scripting: Fixed a performance regression isuee when loading struct fields. ([1276888](https://issuetracker.unity3d.com/issues/performance-regression-matrix4x4-multiplication-significantly-slower))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   Scripting: Fixed an issue where the macOS Standalone player built for Apple silicon architecture would crash on startup with Mono scripting backend when running on macOS Big Sur Beta 6 or later. ([1278359](https://issuetracker.unity3d.com/issues/macos-apple-silicon-standalone-player-with-mono-scripting-backend-crashes-on-macos-big-sur-beta-6))  
...
*   Text: Fixed an issue where IME would get permanently disabled on focus and / or context change. ([1091254](https://issuetracker.unity3d.com/issues/korean-symbols-can-not-be-typed-into-gui-dot-textarea-korean-ime-is-disabled))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   WebGL: Fixed an issue with compressed single channel textures attempting to target unsupported BC4 compression format. ([1274662](https://issuetracker.unity3d.com/issues/webgl-settings-are-automatically-set-to-use-unsupported-format-when-the-texture-has-certain-settings-set))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   XR: Fixed an issue where the Lumin builds did not properly include source plugins when building the IL2CPP library. ([1218580](https://issuetracker.unity3d.com/issues/lumin-building-for-lumin-ignores-raw-c-or-cpp-plugin-files))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   XR: Fixed an issue where the Lumin builds needlessly packaged pre-compiled managed DLLs into the final MPK. ([1233875](https://issuetracker.unity3d.com/issues/lumin-il2cpp-converted-c-number-dlls-are-copied-to-the-build-file))  
    _This has already been backported to older releases and will not be mentioned in final notes._
    
*   XR: Fixed an issue where VSync did not displaying at all when VR enabled to 2020.2. ([1114382](https://issuetracker.unity3d.com/issues/vr-vsync-is-not-displayed-in-the-profiler-when-running-with-vr-enabled))  
    _This has already been backported to older releases and will not be mentioned in final notes._
...
*   2D: Fixed an editor crashes when packing Sprites. ([1259149](https://issuetracker.unity3d.com/issues/2d-spriteatlas-v2-editor-crashes-when-packing-sprites))
...
*   2D: Fixed Bounds/RectInt.allPositionsWithin returning positions with a size 0 for any axis. ([1227811](https://issuetracker.unity3d.com/issues/boundsint-allpositionswithin-return-points-when-boundsint-x-or-y-size-is-zero-and-non-zero-in-other-axes))
...
*   GI: Fixed an issue in the GPU->CPU fallback logic when no valid OpenCL context could be created. ([1208704](https://issuetracker.unity3d.com/issues/lighting-baking-stucks-when-the-static-object-starts-to-bake-with-progressive-gpu-lightmapper))
...
*   GI: Fixed case where Editor hangs on close and when baking is canceled. ([1235769](https://issuetracker.unity3d.com/issues/editor-hangs-on-close-when-baking-is-canceled-and-cpu-usage-is-100-percent))
...
*   Editor: Updated the documentation of CreateEditor API to indicate that it has to be destroyed explicitly. ([1250942](https://issuetracker.unity3d.com/issues/memory-leak-when-calling-editor-dot-createeditor))
