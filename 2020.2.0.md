...
-   Editor: Fixed cancel of multiple objects rename that would set their names to \"\" instead of keeping their original names. ([1218183](https://issuetracker.unity3d.com/issues/multiple-selected-game-objects-with-different-names-are-renamed-to-when-pressing-esc-key-while-typing-a-new-name))
...
-   GI: Ensured thatTempBuffer is not released in memory when users are using deprecated Realtime Global Illumination functionality.
...
-   Kernel: AlignOf returns C# required structure alignment, instead of 4.
